 ...
 */
public class StpColFacDeletionCmd extends AbstractStpCommand {
    ...
    public boolean deletedColFac(Map ctx) throws Exception {
        ...
        ICollateralTrxValue obCollateralTrxValue = (ICollateralTrxValue) ctx.get(COL_TRX_VALUE);
        ICollateral obCollateral = (ICollateral) obCollateralTrxValue.getCollateral();
        ICollateralLimitMap[] obCollateralLimitMap = (ICollateralLimitMap[]) obCollateral.getCollateralLimits();
        IStpMasterTrans obStpMasterTrans = (IStpMasterTrans) ctx.get(STP_TRX_VALUE);
        Map stpTransMap = StpCommandUtil.getTrxMap(obStpMasterTrans);
        ...
        if (existObStpTrans != null) {
            for (int k = 0; k < existObStpTrans.length; k++) {
                ...
                if (existObStpTrans[k].getTrxType().equals(TRX_TYPE_COL_LINK_FAC_CREATE) && TRX_SUCCESS.equals(existObStpTrans[k].getStatus())) {
                    if (!ArrayUtils.isEmpty(obCollateralLimitMap)) {
                        for (int i = 0; i < obCollateralLimitMap.length; i++) {
                            if (existObStpTrans[k].getReferenceId().equals(new Long(obCollateralLimitMap[i].getChargeID()))) {
                                deletedIns = false;
                                break;
                            } else {
                                deletedIns = true;
                    ...
                    if (deletedIns) {
                        ...
                        if (obStpTrans != null) {
                            ...
                            List aList = getStpTransBusManager().getColLimitMapByID(existObStpTrans[k].getReferenceId());
                            ArrayList iList = (ArrayList) aList.get(0);
                            if (iList != null) {
                                addHdrField.put(MSG_APP_NO_FIELD, iList.get(0));
                                addHdrField.put(MSG_FAC_CODE_FIELD, iList.get(1));
