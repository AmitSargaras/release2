 ...
 * @since 19 Sept 2008
 */
public class StpFacRelDeletionCmd extends AbstractStpCommand implements IStpTransType, IStpConstants {
    ...
    public boolean execute(Map ctx) throws Exception {
        ...
        if (StringUtils.equals(obFacilityTrxValue.getStatus(), ICMSConstant.STATE_LOADING_DELETE)) {
            ...
            for (Iterator iterator = stpTransMap.values().iterator(); iterator.hasNext();) {
                ...
                if (StringUtils.equals(obStpTrans.getTrxType(), TRX_TYPE_FAC_RELATION_CREATE) && TRX_SUCCESS.equals(obStpTrans.getStatus())) {
                    if (obFacilityMaster.getFacilityRelationshipSet() != null && obFacilityMaster.getFacilityRelationshipSet().size() > 0) {
                        for (Iterator iterator1 = obFacilityMaster.getFacilityRelationshipSet().iterator(); iterator1.hasNext();) {
                            IFacilityRelationship relationship = (IFacilityRelationship) iterator1.next();
                            if (obStpTrans.getReferenceId().equals(relationship.getCmsRefId())) {
                                deleteInd = false;
                                break;
                            }
                        }
                    ...
                    if (deleteInd) {
                        ...
                        // StpTrans OB will be null if delete trx had been succeed
                        if (obStpTrans != null) {
                            List aList = getStpTransBusManager().getFacilityRelCifNumberByID(obStpTrans.getReferenceId());
                            if (aList != null && aList.size() > 0) {
                                String cifNumber = (String) aList.get(0);
                                fieldValMap.put("ACCIFN", cifNumber);
