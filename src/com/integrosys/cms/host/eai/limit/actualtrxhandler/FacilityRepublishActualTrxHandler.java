...
public class FacilityRepublishActualTrxHandler extends FacilityActualTrxHandler {

	public Message persistActualTrx(Message msg) {
		return super.persistActualTrx(msg);
	}
	...
	public Message preprocess(Message msg) {
		...
		while (iter.hasNext()) {
			...
			if (limit.getFacilityIncrementals() != null && !limit.getFacilityIncrementals().isEmpty()) {
				for (Iterator iterator = limit.getFacilityIncrementals().iterator(); iterator.hasNext();) {
					FacilityIncremental incremental = (FacilityIncremental) iterator.next();
					incremental.setUpdateStatusIndicator(limit.getLimitGeneral().getUpdateStatusIndicator());
					incremental.setChangeIndicator(limit.getLimitGeneral().getChangeIndicator());
				}
			}
