 ...
 * $Header: /home/cms2/cvsroot/cms2/src/com/integrosys/cms/app/checklist/trx/OfficeCheckListApproveOperation.java,v 1.18 2006/10/05 02:51:13 hmbao Exp $
 */
...
import com.integrosys.cms.app.eventmonitor.waiverdeferral.DummyMonWaiverDeferalApproval;
 ...
 */
public class OfficeCheckListApproveOperation extends CMSTrxForwardOperationBase {
	private ManagerApproveUpdateCheckListReceiptOperation op = new ManagerApproveUpdateCheckListReceiptOperation();

	private ICheckListItem[] requestItemsOfStage = null;
	...
	public ITrxValue preProcess(ITrxValue value) throws TrxOperationException {
		...
		DefaultLogger.debug(this, "-----preProcess...going to ManagerApproveUpdateCheckListReceiptOperation");
		op.setSendCheckListItemNotificationInd((ICheckListTrxValue) value);
		ICheckList checkList = ((ICheckListTrxValue) value).getStagingCheckList();
		ICheckListItem[] items = checkList.getCheckListItemList();

		requestItemsOfStage = filter(items, new String[] { ICMSConstant.STATE_ITEM_PENDING_DEFERRAL,
	...
	public ITrxResult performProcess(ITrxValue value) throws TrxOperationException {
		DefaultLogger.debug(this, "-----performProcess...going to ManagerApproveUpdateCheckListReceiptOperation");
	...
	public ITrxResult postProcess(ITrxResult result) throws TrxOperationException {
		DefaultLogger.debug(this, "-----postProcess...going to ManagerApproveUpdateCheckListReceiptOperation");
		// Auto-generated method stub
	...
	private void assembleCommonNotifInfo(OBWaiverDeferralInfo info, ICheckListTrxValue trxValue) {
		info.setCheckListType(trxValue.getStagingCheckList().getCheckListType());
		info.setApprovalDate(trxValue.getTransactionDate());
		info.setLastComment(trxValue.getComment());
		info.setItemList(requestItemsOfStage);
		...
		info.setDetails(DummyMonWaiverDeferalApproval.EVEVNT_ID + " , "
	...
	private void fireNotification(OBWaiverDeferralInfo info) throws Exception {
		DummyMonWaiverDeferalApproval mon = new DummyMonWaiverDeferalApproval();
		...
		for (int i = 0; i < inItems.length; i++) {
			for (int j = 0; j < status.length; j++) {
				if ((inItems[i].getItemStatus() != null) && ((inItems[i].getItemStatus()).equals(status[j]))) {
					ICheckListItem reqItem = inItems[i];
					// This item approved
					if (status[j].equals(ICMSConstant.STATE_ITEM_PENDING_DEFERRAL)) {
						reqItem.setItemStatus(ICMSConstant.STATE_ITEM_DEFERRED);
					}
					else if (status[j].equals(ICMSConstant.STATE_ITEM_PENDING_WAIVER)) {
						reqItem.setItemStatus(ICMSConstant.STATE_ITEM_WAIVED);
					}
					items.add(reqItem);
					break;
				}
