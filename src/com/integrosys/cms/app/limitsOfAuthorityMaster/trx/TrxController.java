	...
	private ITrxOperation factoryOperation(ITrxValue value, ITrxParameter param) throws TrxParameterException {
		...
		if (toState != null) {
			if (toState.equals(ICMSConstant.STATE_DRAFT)) {
				if (action.equals(ICMSConstant.LimitsOfAuthorityMaster.ACTION_MAKER_CREATE_LIMITS_OF_AUTHORITY)) {
					return (ITrxOperation) getNameTrxOperationMap().get("MakerUpdateDraftCreateOperation");
				}
				if (action.equals(ICMSConstant.LimitsOfAuthorityMaster.ACTION_MAKER_UPDATE_LIMITS_OF_AUTHORITY)) {
					return (ITrxOperation) getNameTrxOperationMap().get("MakerUpdateOperation");
				}
			}
		}
		if (action.equals(ICMSConstant.LimitsOfAuthorityMaster.ACTION_MAKER_SAVE_LIMITS_OF_AUTHORITY)) {
			return (ITrxOperation) getNameTrxOperationMap().get("MakerSaveOperation");
		}
		if (action.equals(ICMSConstant.LimitsOfAuthorityMaster.ACTION_MAKER_SAVE_UPDATE_LIMITS_OF_AUTHORITY)) {
		...
		if ((toState == null) || (toState.equals(ICMSConstant.STATE_ND))) {
			...
			if (action.equals(ICMSConstant.LimitsOfAuthorityMaster.ACTION_CHECKER_APPROVE_LIMITS_OF_AUTHORITY)) {
				return (ITrxOperation) getNameTrxOperationMap().get("CheckerApproveCreateOperation");
			}

			if (action.equals(ICMSConstant.LimitsOfAuthorityMaster.ACTION_CHECKER_REJECT_LIMITS_OF_AUTHORITY)) {
			...
			if (action.equals(ICMSConstant.LimitsOfAuthorityMaster.ACTION_MAKER_UPDATE_LIMITS_OF_AUTHORITY)) {
				return (ITrxOperation) getNameTrxOperationMap().get("MakerUpdateOperation");
			} else if (action.equals(ICMSConstant.LimitsOfAuthorityMaster.ACTION_MAKER_DELETE_LIMITS_OF_AUTHORITY)) {
			...
			} else if (action.equals(ICMSConstant.LimitsOfAuthorityMaster.ACTION_CHECKER_REJECT_LIMITS_OF_AUTHORITY)) {
				return (ITrxOperation) getNameTrxOperationMap().get("CheckerRejectOperation");
			}
			throw new TrxParameterException("Unknown Action: " + action + " with toState: " + toState);
		} else if (toState.equals(ICMSConstant.STATE_REJECTED)) {
			if (action.equals(ICMSConstant.LimitsOfAuthorityMaster.ACTION_MAKER_EDIT_REJECTED_LIMITS_OF_AUTHORITY)) {
			...
			} else if (action.equals(ICMSConstant.LimitsOfAuthorityMaster.ACTION_MAKER_CLOSE_REJECTED_LIMITS_OF_AUTHORITY)) {
				if (fromState.equals(ICMSConstant.STATE_PENDING_CREATE)) {
		...
		} else if (action.equals(ICMSConstant.LimitsOfAuthorityMaster.ACTION_MAKER_CLOSE_DRAFT_LIMITS_OF_AUTHORITY)) {
