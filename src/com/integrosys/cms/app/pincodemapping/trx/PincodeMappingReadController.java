...
public class PincodeMappingReadController extends AbstractTrxController implements
ITrxOperationFactory {

	/**
	 * Default Constructor
	 */
	public PincodeMappingReadController() {
	}
	...
	public ITrxOperation getOperation(ITrxValue value, ITrxParameter param)
		...
		if (action != null) {
			if (action.equals(ICMSConstant.ACTION_READ_PINCODE_MAPPING)) {
				return (ITrxOperation) getNameTrxOperationMap().get(
						"ReadPincodeMappingOperation");
			} else if (action.equals(ICMSConstant.ACTION_READ_PINCODE_MAPPING_ID)) {
				return (ITrxOperation) getNameTrxOperationMap().get(
						"ReadPincodeMappingIDOperation");
			} else {
