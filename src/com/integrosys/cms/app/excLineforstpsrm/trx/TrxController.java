	...
	private ITrxOperation factoryOperation(ITrxValue value, ITrxParameter param) throws TrxParameterException {
		...
		if (toState != null) {
			if (toState.equals(ICMSConstant.STATE_DRAFT)) {
				if (action.equals(ICMSConstant.ACTION_MAKER_CREATE_EXC_LINE_FR_STP_SRM)) {
					return (ITrxOperation) getNameTrxOperationMap().get("MakerUpdateDraftCreateOperation");
				}
				if (action.equals(ICMSConstant.ACTION_MAKER_UPDATE_EXC_LINE_FR_STP_SRM)) {
					return (ITrxOperation) getNameTrxOperationMap().get("MakerUpdateOperation");
				}
			}
		}
		if (action.equals(ICMSConstant.ACTION_MAKER_SAVE_EXC_LINE_FR_STP_SRM)) {
			return (ITrxOperation) getNameTrxOperationMap().get("MakerSaveOperation");
		}
		if (action.equals(ICMSConstant.ACTION_MAKER_SAVE_UPDATE_EXC_LINE_FR_STP_SRM)) {
		...
		if ((toState == null) || (toState.equals(ICMSConstant.STATE_ND))) {
			...
			if (action.equals(ICMSConstant.ACTION_CHECKER_APPROVE_EXC_LINE_FR_STP_SRM)) {
				return (ITrxOperation) getNameTrxOperationMap().get("CheckerApproveCreateOperation");
			}

			if (action.equals(ICMSConstant.ACTION_CHECKER_REJECT_EXC_LINE_FR_STP_SRM)) {
			...
			if (action.equals(ICMSConstant.ACTION_MAKER_UPDATE_EXC_LINE_FR_STP_SRM)) {
				return (ITrxOperation) getNameTrxOperationMap().get("MakerUpdateOperation");
			} else if (action.equals(ICMSConstant.ACTION_MAKER_DELETE_EXC_LINE_FR_STP_SRM)) {
			...
			} else if (action.equals(ICMSConstant.ACTION_CHECKER_REJECT_EXC_LINE_FR_STP_SRM)) {
				return (ITrxOperation) getNameTrxOperationMap().get("CheckerRejectOperation");
			}
			throw new TrxParameterException("Unknown Action: " + action + " with toState: " + toState);
		} else if (toState.equals(ICMSConstant.STATE_REJECTED)) {
			if (action.equals(ICMSConstant.ACTION_MAKER_EDIT_REJECTED_EXC_LINE_FR_STP_SRM)) {
			...
			} else if (action.equals(ICMSConstant.ACTION_MAKER_CLOSE_REJECTED_EXC_LINE_FR_STP_SRM)) {
				if (fromState.equals(ICMSConstant.STATE_PENDING_CREATE)) {
		...
		} else if (action.equals(ICMSConstant.ACTION_MAKER_CLOSE_DRAFT_EXC_LINE_FR_STP_SRM)) {
