...
public class DownloadReportCommand extends AbstractCommand  implements IFileUploadConstants {
	...
	public HashMap doExecute(HashMap map) throws CommandProcessingException,CommandValidationException {
		...
		try {
			...
			fileName = IFileUploadConstants.FILEUPLOAD_LEIDETAILS_TRANS_SUBTYPE+"_"+DateFormatUtils.format(new Date(), "yyyyMMddHHmmss")+".xls";
			

			List<OBLeiDetailsFile> totalUploadedList = (List<OBLeiDetailsFile>) map.get("totalUploadedList");
			List<OBLeiDetailsFile> listOfErrorDate = (List<OBLeiDetailsFile>) map.get("listOfErrorDate");
			List<OBLeiDetailsFile> errorList = (List<OBLeiDetailsFile>) map.get("errorList");
			List<Object[]> totalUploadedObjList = new ArrayList<Object[]>();
			Object[] obj = null;
			if (totalUploadedList != null && totalUploadedList.size() > 0) {
				for (OBLeiDetailsFile l1 : totalUploadedList) {
					obj = new Object[5];
					...
					if(null==l1.getLeiCode()){
					obj[1] ="";	
					}else{
					obj[1] = l1.getLeiCode();
					}
					if(null==l1.getLeiExpDate()){
						obj[2] ="";	
						}else{
							obj[2] = simpleDateFormat.format(l1.getLeiExpDate());
						}
			...
			if (errorList != null && errorList.size() > 0) {
				for (OBLeiDetailsFile l1 : errorList) {
					obj = new Object[5];
					if(null == l1.getPartyId()) {
						obj[0] = "";
					}else {
				...
				Map parameters = generateParamatersMap(IFileUploadConstants.FILEUPLOAD_LEIDETAILS_TRANS_SUBTYPE);
	...
	private Map generateParamatersMap(String uploadFileType) {
		...
		columnLabels= new String[]{"PARTY ID","LEI Code","LEI Expiry Date","Status", "Reason"};
		columnWidths= new Integer[]{50,50,50,50,50};
