...
public class CheckerApproveInsertMasterOperation extends AbstractHolidayTrxOperation {
     ...
     */
    private IHolidayTrxValue actualMaster(IHolidayTrxValue idxTrxValue) throws HolidayException, TrxParameterException, TransactionException, ConcurrentUpdateException {
        try {
            IFileMapperId staging = idxTrxValue.getStagingFileMapperID();
            IFileMapperId actual = idxTrxValue.getFileMapperID();

            IFileMapperId updatedActual = getHolidayFileMapperIdBusManager().insertHoliday(staging,idxTrxValue);           
