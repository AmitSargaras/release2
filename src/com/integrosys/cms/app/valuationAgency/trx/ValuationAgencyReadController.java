 ...
 */
public class ValuationAgencyReadController extends AbstractTrxController
	 ...
	 * Default Constructor
	 */
	public ValuationAgencyReadController() {
	}
	...
	public ITrxOperation getOperation(ITrxValue value, ITrxParameter param)
		...
		if (action != null) {
			if (action.equals(ICMSConstant.ACTION_READ_VALUATION_AGENCY)) {
				return (ITrxOperation) getNameTrxOperationMap().get(
						"ReadValuationAgencyOperation");
			} else if (action
					.equals(ICMSConstant.ACTION_READ_VALUATION_AGENCY_ID)) {
				return (ITrxOperation) getNameTrxOperationMap().get(
						"ReadValuationAgencyIDOperation");
			} else {
