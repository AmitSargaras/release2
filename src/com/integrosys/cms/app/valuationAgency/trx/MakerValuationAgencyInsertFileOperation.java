...
public class MakerValuationAgencyInsertFileOperation extends AbstractValuationAgencyTrxOperation {

	   /**
	    * Defaulc Constructor
	    */
	    public MakerValuationAgencyInsertFileOperation()
	    {
	        super();
	    }
	  ...
	  //      DefaultLogger.debug(this, " ---- trxValue.getStagingPrIdx() is null ? ----- " + (trxValue.getStagingFileMapperID()==null));
		...
		*/
		private IValuationAgencyTrxValue createFileIdTransaction(IValuationAgencyTrxValue anICCIValuationAgencyTrxValue) throws TrxOperationException,ValuationAgencyException
		{
			try
			{
				anICCIValuationAgencyTrxValue = prepareInsertTrxValue(anICCIValuationAgencyTrxValue);
				ICMSTrxValue trxValue = createTransaction(anICCIValuationAgencyTrxValue);
	            OBValuationAgencyTrxValue valuationAgencyTrxValue = new OBValuationAgencyTrxValue (trxValue);
	            valuationAgencyTrxValue.setStagingFileMapperID(anICCIValuationAgencyTrxValue.getStagingFileMapperID());
	            valuationAgencyTrxValue.setFileMapperID(anICCIValuationAgencyTrxValue.getFileMapperID());
