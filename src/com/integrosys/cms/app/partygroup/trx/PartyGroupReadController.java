...
/**
 * @author bharat waghela Read Party group Controller
 */
public class PartyGroupReadController extends AbstractTrxController implements
		ITrxOperationFactory {
	 ...
	 * Default Constructor
	 */
	public PartyGroupReadController() {
	}
	...
	public ITrxOperation getOperation(ITrxValue value, ITrxParameter param)
		...
		if (action != null) {
			if (action.equals(ICMSConstant.ACTION_READ_PARTY_GROUP)) {
				return (ITrxOperation) getNameTrxOperationMap().get(
						"ReadPartyGroupOperation");
			} else if (action.equals(ICMSConstant.ACTION_READ_PARTY_GROUP_ID)) {
				return (ITrxOperation) getNameTrxOperationMap().get(
						"ReadPartyGroupIDOperation");
			} else {
