...
public class ExpiredCCNonBorrowerDocumentNotificationDAO extends AbstractDocExpiryDAO {
	private DBUtil dbUtil;

	private static String SELECT_CHECKLIST_ITEM = null;
	...
	static {
		String theSQL = " SELECT s.doc_item_id, s.doc_item_ref, citem.doc_description, citem.status, citem.doc_date, "
				...
				+ "       stage_checklist.doc_orig_country cms_orig_country, "
				+ "       stage_checklist.doc_orig_organisation cms_orig_organisation, "
				+ "       sci_le_main_profile.lmp_sgmnt_code_value AS SEGMENT, "
				+ "       calendar_days (cast(? as timestamp), citem.expiry_date) AS daysdue, "
				...
				+ "       cms_checklist_item citem, " + "       TRANSACTION t, " + "       sci_le_sub_profile, "
				+ "       sci_le_main_profile, " + "       stage_checklist "
				+ " WHERE s.checklist_id = stage_checklist.checklist_id "
				+ "   AND stage_checklist.cms_lmp_sub_profile_id = "
				...
				+ "   AND stage_checklist.CATEGORY = 'CC' " + "   AND stage_checklist.sub_category = 'NON_BORROWER' "
				+ "   AND t.staging_reference_id = s.checklist_id " + "   AND t.reference_id = citem.checklist_id "
				+ "   AND citem.expiry_date IS NOT NULL " + "   AND s.doc_item_ref = citem.doc_item_ref "
				+ "   AND (cust.status <> 'PERM_UPLIFTED' OR cust.status IS NULL) "
				+ "   AND citem.status = 'COMPLETED' "
				+ "   AND calendar_days (cast(? as timestamp), citem.expiry_date) ";

		StringBuffer strBuf = new StringBuffer();
		strBuf.append(theSQL).append(getEqualComparator()).append(" ? ");

		SELECT_CHECKLIST_ITEM = strBuf.toString();
	...
	public IMonitorDAOResult getInitialSet(IRuleParam ruleParam) throws MonitorDaoResultRetrievalFailureException {
		StringBuffer sqlBuffer = new StringBuffer(SELECT_CHECKLIST_ITEM);
		ResultSet rs = null;
		try {
			...
			if (ruleParam.hasCountryCode()) {
				sqlBuffer.append(" AND stage_checklist.DOC_ORIG_COUNTRY = '");
