...
public class ExpiredCCDocumentNotificationDAO extends AbstractDocExpiryDAO {
	...
	private String getCheckListItemSQL() {
		String theSQL = " SELECT s.doc_item_id, s.doc_item_ref, citem.doc_description, citem.status, citem.doc_date, "
				...
				+ "		(select sci_security_dtl_id from cms_security sec where sec.cms_collateral_id = stage_checklist.cms_collateral_id) as sci_security_dtl_id, "
				+ "		(select subtype_name from view_collateral_maturity_date col where col.cms_collateral_id = stage_checklist.cms_collateral_id) as subtype_name, "
				+ "		(select security_maturity_date from view_collateral_maturity_date col where col.cms_collateral_id = stage_checklist.cms_collateral_id) as security_maturity_date "
	...
	public IMonitorDAOResult getInitialSet(IRuleParam ruleParam) throws MonitorDaoResultRetrievalFailureException {
		StringBuffer sqlBuffer = new StringBuffer(getCheckListItemSQL());
		ResultSet rs = null;
		try {
			...
			if (ruleParam.hasCountryCode()) {
				...
				sqlBuffer.append(param.getCountryCode());
				sqlBuffer.append("'");
			}
