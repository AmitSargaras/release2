package com.integrosys.cms.app.systemBankBranch.bus;

import com.integrosys.base.businfra.contact.OBCountry;
import com.integrosys.base.techinfra.util.ReplicateUtils;
import com.integrosys.cms.app.geography.city.bus.OBCity;
import com.integrosys.cms.app.geography.region.bus.OBRegion;
import com.integrosys.cms.app.geography.state.bus.OBState;
import com.integrosys.cms.app.systemBank.bus.OBSystemBank;

/**
 * Created by IntelliJ IDEA.
 * Replication utility used for replicating stock to interact with persistent
 *
 * @author Andy Wong
 * @since 16 Sep 2008
 */
public abstract class SystemBankBranchReplicationUtils {

	/**
	 * <p>
	 * Replicate Property Index which is ready to create a entity into
	 * persistent storage. Normally to create a staging copy will use this.
	 *
	 * <p>
	 * <b>note</b> must know which field(s) need to be filtered when doing
	 * replication
	 *
	 */
	public static ISystemBankBranch replicateSystemBankBranchForCreateStagingCopy(ISystemBankBranch systemBankBranch) {

       
          
      

        ISystemBankBranch replicatedIdx = (ISystemBankBranch) ReplicateUtils.replicateObject(systemBankBranch,
				new String[] { "systemBankCode","country","state","region","cityTown"});
        if (replicatedIdx.getSystemBankCode() == null)
        	replicatedIdx.setSystemBankCode(new OBSystemBank());

        replicatedIdx.getSystemBankCode().setId(systemBankBranch.getSystemBankCode().getId());
        replicatedIdx.getSystemBankCode().setSystemBankCode(systemBankBranch.getSystemBankCode().getSystemBankCode());
       // replicatedIdx.getSystemBankCode().setId(systemBankBranch.getSystemBankCode().getId());
        if(systemBankBranch.getCountry()!=null){
        if (replicatedIdx.getCountry() == null)
        	replicatedIdx.setCountry(new com.integrosys.cms.app.geography.country.bus.OBCountry());

        replicatedIdx.getCountry().setIdCountry(systemBankBranch.getCountry().getIdCountry());
        }
        if(systemBankBranch.getState()!=null){
        if (replicatedIdx.getState() == null)
        	replicatedIdx.setState(new OBState());

        replicatedIdx.getState().setIdState(systemBankBranch.getState().getIdState());
        }
        if(systemBankBranch.getRegion()!=null){
        if (replicatedIdx.getRegion() == null)
        	replicatedIdx.setRegion(new OBRegion());

        replicatedIdx.getRegion().setIdRegion(systemBankBranch.getRegion().getIdRegion());
        }
        if(systemBankBranch.getCityTown()!=null){
        if (replicatedIdx.getCityTown() == null)
        	replicatedIdx.setCityTown(new OBCity());

        replicatedIdx.getCityTown().setIdCity(systemBankBranch.getCityTown().getIdCity());
        }
        
		

        return replicatedIdx;
	}
}