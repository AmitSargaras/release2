 ...
 * $Header: /home/cms2/cvsroot/cms2/src/com/integrosys/cms/app/generatereq/trx/MakerCreateDeferralRequestOperation.java,v 1.3 2003/09/19 09:31:07 hltan Exp $
 */
...
/**
 * This operation creates a pending deferral request
 * 
 * @author $Author: hltan $
 * @version $Revision: 1.3 $
 * @since $Date: 2003/09/19 09:31:07 $ Tag: $Name: $
 */
public class MakerCreateDeferralRequestOperation extends AbstractDeferralRequestTrxOperation {
	/**
	 * Defaulc Constructor
	 */
	public MakerCreateDeferralRequestOperation() {
		super();
	}
	...
	public String getOperationName() {
		return ICMSConstant.ACTION_MAKER_GENERATE_DEFERRAL_REQ;
	}
		...
		try {
			...
			for (int ii = 0; ii < itemList.length; ii++) {
				if (!checklist.contains(new Long(itemList[ii].getCheckListID()))) {
					...
					proxy.systemGenerateCheckListDeferral(checkListTrxValue);
				}
			}
		}
	...
	public ITrxResult performProcess(ITrxValue anITrxValue) throws TrxOperationException {
		...
		trxValue = createStagingDeferralRequest(trxValue);
		trxValue = createDeferralRequestTransaction(trxValue);
	 ...
	 */
	private IDeferralRequestTrxValue createDeferralRequestTransaction(
		...
		try {
			...
			ICMSTrxValue trxValue = createTransaction(anIDeferralRequestTrxValue);
			OBDeferralRequestTrxValue deferralReqTrxValue = new OBDeferralRequestTrxValue(trxValue);
			deferralReqTrxValue.setStagingDeferralRequest(anIDeferralRequestTrxValue.getStagingDeferralRequest());
			deferralReqTrxValue.setDeferralRequest(anIDeferralRequestTrxValue.getDeferralRequest());
			return deferralReqTrxValue;
		}
		catch (TransactionException ex) {
			throw new TrxOperationException(ex);
		}
