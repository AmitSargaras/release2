...
public class InsertFileTrxController extends CMSTrxController {
    ...
    public String getInstanceName() {
        return ICMSConstant.INSTANCE_INSERT_CITY;
    }
        ...
        if(toState!=null){
         ...
         if ((toState == null) || (toState.equals(ICMSConstant.STATE_ND))) {
            if (action.equals(ICMSConstant.ACTION_MAKER_FILE_INSERT)) {
                return (ITrxOperation) getNameTrxOperationMap().get("MakerCityInsertFileOperation");
            }
            if (action.equals(ICMSConstant.ACTION_CHECKER_FILE_MASTER)) {
                return (ITrxOperation) getNameTrxOperationMap().get("CheckerCreateCityOperation");
            }
            throw new TrxParameterException("Unknown Action: " + action + " with toCity: " + toState);
             ...
             if (action.equals(ICMSConstant.ACTION_MAKER_INSERT_CLOSE_REJECTED_MASTER)) {
                if (fromState.equals(ICMSConstant.STATE_PENDING_INSERT)) {
                    return (ITrxOperation) getNameTrxOperationMap().get("MakerCloseRejectedInsertMasterCityOperation");
                }
            }
            throw new TrxParameterException("Unknown Action: " + action + " with toCity: " + toState);
            ...
            if (action.equals(ICMSConstant.ACTION_CHECKER_INSERT_APPROVE_MASTER)) {
                return (ITrxOperation) getNameTrxOperationMap().get("CheckerApproveInsertMasterCityOperation");
            }
            if (action.equals(ICMSConstant.ACTION_CHECKER_INSERT_REJECT_MASTER)) {
                return (ITrxOperation) getNameTrxOperationMap().get("CheckerRejectInsertMasterCityOperation");
            }
           
           
            throw new TrxParameterException("Unknown Action: " + action + " with toCity: " + toState);
        } 
        throw new TrxParameterException("To City does not match presets! No operations found!");
    }
