...
public class CheckerApproveInsertMasterOperation extends AbstractCityTrxOperation {
     ...
     */
    private ICityTrxValue actualMaster(ICityTrxValue idxTrxValue) throws NoSuchGeographyException, TrxParameterException, TransactionException, ConcurrentUpdateException {
        try {
            IFileMapperId staging = idxTrxValue.getStagingFileMapperID();
            IFileMapperId actual = idxTrxValue.getFileMapperID();

            IFileMapperId updatedActual = getCityFileMapperIdBusManager().insertCity(staging,idxTrxValue);           
