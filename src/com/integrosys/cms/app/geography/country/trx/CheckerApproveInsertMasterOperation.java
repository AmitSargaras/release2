...
public class CheckerApproveInsertMasterOperation extends AbstractCountryTrxOperation {
     ...
     */
    private ICountryTrxValue actualMaster(ICountryTrxValue idxTrxValue) throws CountryException, TrxParameterException, TransactionException, ConcurrentUpdateException {
        try {
            IFileMapperId staging = idxTrxValue.getStagingFileMapperID();
            IFileMapperId actual = idxTrxValue.getFileMapperID();

            IFileMapperId updatedActual = getCountryFileMapperIdBusManager().insertCountry(staging,idxTrxValue);           
