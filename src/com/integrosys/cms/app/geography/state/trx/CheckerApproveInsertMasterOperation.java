...
public class CheckerApproveInsertMasterOperation extends AbstractStateTrxOperation {
     ...
     */
    private IStateTrxValue actualMaster(IStateTrxValue idxTrxValue) throws NoSuchGeographyException, TrxParameterException, TransactionException, ConcurrentUpdateException {
        try {
            IFileMapperId staging = idxTrxValue.getStagingFileMapperID();
            IFileMapperId actual = idxTrxValue.getFileMapperID();

            IFileMapperId updatedActual = getStateFileMapperIdBusManager().insertState(staging,idxTrxValue);           
