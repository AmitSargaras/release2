...
/**
 * @author govind.sahu
 *  CreditApproval Trx controller to manage trx operations
 */
public class InsertFileTrxController extends CMSTrxController {
    ...
    public String getInstanceName() {
        return ICMSConstant.INSTANCE_INSERT_CREDIT_APPROVAL;
    }
        ...
        if(toState!=null){
         ...
         if ((toState == null) || (toState.equals(ICMSConstant.STATE_ND))) {
            if (action.equals(ICMSConstant.ACTION_MAKER_FILE_INSERT)) {
                return (ITrxOperation) getNameTrxOperationMap().get("MakerCreditApprovalInsertFileOperation");
            }
            if (action.equals(ICMSConstant.ACTION_CHECKER_FILE_MASTER)) {
                return (ITrxOperation) getNameTrxOperationMap().get("CheckerCreateCreditApprovalOperation");
            }
