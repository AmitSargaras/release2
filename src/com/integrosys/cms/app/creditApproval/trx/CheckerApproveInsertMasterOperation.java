...
public class CheckerApproveInsertMasterOperation extends AbstractCreditApprovalTrxOperation {
     ...
     */
    private ICreditApprovalTrxValue actualMaster(ICreditApprovalTrxValue idxTrxValue) throws CreditApprovalException, TrxParameterException, TransactionException, ConcurrentUpdateException {
        try {
            IFileMapperId staging = idxTrxValue.getStagingFileMapperID();
            IFileMapperId actual = idxTrxValue.getFileMapperID();

            IFileMapperId updatedActual = getCreditApprovalFileMapperIdBusManager().insertCreditApproval(staging,idxTrxValue);           
