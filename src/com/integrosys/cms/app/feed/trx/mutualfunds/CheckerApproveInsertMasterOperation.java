...
public class CheckerApproveInsertMasterOperation extends AbstractMutualFundsTrxOperation {
     ...
     */
    private IMutualFundsFeedGroupTrxValue actualMaster(IMutualFundsFeedGroupTrxValue idxTrxValue) throws CreditApprovalException, TrxParameterException, TransactionException, ConcurrentUpdateException {
        try {
            IFileMapperId staging = idxTrxValue.getStagingFileMapperID();
            IFileMapperId actual = idxTrxValue.getFileMapperID();

            IFileMapperId updatedActual = getMutualfundsFeedFileMapperIdBusManager().insertMutualFundsFeedEntry(staging,idxTrxValue);           
