 ...
 */
public class ReadBondGroupOperation extends AbstractBondTrxOperation implements ITrxReadOperation {

	/**
	 * Defaulc Constructor
	 */
	public ReadBondGroupOperation() {
		super();
	}
	...
	public String getOperationName() {
		return ICMSConstant.ACTION_READ_BOND_FEED_GROUP;
	}
	...
	public ITrxValue getTransaction(ITrxValue value) throws TransactionException {
		...
		IBondFeedGroupTrxValue vv = (IBondFeedGroupTrxValue) value;
		...
		try {
			...
			if ((trxId == null) || trxId.equals("")) {
				...
				IBondFeedGroup feedGroup = getBondFeedBusManager().getBondFeedGroup(ICMSConstant.BOND_FEED_GROUP_TYPE);

				DefaultLogger.debug(this, "bond feed group id = " + feedGroup.getBondFeedGroupID());

				trxValue = trxManager.getTrxByRefIDAndTrxType(String.valueOf(feedGroup.getBondFeedGroupID()),
						ICMSConstant.INSTANCE_BOND_FEED_GROUP);

				vv = new OBBondFeedGroupTrxValue(trxValue);
				vv.setBondFeedGroup(feedGroup);
				...
				if (vv.getReferenceID() != null) {
					...
					IBondFeedGroup feedGroup = getBondFeedBusManager().getBondFeedGroup(
			...
			if (vv.getStagingReferenceID() != null) {
				...
				IBondFeedGroup stagingFeedGroup = getStagingBondFeedBusManager().getBondFeedGroup(
						Long.parseLong(trxValue.getStagingReferenceID()));
				vv.setStagingBondFeedGroup(stagingFeedGroup);
