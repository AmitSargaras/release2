...
public class CheckerApproveInsertMasterOperation extends AbstractStockTrxOperation {
     ...
     */
    private IStockFeedGroupTrxValue actualMaster(IStockFeedGroupTrxValue idxTrxValue) throws CreditApprovalException, TrxParameterException, TransactionException, ConcurrentUpdateException {
        try {
            IFileMapperId staging = idxTrxValue.getStagingFileMapperID();
            IFileMapperId actual = idxTrxValue.getFileMapperID();

            IFileMapperId updatedActual = getStockFeedFileMapperIdBusManager().insertStockFeedEntry(staging,idxTrxValue);           
