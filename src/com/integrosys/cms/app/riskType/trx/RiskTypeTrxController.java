...
public class RiskTypeTrxController extends CMSTrxController{
    ...
    public String getInstanceName() {
        return ICMSConstant.INSTANCE_RISK_TYPE;
    }
    ...
    private ITrxOperation factoryOperation(ITrxValue value, ITrxParameter param) throws TrxParameterException {
        ...
        if(toState!=null){
        	 if(toState.equals(ICMSConstant.STATE_DRAFT)){
        		 if (action.equals(ICMSConstant.ACTION_MAKER_CREATE_RISK_TYPE)) {
        			 return (ITrxOperation) getNameTrxOperationMap().get("MakerUpdateDraftCreateRiskTypeOperation");
        		 }
        		 if (action.equals(ICMSConstant.ACTION_MAKER_UPDATE_RISK_TYPE)) {
             		return (ITrxOperation) getNameTrxOperationMap().get("MakerUpdateRiskTypeOperation");
             	}
        	}
        }
           if (action.equals(ICMSConstant.ACTION_MAKER_SAVE_RISK_TYPE)) {
                return (ITrxOperation) getNameTrxOperationMap().get("MakerSaveRiskTypeOperation");
            }
            if (action.equals(ICMSConstant.ACTION_MAKER_SAVE_UPDATE_RISK_TYPE)) {
                return (ITrxOperation) getNameTrxOperationMap().get("MakerSaveUpdateRiskTypeOperation");
            }
        if ((toState == null) || (toState.equals(ICMSConstant.STATE_ND))) {
            if (action.equals(ICMSConstant.ACTION_MAKER_CREATE_RISK_TYPE)) {
                return (ITrxOperation) getNameTrxOperationMap().get("MakerCreateRiskTypeOperation");
            }
            ...
            if (action.equals(ICMSConstant.ACTION_CHECKER_APPROVE_RISK_TYPE)) {
                return (ITrxOperation) getNameTrxOperationMap().get("CheckerApproveCreateRiskTypeOperation");
            }

            if (action.equals(ICMSConstant.ACTION_CHECKER_REJECT_RISK_TYPE)) {
                return (ITrxOperation) getNameTrxOperationMap().get("CheckerRejectRiskTypeOperation");
            }
            throw new TrxParameterException("Unknown Action: " + action + " with toState: " + toState);
        } 
        	...
        	if (action.equals(ICMSConstant.ACTION_MAKER_UPDATE_RISK_TYPE)) {
        		return (ITrxOperation) getNameTrxOperationMap().get("MakerUpdateRiskTypeOperation");
        	}else if(action.equals(ICMSConstant.ACTION_MAKER_DELETE_RISK_TYPE)) {
        		return (ITrxOperation) getNameTrxOperationMap().get("MakerDeleteRiskTypeOperation");
        	}
        /*	else if (action.equals(ICMSConstant.ACTION_MAKER_DELETE_EXCLUDED_FACILITY)) {
            ...
            if (action.equals(ICMSConstant.ACTION_CHECKER_APPROVE_RISK_TYPE)) {
                return (ITrxOperation) getNameTrxOperationMap().get("CheckerApproveUpdateRiskTypeOperation");
            } else if (action.equals(ICMSConstant.ACTION_CHECKER_REJECT_RISK_TYPE)) {
                return (ITrxOperation) getNameTrxOperationMap().get("CheckerRejectRiskTypeOperation");
            }
            throw new TrxParameterException("Unknown Action: " + action + " with toState: " + toState);
        } 
        else if (toState.equals(ICMSConstant.STATE_REJECTED)) {
            if (action.equals(ICMSConstant.ACTION_MAKER_EDIT_REJECTED_RISK_TYPE)) {
                if (fromState.equals(ICMSConstant.STATE_PENDING_CREATE)) {
                    return (ITrxOperation) getNameTrxOperationMap().get("MakerEditRejectedCreateRiskTypeOperation");
                } else if (fromState.equals(ICMSConstant.STATE_PENDING_UPDATE)) {
                    return (ITrxOperation) getNameTrxOperationMap().get("MakerEditRejectedUpdateRiskTypeOperation");
                }else if (fromState.equals(ICMSConstant.STATE_PENDING_DELETE)) {
                    return (ITrxOperation) getNameTrxOperationMap().get("MakerEditRejectedDeleteRiskTypeOperation");
                }
            } else if (action.equals(ICMSConstant.ACTION_MAKER_CLOSE_REJECTED_RISK_TYPE)) {
                if (fromState.equals(ICMSConstant.STATE_PENDING_CREATE)) {
                    return (ITrxOperation) getNameTrxOperationMap().get("MakerCloseRejectedCreateRiskTypeOperation");
                } else if (fromState.equals(ICMSConstant.STATE_PENDING_UPDATE)) {
                    return (ITrxOperation) getNameTrxOperationMap().get("MakerCloseRejectedUpdateRiskTypeOperation");
                } else if (fromState.equals(ICMSConstant.STATE_PENDING_DELETE)) {
                    return (ITrxOperation) getNameTrxOperationMap().get("MakerCloseRejectedDeleteRiskTypeOperation");
                }
            }
        }     
        else if (toState.equals(ICMSConstant.STATE_PENDING_DELETE)) {
            if (action.equals(ICMSConstant.ACTION_CHECKER_APPROVE_RISK_TYPE)) {
                return (ITrxOperation) getNameTrxOperationMap().get("CheckerApproveDeleteRiskTypeOperation");
            } else if (action.equals(ICMSConstant.ACTION_CHECKER_REJECT_RISK_TYPE)) {
                return (ITrxOperation) getNameTrxOperationMap().get("CheckerRejectRiskTypeOperation");
            } 
        }
        ...
        else if (action.equals(ICMSConstant.ACTION_MAKER_CLOSE_DRAFT_RISK_TYPE)) {
        	return (ITrxOperation) getNameTrxOperationMap().get("MakerCloseDraftRiskTypeOperation");
        }
