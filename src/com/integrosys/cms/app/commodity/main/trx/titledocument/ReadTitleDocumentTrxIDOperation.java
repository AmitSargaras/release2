 ...
 * $Header: /home/cms2/cvsroot/cms2/src/com/integrosys/cms/app/commodity/main/trx/titledocument/ReadTitleDocumentTrxIDOperation.java,v 1.3 2004/08/17 06:52:31 wltan Exp $
 */
 ...
 * @since $Date: 2004/08/17 06:52:31 $ Tag: $Name: $
 */
public class ReadTitleDocumentTrxIDOperation extends CMSTrxOperation implements ITrxReadOperation {
	/**
	 * Default Constructor
	 */
	public ReadTitleDocumentTrxIDOperation() {
		super();
	}
	...
	public ITrxValue getTransaction(ITrxValue val) throws TransactionException {
		try {
			...
			OBTitleDocumentTrxValue newValue = new OBTitleDocumentTrxValue(trxValue);

			String stagingRef = trxValue.getStagingReferenceID();
			String actualRef = trxValue.getReferenceID();

			ITitleDocument[] actualTitleDocument = null;
			ITitleDocument[] stagingTitleDocument = null;
			DefaultLogger.debug(this, "Actual Reference: " + actualRef + " , Staging Reference: " + stagingRef);

			if (null != actualRef) {
				actualTitleDocument = (ITitleDocument[]) getCommodityMainInfoManager().getCommodityMainInfosByGroupID(
						(actualRef), ICommodityMainInfo.INFO_TYPE_TITLEDOC);
			}
			if (null != stagingRef) {
				...
				ICommodityMainInfo[] commInfo = getStagingCommodityMainInfoManager().getCommodityMainInfosByGroupID(
						(stagingRef), ICommodityMainInfo.INFO_TYPE_TITLEDOC);
				stagingTitleDocument = (ITitleDocument[]) commInfo;
				DefaultLogger.debug(this, "$$$ReadOp 2 : after retrieve "
						+ (stagingTitleDocument != null ? " Length is : " + stagingTitleDocument.length + ""
								: " IS NULL "));
			}

			newValue.setStagingTitleDocument(stagingTitleDocument);
			newValue.setTitleDocument(actualTitleDocument);
			DefaultLogger.debug(this, "$$$ReadOp 3 : newValue.getStagingTitleDocument()"
					+ (newValue.getStagingTitleDocument() != null ? " Length is : "
							+ newValue.getStagingTitleDocument().length + "" : " IS NULL "));
			return newValue;
		}
		catch (Exception ex) {
			ex.printStackTrace();
