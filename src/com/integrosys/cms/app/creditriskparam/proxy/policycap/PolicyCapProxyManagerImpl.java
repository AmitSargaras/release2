 ...
 */
public class PolicyCapProxyManagerImpl implements IPolicyCapProxyManager {
	...
	public IPolicyCapTrxValue makerUpdatePolicyCap(ITrxContext ctx, IPolicyCapTrxValue trxValue,
		...
		try {
			return getPolicyCapProxyManager().makerUpdatePolicyCap(ctx, trxValue, policyCapList);

		}
		catch (RemoteException ex) {
			throw new PolicyCapException("Exception in makerUpdatePolicyCap: " + ex.toString());
		}
	}

	/**
	 * Maker updates policy cap group for an exchange + bank entity
	 * @param ctx of ITrxContext type
	...
	public IPolicyCapGroupTrxValue makerUpdatePolicyCapGroup(ITrxContext ctx, IPolicyCapGroupTrxValue trxValue,
		...
		try {
			return getPolicyCapProxyManager().makerUpdatePolicyCapGroup(ctx, trxValue, policyCapGroup);

		}
		catch (RemoteException ex) {
			throw new PolicyCapException("Exception in makerUpdatePolicyCapGroup: " + ex.toString());
		}
	}
	...
	public IPolicyCapTrxValue checkerApproveUpdatePolicyCap(ITrxContext ctx, IPolicyCapTrxValue trxValue)
		...
		try {
			return getPolicyCapProxyManager().checkerApproveUpdatePolicyCap(ctx, trxValue);

		}
		catch (RemoteException ex) {
			throw new PolicyCapException("Exception in checkerApproveUpdatePolicyCap: " + ex.toString());
		}
	}
	...
	public IPolicyCapGroupTrxValue checkerApproveUpdatePolicyCapGroup(ITrxContext ctx, IPolicyCapGroupTrxValue trxValue)
			throws PolicyCapException {
		try {
			return getPolicyCapProxyManager().checkerApproveUpdatePolicyCapGroup(ctx, trxValue);

		}
		catch (RemoteException ex) {
			throw new PolicyCapException("Exception in checkerApproveUpdatePolicyCapGroup: " + ex.toString());
		}
	}
	...
	public IPolicyCapTrxValue checkerRejectUpdatePolicyCap(ITrxContext ctx, IPolicyCapTrxValue trxValue)
			throws PolicyCapException {
		try {
			return getPolicyCapProxyManager().checkerRejectUpdatePolicyCap(ctx, trxValue);

		}
		catch (RemoteException ex) {
			throw new PolicyCapException("Exception in checkerRejectUpdatePolicyCap: " + ex.toString());
		}
	}
	...
	public IPolicyCapGroupTrxValue checkerRejectUpdatePolicyCapGroup(ITrxContext ctx, IPolicyCapGroupTrxValue trxValue)
			throws PolicyCapException {
		try {
			return getPolicyCapProxyManager().checkerRejectUpdatePolicyCapGroup(ctx, trxValue);
	...
	public IPolicyCapTrxValue makerClosePolicyCap(ITrxContext ctx, IPolicyCapTrxValue trxValue)
			throws PolicyCapException {
		try {
			return getPolicyCapProxyManager().makerClosePolicyCap(ctx, trxValue);

		}
		catch (RemoteException ex) {
			throw new PolicyCapException("Exception in makerClosePolicyCap: " + ex.toString());
		}
	}
	 ...
	 * @param ctx - Transaction Context to use
	 * @param trxValue - Policy Cap Group to be closed
	...
	public IPolicyCapGroupTrxValue makerClosePolicyCapGroup(ITrxContext ctx, IPolicyCapGroupTrxValue trxValue)
			throws PolicyCapException {
		try {
			return getPolicyCapProxyManager().makerClosePolicyCapGroup(ctx, trxValue);
	 ...
	 * @param groupID - Transaction Context to use
	 * @return IPolicyCapTrxValue
	 * @throws PolicyCapException on errors
	 */
	public IPolicyCapTrxValue getPolicyCapTrxValue(long groupID) throws PolicyCapException {
		try {
			return getPolicyCapProxyManager().getPolicyCapTrxValue(groupID);

		}
		catch (RemoteException ex) {
			throw new PolicyCapException("Exception in getPolicyCapByExchange: " + ex.toString());
		}
	}

	/**
	 * Get the Policy Cap Group
	 * @param policyCapGroupID - Transaction Context to use
	 * @return IPolicyCapGroupTrxValue
	 * @throws PolicyCapException on errors
	 */
	public IPolicyCapGroupTrxValue getPolicyCapGroupTrxValue(long policyGroupID) throws PolicyCapException {
		try {
			return getPolicyCapProxyManager().getPolicyCapGroupTrxValue(policyGroupID);
	...
	public IPolicyCapTrxValue getPolicyCapTrxValueByTrxID(String trxID) throws PolicyCapException {
		try {
			return getPolicyCapProxyManager().getPolicyCapTrxValueByTrxID(trxID);

		}
		catch (RemoteException ex) {
			throw new PolicyCapException("Exception in getPolicyCapTrxValueByTrxID: " + ex.toString());
		}
	}
	...
	public IPolicyCapGroupTrxValue getPolicyCapGroupTrxValueByTrxID(String trxID) throws PolicyCapException {
		try {
			return getPolicyCapProxyManager().getPolicyCapGroupTrxValueByTrxID(trxID);
	...
	public IPolicyCap[] getPolicyCapByExchange(String exchangeCode) throws PolicyCapException {
		try {
			return getPolicyCapProxyManager().getPolicyCapByExchange(exchangeCode);
	 ...
	 * @return IPolicyCapGroup - the Policy Cap Group for the given exchange and
	 *         bank
	 * @throws PolicyCapException on errors
	 */
	public IPolicyCapGroup getPolicyCapGroupByExchangeBank(String exchangeCode, String bankEntity)
			throws PolicyCapException {
		try {
			return getPolicyCapProxyManager().getPolicyCapGroupByExchangeBank(exchangeCode, bankEntity);
	 ...
	 * 
	 * @return SBPolicyCapProxyManager
	 */
	private SBPolicyCapProxyManager getPolicyCapProxyManager() {
		SBPolicyCapProxyManager proxy = (SBPolicyCapProxyManager) BeanController.getEJB(
				ICMSJNDIConstant.SB_POLICY_CAP_PROXY_JNDI, SBPolicyCapProxyManagerHome.class.getName());
		return proxy;
	}

	protected void rollback() throws PolicyCapException {
		// do nothing
	}
