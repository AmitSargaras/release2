 ...
 * $Header: /home/cms2/cvsroot/cms2/src/com/integrosys/cms/app/collaborationtask/trx/ReadCCTaskOperation.java,v 1.2 2005/10/14 08:14:59 hshii Exp $
 */
...
/**
 * This operation is responsible for the creation of a CC task transaction
 * 
 * @author $Author: hshii $
 * @version $Revision: 1.2 $
 * @since $Date: 2005/10/14 08:14:59 $ Tag: $Name: $
 */
public class ReadCCTaskOperation extends CMSTrxOperation implements ITrxReadOperation {
	/**
	 * Default Constructor
	 */
	public ReadCCTaskOperation() {
		super();
	}
	...
	public String getOperationName() {
		return ICMSConstant.ACTION_READ_CC_TASK;
	}
	...
	public ITrxValue getTransaction(ITrxValue anITrxValue) throws TransactionException {
		try {
			...
			OBCCTaskTrxValue newValue = new OBCCTaskTrxValue(trxValue);
			...
			if (stagingRef != null) {
				...
				ICCTask stagingCCTask = getSBStagingCollaborationTaskBusManager().getCCTask(stagingPK);
				newValue.setStagingCCTask(stagingCCTask);
			}

			if (actualRef != null) {
				long actualPK = (new Long(actualRef)).longValue();
				ICCTask actualCCTask = getSBCollaborationTaskBusManager().getCCTask(actualPK);
				newValue.setCCTask(actualCCTask);
			}

			OBCMSTrxHistoryLog[] logs = (OBCMSTrxHistoryLog[]) ((CMSTransactionDAO) CMSTransactionDAOFactory.getDAO())
					.getTransactionLogs(trxValue.getTransactionID(), ICMSConstant.COL_TASK_MAX_HISTORY).toArray(
