...
/**
 * Process data file like excel, csv, fixed length and dump it to an array.
 * 
 * @author $Author: kltan, pctan $<br>
 * @version $Revision:$
 * @since $Date: $ Tag: $Name: $
 * 
 *        how to run: New this class by passing in the file name of the property
 *        file. By default the property file will reside at
 *        com\mbb\cms\app\common\properties\ Execute doProcessData method to get
 *        the processed data in ArrayList. eg. ProcessDataFile dataFile = new
 *        ProcessDataFile( PROP_FILE_NAME );
 * 
...
public class ProcessDataFile {
	...
	public ProcessDataFile(String propertiesFileName) {
	...
	public ProcessDataFile(String propertiesFileName,String batchJobName) {
...
//				delimiter="|";
				...
				csv.readFileCSV(fileUpload,this,dataFile,master);
				isValid = csv.isValidationStatus();
				errorList = csv.getErrorHm();
				maxCount = csv.getMaxRowCount();
				isCheckSumFooter = csv.isCheckSumFooter();
				if(master.equals(ICMSConstant.USER_UPLOAD)){
					userMasterValidationStatus = csv.isUserMasterValidationStatus();
					secondaryDelimiter = csv.getSecondaryDelimiter();
				}
			}
			catch (Exception e) {
				DefaultLogger.error(this, "", e);
	...
	public ArrayList doProcessData() {
		...
		else if (getFileFormat().equals(CSV_FILE_FORMAT)) {
			CSVReader csv = new CSVReader();
			csv.readCSV(this);
		}
		else if (getFileFormat().equals(FL_FILE_FORMAT)) {
			FixLengthReader fixLength = new FixLengthReader();
			fixLength.readFixLength(this);
		}
	...
	public ArrayList processFileUpload(File fileUpload,String master) {
		System.out.println("DeferralExtensionJob....Inside processFileUpload ");
		try {
			String templatePath = (new BatchResourceFactory()).getTemplateXmlFileName(master);
			System.out.println("DeferralExtensionJob....templatePath.. "+templatePath);
			/*InputStream is = getClass().getResourceAsStream(templatePath);
			InputStreamReader read = new InputStreamReader(is);*/
			FileInputStream fis = new FileInputStream(templatePath);
			InputStreamReader read = new InputStreamReader(fis);
...
//			dataFileFormat = "csv";
			...
			rowHeaderIndex = 0;
			columnStartIndex = 0;
			totalColumn = dataFile.getHeader().getColumn().length;
...
//			delimiter="|";
			delimiter = dataFile.getDelimiter();
			System.out.println("DeferralExtensionJob....dataFileFormat.. "+dataFileFormat);
			if(!ICMSConstant.EXCHANGE_RATE_UPLOAD.equalsIgnoreCase(master))
			...
			//MAX_RECORD_PROCESS
			//maxRecProcess=50; 
			System.out.println("DeferralExtensionJob....before if => master.. "+master);
			if("UbsUpload".equalsIgnoreCase(master) || "FinwareUpload".equalsIgnoreCase(master) 
					|| "BaselUpdateUpload".equalsIgnoreCase(master) || "FdUpload".equalsIgnoreCase(master)
					|| "DeferralExtensionUpload".equalsIgnoreCase(master)
					|| ICMSConstant.EXCHANGE_RATE_UPLOAD.equalsIgnoreCase(master))
			{
				CSVReader csv = new CSVReader();
				if(ICMSConstant.EXCHANGE_RATE_UPLOAD.equalsIgnoreCase(master))
				{
					System.out.println("Inside if condition EXCHANGE_RATE_UPLOAD .. ");
					csv.readCSVFile(fileUpload,this,dataFile,master);
					isValid = csv.isValidationStatus();
					errorList = csv.getErrorHm();
					maxCount = csv.getMaxRowCount();
					isCheckSumFooter = csv.isCheckSumFooter();
				...
				{
					System.out.println("DeferralExtensionJob....Before going Inside else for csv.readFileCSV.. ");
					csv.readFileCSV(fileUpload,this,dataFile,master);
					System.out.println("DeferralExtensionJob....After  else for csv.readFileCSV.. ");
				}
			}
			else{
				System.out.println("DeferralExtensionJob....If Condition skip =>master=>"+master);
				PoiExcel excel = new PoiExcel();
				if(fileUpload.getName().endsWith(".xls") || fileUpload.getName().endsWith(".XLS")){
					excel.readExcel(fileUpload,this);
				}else if(fileUpload.getName().endsWith(".xlsx") || fileUpload.getName().endsWith(".XLSX")){
					excel.readXLSXFile(fileUpload,this);
				}
			}
			/*isValid = excel.isValidationStatus();
			errorList = csv.getErrorHm();
			maxCount = csv.getMaxRowCount();
			isCheckSumFooter = csv.isCheckSumFooter();
			if(master.equals(ICMSConstant.USER_UPLOAD)){
			...
			DefaultLogger.error(this, "Exception caught in DeferralExtensionJob....Inside processFileUpload ", e);
			System.out.println("Exception caught in DeferralExtensionJob....Inside processFileUpload "+ e);
			e.printStackTrace();
			throw new ProcessFileException(e.getMessage());
