...
public class FixedDepositFileUploadJob implements FixedDepositFileUploadConstant{

private final static Logger logger = LoggerFactory.getLogger(FixedDepositFileUploadJob.class);
	
	public static void main(String[] args) {
		new FixedDepositFileUploadJob().execute();
	}
	...
	public void execute() {
		...
		String headers=null;
		if(StringUtils.isNotBlank(serverName) && serverName.equalsIgnoreCase(SERVER_NAME)) {
			
			Date today = DateUtil.clearTime(new Date());
			System.out.println("File upload for MTM valuation process for FD has been started at " 
					  + Calendar.getInstance().getTime());
			logger.info("File upload for MTM valuation process for FD has been started at " 
					  + Calendar.getInstance().getTime());
			
			try {
				ICollateralDAO dao = CollateralDAOFactory.getDAO();
				//if else for sql
				List<HashMap<String,String>> uploadList = dao.getDetailsForFdDepositFileUploadJob();
				
				if(uploadList == null || uploadList.size() == 0) { 
					System.out.println("File upload for MTM valuation process for FD has no data"
							+ " to upload at SFTP location");
					logger.warn("File upload for MTM valuation process for FD has no data"
							+ " to upload at SFTP location");
				} else {
					//if for new Code
					if(invrstmentAccNoFlag.equalsIgnoreCase("true")) {
						headers = join(FixedDepositFileUploadNewHeader.getHeaderNames(), SEPARATOR, true);
					}//for old flag
					else
					{
						headers = join(FixedDepositFileUploadHeader.getHeaderNames(), SEPARATOR, true);	
					}
					...
					processUpload.logData(ICMSConstant.FD_STP_FILE_UPLOAD, String.format("%s", resbundle.getString(FTP_UPLOAD_REMOTE_DIR)),
							String.format("%s_%s.%s",FILE_NAME_PREFIX, todaysDate, FILE_NAME_POSTFIX), uploadList.size());
					System.out.println("File upload for MTM valuation process for FD is completed at " 
							 + Calendar.getInstance().getTime());
					logger.info("File upload for MTM valuation process for FD is completed at " 
								 + Calendar.getInstance().getTime());
				...
				System.out.println("File upload for MTM valuation process for FD stopped abrutly at " 
						+ Calendar.getInstance().getTime() + " because of " + e.getMessage());
				logger.error("File upload for MTM valuation process for FD stopped abrutly at " 
										+ Calendar.getInstance().getTime() + " because of " + e.getMessage());
				e.printStackTrace();
			}
			catch (Exception e) {
	...
	private void uploadFileToSFTP(String localFilePath, String remoteDataDir) throws Exception {
		...
		CMSFtpClient sftpClient = CMSFtpClient.getInstance("ofa",ICMSConstant.FD_STP_FILE_UPLOAD);
		
		sftpClient.openConnection();
		System.out.println("SFTP connection was opened for file transfer");
		...
		System.out.println("SFTP connection successfully closed after file transfer");
	...
	private List<String> enumCheck(String invrstmentAccNoFlag, HashMap<String, String> item)
	{
		...
		{
			FixedDepositFileUploadHeader[] headerList = FixedDepositFileUploadHeader.values();
		for(FixedDepositFileUploadHeader header: headerList) {
		...
		{  FixedDepositFileUploadNewHeader[] headerList = FixedDepositFileUploadNewHeader.values();
			for(FixedDepositFileUploadNewHeader header: headerList) {
				String itemValue = item.containsKey(header.name()) ? item.get(header.name()) : "";
				itemValue = AbstractCommonMapper.isEmptyOrNull(itemValue) ? "" : itemValue.trim();
				line.add(itemValue);	
		}
