...
public class DiscrepencyDeferralFacilityJob implements IBorrower {

	private final static Logger logger = LoggerFactory.getLogger(DiscrepencyDeferralFacilityJob.class);

	public static void main(String[] args) {

		new DiscrepencyDeferralFacilityJob().execute();
	}

	public DiscrepencyDeferralFacilityJob() {
	}
	...
	public void execute() {
		...
		if (null != dfsoChanges && "Y".equalsIgnoreCase(dfsoChanges) 
			...
			try {
				System.out.println("Starting DiscrepencyFacilityJob");
			...
			//	IGeneralParamDao dao1 = (IGeneralParamDao) BeanHouse.get("generalParamDao");
				...
				String fileName = DFSO_DiscrepencyDeferralFacility_FILENAME;
				Date date = new Date();
				...
				System.out.println("dfso DiscrepencyFacility......" + serverFilePath + fileName);
				...
				header[14]="DOC_REMARKS";
				header[15]="FACILITYCATEGORY";
				header[16]="SANCTIONEDAMOUNT";
				header[17]="RELEASABLEAMOUNT";
				
				header[18]="RELEASEDAMOUNT";
				header[19]="UTILIZEDAMOUNT";
				header[20]="FACILITY_NAME";
				header[21]="FACILITY_ID";
			
				out.writeNext(header);
			
				List DiscrepencyFacilityList = dao.getDiscrepencyDeferralFacility();
				if (DiscrepencyFacilityList != null) {
					for (int i = 0; i < DiscrepencyFacilityList.size(); i++) {
						String[] arr = (String[]) (DiscrepencyFacilityList.get(i));
				...
				DefaultLogger.debug(this, "DiscrepencyFacility in catch IOException......" + e.getMessage());
				e.printStackTrace();
			} catch (Exception e) {
				DefaultLogger.debug(this, "DiscrepencyFacility in catch Exception......" + e.getMessage());
				e.printStackTrace();
			}
		}
	}
	...
	public static String toDateFormat(Date date, String formatter) throws Exception {
		try {
			...
			DefaultLogger.debug("", "DiscrepencyFacility toDateFormat" + e.getMessage());
			e.printStackTrace();
			throw e;
		}
	}
	...
	private void deleteOldFiles() {
		System.out.println("<<<<In execute() of DiscrepencyFacility deleteOldFiles()");
		try {
			ResourceBundle bundle = ResourceBundle.getBundle("ofa");
	...
	public void moveFile(File sourceFile, String fileName) {
		System.out.println("<<<<In execute() of DiscrepencyFacility moveFile()");
		...
		try {
			...
			try {
				...
				if (delete == false) {
					System.out.println("DiscrepencyFacility file  deletion failed for file:" + sourceFile.getPath());
				}
			} catch (Exception e) {
				e.printStackTrace();
