...
public class StockStatementDPNoJob implements IBorrower {

	private final static Logger logger = LoggerFactory.getLogger(StockStatementDPNoJob.class);

	public static void main(String[] args) {

		new StockStatementDPNoJob().execute();
	}

	public StockStatementDPNoJob() {
	}
	...
	public void execute() {
		...
		if (null != dfsoChanges && "Y".equalsIgnoreCase(dfsoChanges) 
			...
			try {
				System.out.println("Starting StockStatementDPNoJob");
			...
			//	IGeneralParamDao dao1 = (IGeneralParamDao) BeanHouse.get("generalParamDao");
				...
				String fileName = DFSO_StockStatementDPNo_FILENAME;
				Date date = new Date();
				...
				List StockStatementDPNo = dao.getStockStatementDPNo();
				if (StockStatementDPNo != null) {
					for (int i = 0; i < StockStatementDPNo.size(); i++) {
						String[] arr = (String[]) (StockStatementDPNo.get(i));
