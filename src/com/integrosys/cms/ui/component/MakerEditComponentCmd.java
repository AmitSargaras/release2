...
public class MakerEditComponentCmd extends AbstractCommand implements	ICommonEventConstant {
	    ...
	    public HashMap doExecute(HashMap map) throws CommandProcessingException, CommandValidationException {
			...
			try {
				...
				if(trxValueIn.getFromState().equals("PENDING_PERFECTION")){
					trxValueOut = getComponentProxy().makerUpdateSaveCreateComponent(ctx, trxValueIn, component);
				}else{
					if(event.equals("maker_edit_component")){
						...
						if(component.getAge()==null||component.getAge().trim().equals("")){	
							execute=true;
							exceptionMap.put("age", new ActionMessage("error.string.mandatory"));
							resultMap.put("request.ITrxValue", trxValueIn);
							returnMap.put(ICommonEventConstant.COMMAND_EXCEPTION_MAP, exceptionMap);
							returnMap.put(ICommonEventConstant.COMMAND_RESULT_MAP, resultMap);
						}}
					...
					if(!execute){
							trxValueOut = getComponentProxy().makerUpdateComponent(ctx, trxValueIn, component);
						}
					}
					if (event.equals("maker_delete_component")||event.equals("maker_save_update")) {
						trxValueOut = getComponentProxy().makerUpdateComponent(ctx, trxValueIn, component);
					} /*else {
