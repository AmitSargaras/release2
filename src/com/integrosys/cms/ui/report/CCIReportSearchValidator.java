...
/**
 * Description:  Validator class for CCI Reports
 *              Checks that required values are entered by the user
 *
 * @author $Author:  $<br>
 * @version $Revision: $
 * @since $Date: $
 * Tag: $Name:  $
 */
public class CCIReportSearchValidator {

    public static ActionErrors validateInput(CommonForm aForm, Locale locale) {

        DefaultLogger.info(CCIReportSearchValidator.class.getName(),
                "Entering method validateInput");

        CCIReportSearchForm form = (CCIReportSearchForm) aForm;
        ...
        if (event.equals(CCIReportSearchAction.EVENT_SEARCH_REPORT)) {
    ...
    /**
     * Checks that entered date is not null or empty
     * Further checks that date entered is not past current date
     * nor before six months period
     * Reports past current date have not been generated
     * Reports that are older than six months are not retrievable online
     *
     * @param dateEntered
     * @param labelKey
     * @param errors
...
//          Date prevDay = rightNow.getTime();
            ...
            if (convertedDate.after(today) || convertedDate.before(markerDate)) {
                errors.add("searchDate",
                        new ActionMessage("error.string.reports.validDateRange",
                                DateUtil.convertToDisplayDate(earliestSearchDate),
                                DateUtil.convertToDisplayDate(today)));
            }
        }
    ...
    private static boolean validateString(String s, String labelKey, ActionErrors errors, String errorKey) {
        DefaultLogger.info(CCIReportSearchValidator.class.getName(),
                "Entering method validateString");
    ...
    private static int getSearchReportDuration() {
        ...
        DefaultLogger.debug(CCIReportSearchValidator.class.getName(),
                "number of days" + days);
        return days;
    }
}
