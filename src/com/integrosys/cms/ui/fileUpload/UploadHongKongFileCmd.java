...
public class UploadHongKongFileCmd extends AbstractCommand implements ICommonEventConstant{
...
public String[][] getParameterDescriptor() {
   ...
   public HashMap doExecute(HashMap map) throws CommandProcessingException, CommandValidationException {
		...
		DefaultLogger.debug(this, "#####################In UploadHongKongFileCmd ##### size of dataFromCacheView map is : "+dataFromCacheView.size());
		DefaultLogger.debug(this, "#####################In UploadHongKongFileCmd ##### size of dataFromUpdLineFacilityMV map is : "+dataFromUpdLineFacilityMV.size());
		...
		trxValueOut.setTransactionSubType("HONGKONG_UPLOAD");
		...
		try{
			...
			if(fileName.endsWith(".xls")||fileName.endsWith(".XLS") || fileName.endsWith(".xlsx") || fileName.endsWith(".XLSX")){  
				...
				if(resultList.size()>0){
					mp = (HashMap)jdbc.insertHongKongfile(resultList, this,fileName,user,applicationDate,dataFromCacheView,dataFromUpdLineFacilityMV);
					totalUploadedList = (ArrayList) mp.get("totalUploadedList");
					
					//jdbc.updateXrefStageAmount(totalUploadedList,"HONGKONG");
					
					errorList = (ArrayList) mp.get("errorList");
					if(totalUploadedList.size()>0){
						IFileUpload fileObj=new OBFileUpload();
						fileObj.setFileType("HONGKONG_UPLOAD");
						...
						for(int i=0;i<totalUploadedList.size();i++){
							...
							DefaultLogger.debug(this, "##### UploadHongKongFileCmd.java ########## Line No 169 #######:: "+hongkongRecord.getStatus());
						...
						if(trxValueOut!=null){
							...
							for (int j = 0; j < consolidateList.size(); j += batchSize) {
								...
								jdbc.createEntireHongkongStageFile(batchList);
							}
						}
					}
					...
					//jdbc.spUpdateReleasedAmount("HONGKONG_UPLOAD","STAGE");
				}
			}
			else{
				fileType="NOT_CSV";
