...
public class UploadReleaselinedetailsFileCmd extends AbstractCommand implements ICommonEventConstant{
	...
	public UploadReleaselinedetailsFileCmd(){		
	}
	...
	public HashMap doExecute(HashMap map) throws CommandProcessingException, CommandValidationException {
		...
		ArrayList<OBReleaselinedetailsFile> errorList=new ArrayList();
		...
		try{
			...
			DefaultLogger.debug(this, "#####################In UploadReleaselinedetailsFileCommand ##### line no 106#######:: "+fileName);
			...
			if(fileName.endsWith(".XLS") || fileName.endsWith(".xls") || fileName.endsWith(".xlsx") || fileName.endsWith(".XLSX")){  
				...
				if(resultList.size()>0){
					...
					mp = (HashMap)jdbc.insertReleaselinedetailsfile(resultList, this,fileName,user,applicationDate,dataFromCacheView,dataFromUpdLineFacilityMV);
					...
					if(totalUploadedList.size()>0){
						...
						for(int i=0;i<totalUploadedList.size();i++){
							OBReleaselinedetailsFile ubsRecord=(OBReleaselinedetailsFile)totalUploadedList.get(i);
						...
						if(trxValueOut!=null){
							...
							for(int i=0;i<errorList.size();i++){
								OBReleaselinedetailsFile ubsRecord=(OBReleaselinedetailsFile)errorList.get(i);
								ubsRecord.setFileId(Long.parseLong(trxValueOut.getStagingReferenceID()));
								consolidateList.add(ubsRecord);
							}
							...
							for (int j = 0; j < consolidateList.size(); j += batchSize) {
								List<OBReleaselinedetailsFile> batchList = consolidateList.subList(j, j + batchSize > consolidateList.size() ? consolidateList.size() : j + batchSize);
