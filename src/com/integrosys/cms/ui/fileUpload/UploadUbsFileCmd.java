...
public class UploadUbsFileCmd extends AbstractCommand implements ICommonEventConstant{
	...
	public HashMap doExecute(HashMap map) throws CommandProcessingException, CommandValidationException {
		...
		trxValueOut.setTransactionSubType("UBS_UPLOAD");
		...
		try{
			...
			DefaultLogger.debug(this, "#####################In UploadUbsFileCommand ##### line no 106#######:: "+fileName);
			...
			if(fileName.endsWith(".CSV") || fileName.endsWith(".csv")){  
				...
				if(resultList.size()>0){
					...
					mp = (HashMap)jdbc.insertUbsfile(resultList, this,fileName,user,applicationDate,dataFromCacheView,dataFromUpdLineFacilityMV);
					...
					if(totalUploadedList.size()>0){
						...
						if(trxValueOut!=null){
							...
							for (int j = 0; j < consolidateList.size(); j += batchSize) {
								...
								jdbc.createEntireUbsStageFile(batchList);
							}
