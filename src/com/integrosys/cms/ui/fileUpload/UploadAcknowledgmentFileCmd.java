...
public class UploadAcknowledgmentFileCmd extends AbstractCommand implements ICommonEventConstant{
	...
	public UploadAcknowledgmentFileCmd(){		
	}
	...
	public HashMap doExecute(HashMap map) throws CommandProcessingException, CommandValidationException {
		...
		ArrayList<OBAcknowledgmentFile> errorList=new ArrayList();
		...
		trxValueOut.setTransactionSubType("ACK_UPLOAD");
		...
		try{
			...
			DefaultLogger.debug(this, "#####################In UploadAcknowledgmentFileCommand ##### line no 106#######:: "+fileName);
			...
			if(fileName.endsWith(".XLS") || fileName.endsWith(".xls") || fileName.endsWith(".xlsx") || fileName.endsWith(".XLSX")){  
				...
				if(resultList.size()>0){
					...
					mp = (HashMap)jdbc.insertAcknowledgmentfile(resultList, this,fileName,user,applicationDate,dataFromCacheView,dataFromUpdLineFacilityMV);
					...
					if(totalUploadedList.size()>0){
						...
						for(int i=0;i<totalUploadedList.size();i++){
							OBAcknowledgmentFile ubsRecord=(OBAcknowledgmentFile)totalUploadedList.get(i);
						...
						if(trxValueOut!=null){
							...
							for(int i=0;i<errorList.size();i++){
								OBAcknowledgmentFile ubsRecord=(OBAcknowledgmentFile)errorList.get(i);
								ubsRecord.setFileId(Long.parseLong(trxValueOut.getStagingReferenceID()));
								consolidateList.add(ubsRecord);
							}
							...
							for (int j = 0; j < consolidateList.size(); j += batchSize) {
								List<OBAcknowledgmentFile> batchList = consolidateList.subList(j, j + batchSize > consolidateList.size() ? consolidateList.size() : j + batchSize);
