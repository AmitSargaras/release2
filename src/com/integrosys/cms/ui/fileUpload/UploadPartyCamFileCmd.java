...
public class UploadPartyCamFileCmd extends AbstractCommand implements ICommonEventConstant{
	...
	public HashMap doExecute(HashMap map) throws CommandProcessingException, CommandValidationException {
		...
		try{
			...
			DefaultLogger.debug(this, "#####################In UploadPartyCamFileCommand ##### line no 106#######:: "+fileName);
			...
			if(fileName.endsWith(".CSV") || fileName.endsWith(".csv")){  
				...
				if(resultList.size()>0){
					...
					mp = (HashMap)jdbc.insertPartyCamfile(resultList, this,fileName,user,applicationDate,dataFromCacheView,dataFromUpdLineFacilityMV);
					totalUploadedList = (ArrayList) mp.get("totalUploadedList");
					
					//jdbc.updateXrefStageAmount(totalUploadedList,"UBS-LIMITS");
					...
					if(totalUploadedList.size()>0){
						...
						for(int i=0;i<totalUploadedList.size();i++){
							OBPartyCamFile ubsRecord=(OBPartyCamFile)totalUploadedList.get(i);
						...
						if(trxValueOut!=null){
							...
							for(int i=0;i<errorList.size();i++){
								OBPartyCamFile ubsRecord=(OBPartyCamFile)errorList.get(i);
								ubsRecord.setFileId(Long.parseLong(trxValueOut.getStagingReferenceID()));
								consolidateList.add(ubsRecord);
							}
							...
							for (int j = 0; j < consolidateList.size(); j += batchSize) {
								List<OBPartyCamFile> batchList = consolidateList.subList(j, j + batchSize > consolidateList.size() ? consolidateList.size() : j + batchSize);
				...
				//jdbc.spUpdateReleasedAmount("UBS_UPLOAD","STAGE");
			}
			else{
				fileType="NOT_CSV";
			}
