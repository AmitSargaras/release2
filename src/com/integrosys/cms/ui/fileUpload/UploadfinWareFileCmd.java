...
public class UploadfinWareFileCmd extends AbstractCommand implements ICommonEventConstant{
	protected static final ArrayList finalList = new ArrayList();
...
public String[][] getParameterDescriptor() {
   ...
   public HashMap doExecute(HashMap map) throws CommandProcessingException, CommandValidationException {
		...
		DefaultLogger.debug(this, "#####################In UploadfinWareFileCmd ##### size of dataFromCacheView map is : "+dataFromCacheView.size());
		DefaultLogger.debug(this, "#####################In UploadfinWareFileCmd ##### size of dataFromUpdLineFacilityMV map is : "+dataFromUpdLineFacilityMV.size());
		...
		trxValueOut.setTransactionSubType("FINWARE_UPLOAD");
		...
		try{
			...
			// Added by : Dayananda for FC-UPLOAD changes || CSV format || Ends
				...
				if(resultList.size()>0){
					mp = (HashMap)jdbc.insertFinwareFile(resultList, this,fileName,user,applicationDate,dataFromCacheView,dataFromUpdLineFacilityMV);
					totalUploadedList = (ArrayList) mp.get("totalUploadedList");
					
					//jdbc.updateXrefFinwareStageAmount(totalUploadedList,"FINWARE");
					
					errorList = (ArrayList) mp.get("errorList");
					if(totalUploadedList.size()>0){
						IFileUpload fileObj=new OBFileUpload();
						fileObj.setFileType("FINWARE_UPLOAD");
						...
						if(trxValueOut!=null){
							...
							for (int j = 0; j < consolidateList.size(); j += batchSize) {
								...
								jdbc.createEntireFinwareStageFile(batchList);
							}
						}
					}
				...
				//	jdbc.spUpdateReleasedAmount("FINWARE_UPLOAD","STAGE");
				}
			}
			else{
				fileType="NOT_CSV";
