...
public class ApproveCCEntryListCommand extends AbstractCommand {
	...
	public HashMap doExecute(HashMap map) throws CommandValidationException, CommandProcessingException,
		...
		try {
			ICommonCodeEntriesTrxValue value = proxy.checkerApproveCategory(trxContext, trxValue);

			resultMap.put("request.ITrxValue", value);
		}
		catch (CommonCodeEntriesException e) {
			throw new CommandProcessingException("failed to approve common code entry transaction for transaction ["
					+ trxValue + "]", e);
		}

		try {
			CommonCodeEntryUtil.synchronizeCommonCode(((OBCommonCodeEntry) trxValue.getStagingCommonCodeEntries()
					.getEntries().iterator().next()).getCategoryCode());
		}
		catch (Exception ex) {
			DefaultLogger.warn("failed to synchronize common code entries", ex);
		}
