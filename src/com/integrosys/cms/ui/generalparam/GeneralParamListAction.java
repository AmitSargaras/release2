 ...
 */
public class GeneralParamListAction extends CommonAction implements IPin {
	...
	public ICommand[] getCommandChain(String event) {
		...
		if (EVENT_READ.equals(event)) {
			return new ICommand[] { (ICommand) getNameCommandMap().get("ReadGeneralParamListCommand") };
			// return new ICommand[]{new ReadActualToStagingGeneralParamListCommand()};
		}
			...
			return new ICommand[] { (ICommand) getNameCommandMap().get("AddGeneralParamListCommand") };
		}
		else if (EVENT_ADD_NOOP.equals(event)) {
			return null;
		}
		else if (EVENT_REMOVE.equals(event)) {
			return new ICommand[] { (ICommand) getNameCommandMap().get("DeleteGeneralParamListCommand") };
		}
		else if (EVENT_REMOVE_NOOP.equals(event)) {
			return null;
		}
		else if (EVENT_SAVE.equals(event)) {
			return new ICommand[] { (ICommand) getNameCommandMap().get("SaveGeneralParamListCommand") };
		}
		else if (EVENT_PAGINATE.equals(event)||"checker_paginate".equals(event)) {
			return new ICommand[] { (ICommand) getNameCommandMap().get("PaginateGeneralParamListCommand") };
		}
		else if (EVENT_SAVE_NOOP.equals(event)) {
			return null;
		}
		else if (EVENT_SUBMIT.equals(event)) {
			return new ICommand[] { (ICommand) getNameCommandMap().get("SubmitGeneralParamListCommand") };
		}
			...
			return new ICommand[] { (ICommand) getNameCommandMap().get("ReadGeneralParamListCommand"),
					(ICommand) getNameCommandMap().get("CompareGeneralParamListCommand") };
		}
		else if (EVENT_READ_MAKER_CLOSE.equals(event)) {
			return new ICommand[] { (ICommand) getNameCommandMap().get("ReadGeneralParamListCommand") };
		}
		else if (EVENT_APPROVE.equals(event)) {
			return new ICommand[] { (ICommand) getNameCommandMap().get("ApproveGeneralParamListCommand"),
					(ICommand) getNameCommandMap().get("ReloadMarketableSecValuationProfileCommand") };
		}
		else if (EVENT_REJECT.equals(event)) {
			return new ICommand[] { (ICommand) getNameCommandMap().get("RejectGeneralParamListCommand") };
		}
		else if (EVENT_LIST_CHECKER_APPROVE_REJECT.equals(event)) {
			return new ICommand[] { (ICommand) getNameCommandMap().get("ListCompareGeneralParamListCommand") };
		}
		else if (EVENT_LIST_MAKER_CLOSE.equals(event)) {
			return new ICommand[] { (ICommand) getNameCommandMap().get("ListViewGeneralParamListCommand") };
		}
		else if (EVENT_CLOSE.equals(event)) {
			return new ICommand[] { (ICommand) getNameCommandMap().get("CloseGeneralParamListCommand") };
		}
		else if (EVENT_LIST_STAGING.equals(event)) {
			return new ICommand[] { (ICommand) getNameCommandMap().get("ListGeneralParamListCommand") };
		}
	...
	public ActionErrors validateInput(ActionForm aForm, Locale locale) {
		DefaultLogger.debug(this, "Inside validate Input  class" + ((GeneralParamListForm) aForm));

		return GeneralParamListFormValidator.validateInput((GeneralParamListForm) aForm, locale);
	}
	...
	public IPage getNextPage(String event, HashMap resultMap, HashMap exceptionMap) {
		...
		IGeneralParamGroupTrxValue value = (IGeneralParamGroupTrxValue) resultMap.get("generalParamGroupTrxValue");
		...
		else if ("checker_paginate".equals(event)) {
			forward = "checker_paginate";
		}
