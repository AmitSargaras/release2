 ...
 * @since $Date: 2005/10/14 08:20:42 $ Tag: $Name: $
 */
public class ApproveSecurityColTaskCommand extends AbstractCommand implements ICommonEventConstant {
	/**
	 * Default Constructor
	 */
	public ApproveSecurityColTaskCommand() {
	}
	...
	public HashMap doExecute(HashMap map) throws CommandProcessingException, CommandValidationException {
		...
		try {
			...
			ICollateralTask stagingTask = colTrxVal.getStagingCollateralTask();

			OBTrxContext ctx = (OBTrxContext) map.get("theOBTrxContext");
			ctx.setTrxCountryOrigin(SecColTaskUtil.getSecColTaskTrxCountry(ctx, stagingTask, limit));

			String remarks = (String) map.get("colRemarks");
			stagingTask.setRemarks(remarks);
			colTrxVal.setStagingCollateralTask(stagingTask);
