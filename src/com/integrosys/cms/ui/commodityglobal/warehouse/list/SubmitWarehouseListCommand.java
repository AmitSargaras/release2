 ...
 * $Header: /home/cms2/cvsroot/cms2/src/com/integrosys/cms/ui/commodityglobal/warehouse/list/SubmitWarehouseListCommand.java,v 1.5 2004/07/08 14:25:15 hshii Exp $
 */
...
public class SubmitWarehouseListCommand extends AbstractCommand {
	...
	public HashMap doExecute(HashMap map) throws CommandProcessingException, CommandValidationException {
		...
		try {
			if ((trxValue.getStagingWarehouse() == null) || (trxValue.getStagingWarehouse().length == 0)) {
				exceptionMap.put("warehouseList", new ActionMessage("error.warehouse.empty"));
			}
			else {
				if (trxValue.getStatus().equals(ICMSConstant.STATE_REJECTED)) {
					trxValue = proxy.makerResubmitUpdateWarehouse(ctx, trxValue);
				}
				else if (trxValue.getStatus().equals(ICMSConstant.STATE_ND)) {
					trxValue = proxy.makerCreateWarehouse(ctx, trxValue.getStagingWarehouse());
				}
				else {
					trxValue = proxy.makerUpdateWarehouse(ctx, trxValue);
				}
			}
		}
		catch (Exception e) {
