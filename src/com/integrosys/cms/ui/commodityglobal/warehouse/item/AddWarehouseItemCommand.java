 ...
 * $Header: /home/cms2/cvsroot/cms2/src/com/integrosys/cms/ui/commodityglobal/warehouse/item/AddWarehouseItemCommand.java,v 1.3 2004/08/19 02:13:41 hshii Exp $
 */
...
public class AddWarehouseItemCommand extends AbstractCommand {
	public String[][] getParameterDescriptor() {
		return (new String[][] {
				...
				{ "warehouseName", "java.lang.String", REQUEST_SCOPE }, });
	}
	...
	public HashMap doExecute(HashMap map) throws CommandProcessingException, CommandValidationException {
		...
		if (existingArray != null) {
			for (int i = 0; (i < existingArray.length) && !duplicated; i++) {
				if (warehouseName.equals(existingArray[i].getName().trim().toUpperCase())) {
					duplicated = true;
				}
			}
		}
		if (duplicated) {
			...
			IWarehouse[] newArray = new IWarehouse[arrayLength + 1];
			if (existingArray != null) {
				System.arraycopy(existingArray, 0, newArray, 0, arrayLength);
			}
			newArray[arrayLength] = obj;
			Arrays.sort(newArray, new WarehouseComparator(WarehouseComparator.BY_NAME));

			trxValue.setStagingWarehouse(newArray);
