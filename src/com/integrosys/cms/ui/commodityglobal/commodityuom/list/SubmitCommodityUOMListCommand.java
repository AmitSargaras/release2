 ...
 * $Header: /home/cms2/cvsroot/cms2/src/com/integrosys/cms/ui/commodityglobal/commodityuom/list/SubmitCommodityUOMListCommand.java,v 1.3 2004/06/17 06:32:06 hshii Exp $
 */
...
public class SubmitCommodityUOMListCommand extends AbstractCommand {
		...
		try {
			if ((trxValue.getStagingUnitofMeasure() == null) || (trxValue.getStagingUnitofMeasure().length == 0)) {
				...
				if (trxValue.getStatus().equals(ICMSConstant.STATE_REJECTED)) {
					trxValue = proxy.makerResubmitUpdateUnitofMeasure(ctx, trxValue);
				}
				else {
					trxValue = proxy.makerUpdateUnitofMeasure(ctx, trxValue);
				}
			}
		}
		catch (Exception e) {
