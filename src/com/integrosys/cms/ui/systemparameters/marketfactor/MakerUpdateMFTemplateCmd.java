 ...
 */
public class MakerUpdateMFTemplateCmd extends AbstractCommand {
	...
	public HashMap doExecute(HashMap map) throws CommandProcessingException, CommandValidationException {
		...
		try {
			...
			//System.out.println("MakerUpdateMFTemplateCmd,template="+template);
			// System.out.println("MakerUpdateMFTemplateCmd,MFTemplateTrxObj="+
			// MFTemplateTrxObj);
			...
			if (exceptionMap.size() == 0) {
				...
				if (MFTemplateTrxObj.getStatus().equals(ICMSConstant.STATE_REJECTED_CREATE)) {
					res = proxy.makerCreateMFTemplate(ctx, MFTemplateTrxObj, template);
				}
				else {
					res = proxy.makerUpdateMFTemplate(ctx, MFTemplateTrxObj, template);
				}
