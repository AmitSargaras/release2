 ...
 */
public class MFItemAction extends MFTemplateAction {
	...
	public ActionErrors validateInput(ActionForm aForm, Locale locale) {
		return MFTemplateValidator.validateMFItem(aForm, locale);
	}
	...
	protected ICommand[] getCommandChain(String event) {
		...
		if (EventConstant.EVENT_PREPARE_CREATE.equals(event)) {
			...
			objArray[0] = new PrepareMFItemCmd();
		}
		else if (EventConstant.EVENT_PREPARE_UPDATE.equals(event)) {
			objArray = new ICommand[2];
			objArray[0] = new ReadMFItemCmd();
			objArray[1] = new PrepareMFItemCmd();
		}
			...
			objArray = new ICommand[1];
			objArray[0] = new SaveMFItemCmd();
		}
		else if (EventConstant.EVENT_CANCEL.equals(event) || EventConstant.EVENT_READ_RETURN.equals(event)) {
