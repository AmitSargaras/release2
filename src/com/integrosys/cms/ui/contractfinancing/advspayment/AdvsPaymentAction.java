 ...
 * @since $Date$ Tag: $Name$
 */
public class AdvsPaymentAction extends ContractFinancingAction {
	...
	public ICommand[] getCommandChain(String event) {
		...
		if (EVENT_LIST_SUMMARY.equals(event)) {
			...
			objArray[0] = new ListAdvanceSummaryCommand();
		}
		else if (EVENT_PREPARE_FORM.equals(event)) {
			objArray = new ICommand[2];
			objArray[0] = new PrepareAdvanceCommand();
			objArray[1] = new ReadAdvanceCommand();
		}
			...
			objArray[0] = new SaveAdvanceCommand();
			objArray[1] = new PrepareAdvanceCommand();
			objArray[2] = new ReadAdvanceCommand();
		}
			...
			objArray = new ICommand[1];
			objArray[0] = new CreateAdvanceCommand();
		}
		else if (EVENT_UPDATE.equals(event)) {
			objArray = new ICommand[1];
			objArray[0] = new UpdateAdvanceCommand();
		}
		else if (EVENT_DELETE.equals(event)) {
			objArray = new ICommand[1];
			objArray[0] = new DeleteAdvanceCommand();
		}
	...
	public ActionErrors validateInput(ActionForm aForm, Locale locale) {
		return AdvanceValidator.validateInput((AdvsPaymentForm) aForm, locale);
	}
