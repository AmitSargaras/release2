 ...
 * @since $Date$ Tag: $Name$
 */
public class SG_PaymentAction extends AdvsPaymentAction {
	...
	public ICommand[] getCommandChain(String event) {
		...
		if (EVENT_PREPARE_FORM.equals(event) || EVENT_MAKER_PREPARE_CREATE.equals(event)) {
			...
			objArray[0] = new SG_PreparePaymentCommand();
			objArray[1] = new SG_ReadPaymentCommand();
		}
			...
			objArray[0] = new SG_ReadPaymentCommand();
		}
		else if (EVENT_CREATE.equals(event)) {
			objArray = new ICommand[1];
			objArray[0] = new SG_CreatePaymentCommand();
		}
		else if (EVENT_UPDATE.equals(event)) {
			objArray = new ICommand[1];
			objArray[0] = new SG_UpdatePaymentCommand();
		}
		else if (EVENT_DELETE.equals(event)) {
			objArray = new ICommand[1];
			objArray[0] = new SG_DeletePaymentCommand();
		}
		else {
	...
	public ActionErrors validateInput(ActionForm aForm, Locale locale) {
		return SG_PaymentValidator.validateInput((SG_PaymentForm) aForm, locale);
	}
