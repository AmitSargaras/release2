 ...
 * @since $Date$ Tag: $Name$
 */
public class PaymentAction extends AdvsPaymentAction {
	...
	public ICommand[] getCommandChain(String event) {
		...
		if (EVENT_PREPARE_FORM.equals(event) || EVENT_MAKER_PREPARE_CREATE.equals(event)) {
			...
			objArray[0] = new PreparePaymentCommand();
			objArray[1] = new ReadPaymentCommand();
		}
			...
			objArray[0] = new ReadPaymentCommand();
		}
		else if (EVENT_CREATE.equals(event)) {
			objArray = new ICommand[1];
			objArray[0] = new CreatePaymentCommand();
		}
		else if (EVENT_UPDATE.equals(event)) {
			objArray = new ICommand[1];
			objArray[0] = new UpdatePaymentCommand();
		}
		else if (EVENT_DELETE.equals(event)) {
			objArray = new ICommand[1];
			objArray[0] = new DeletePaymentCommand();
		}
		else {
	...
	public ActionErrors validateInput(ActionForm aForm, Locale locale) {
		return PaymentValidator.validateInput((PaymentForm) aForm, locale);
	}
