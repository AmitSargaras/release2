 ...
 */
public class SG_AdvanceValidator {

	public static ActionErrors validateInput(SG_AdvsPaymentForm form, Locale locale) {
		...
		try {
			...
			if (SG_AdvsPaymentAction.EVENT_CREATE.equals(form.getEvent())
					|| SG_AdvsPaymentAction.EVENT_UPDATE.equals(form.getEvent())) {
				isSubmit = true;
			}
			...
			if (!(errorCode = Validator.checkAmount(form.getActualAdvanceAmount(), isSubmit, 0,
					IGlobalConstant.MAXIMIUM_ALLOWED_AMOUNT_15, "0", locale)).equals(Validator.ERROR_NONE)) {
				errors.add("actualAdvanceAmount", new ActionMessage(ErrorKeyMapper
