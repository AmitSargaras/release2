 ...
 */
public class SG_AdvanceMapper extends AbstractCommonMapper {
	/**
	 * Default Construtor
	 */
	public SG_AdvanceMapper() {
	}
	...
	public Object mapFormToOB(CommonForm cForm, HashMap map) throws MapperException {
		...
		SG_AdvsPaymentForm aForm = (SG_AdvsPaymentForm) cForm;
		...
		try {
			if (AdvsPaymentAction.EVENT_CREATE.equals(event) || AdvsPaymentAction.EVENT_UPDATE.equals(event)
				...
				try {
					...
					if (!aForm.getActualAdvanceCurrency().equals("") && !aForm.getActualAdvanceAmount().equals("")) {
						obAdvance.setActualAdvanceAmount(CurrencyManager.convertToAmount(locale, aForm
								.getActualAdvanceCurrency(), aForm.getActualAdvanceAmount()));
					}
	...
	public CommonForm mapOBToForm(CommonForm cForm, Object obj, HashMap map) throws MapperException {
		try {
			...
			if (obj != null) {
				...
				if (obAdvance.getActualAdvanceAmount() != null) {
					aForm.setActualAdvanceCurrency(obAdvance.getActualAdvanceAmount().getCurrencyCode());
					aForm.setActualAdvanceAmount(new DecimalFormat("#").format(obAdvance.getActualAdvanceAmount()
							.getAmount()));
				}
				aForm.setExpiryDate(DateUtil.formatDate(locale, obAdvance.getExpiryDate()));

				try {
					if (obAdvance.getOperativeLimitZeroWhenPartialPayment() != null) {
						aForm.setOperativeLimitCurrency(obAdvance.getOperativeLimitZeroWhenPartialPayment()
								.getCurrencyCode());
						aForm.setOperativeLimitAmount(new DecimalFormat("#").format(obAdvance
								.getOperativeLimitZeroWhenPartialPayment().getAmount()));
					}
				}
