 ...
 * @since $Date$ Tag: $Name$
 */
public class TNCAction extends ContractFinancingAction {
	...
	public ICommand[] getCommandChain(String event) {
		...
		if (event.equals(EVENT_LIST_SUMMARY) || event.equals(EVENT_CHECKER_LIST_SUMMARY)) {
			objArray = new ICommand[1];
			objArray[0] = new ListTNCSummaryCommand();
		}
		else if (EVENT_PREPARE_FORM.equals(event) || EVENT_MAKER_PREPARE_CREATE.equals(event)) {
			objArray = new ICommand[1];
			objArray[0] = new PrepareTNCCommand();
		}
																...
																// command
			objArray = new ICommand[1];
			objArray[0] = new ReadTNCCommand();
		}
		else if (event.equals(EVENT_CHECKER_VIEW) || EVENT_VIEW.equals(event)
				|| EVENT_MAKER_PREPARE_DELETE.equals(event) || EVENT_PROCESS.equals(event)) {
			objArray = new ICommand[1];
			objArray[0] = new ReadTNCCommand();
		}
		else if (EVENT_CREATE.equals(event)) {
			objArray = new ICommand[1];
			objArray[0] = new CreateTNCCommand();
		}
		else if (EVENT_UPDATE.equals(event)) {
			objArray = new ICommand[1];
			objArray[0] = new UpdateTNCCommand();
		}
		else if (EVENT_DELETE.equals(event)) {
			objArray = new ICommand[1];
			objArray[0] = new DeleteTNCCommand();
		}
		else {
	...
	public ActionErrors validateInput(ActionForm aForm, Locale locale) {
		return TNCValidator.validateInput((TNCForm) aForm, locale);
	}
