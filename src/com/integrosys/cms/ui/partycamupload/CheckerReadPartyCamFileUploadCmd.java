...
public class CheckerReadPartyCamFileUploadCmd extends AbstractCommand implements ICommonEventConstant{
	public CheckerReadPartyCamFileUploadCmd(){
	}
	...
	public HashMap doExecute(HashMap map) throws CommandProcessingException, CommandValidationException,ComponentException {
		...
		try {
			...
			fileList=jdbc.getAllPartyCamFile(trxValue.getStagingReferenceID());
			...
			long countFail = 0;
				...
				for (int i = 0; i < uploadList.size(); i++) {
					OBPartyCamFile obj = (OBPartyCamFile) uploadList.get(i);
