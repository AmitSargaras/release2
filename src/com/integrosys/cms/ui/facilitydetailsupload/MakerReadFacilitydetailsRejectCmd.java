...
public class MakerReadFacilitydetailsRejectCmd extends AbstractCommand implements ICommonEventConstant{
	public MakerReadFacilitydetailsRejectCmd(){
	...
	public HashMap doExecute(HashMap map) throws CommandProcessingException, CommandValidationException,FileUploadException {
		...
		try {
			...
			SearchResult facilitydetailsFileList = new SearchResult();
			...
			trxValue = (OBFileUploadTrxValue) getFacilitydetailsuploadProxy().getFielUploadByTrxID(fileId);
			IFileUploadJdbc jdbc=(IFileUploadJdbc)BeanHouse.get("fileUploadJdbc");
			facilitydetailsFileList=jdbc.getAllFacilitydetailsFile(trxValue.getStagingReferenceID());
			
			List uploadList=new ArrayList();
			if(facilitydetailsFileList!=null){
				uploadList = new ArrayList(facilitydetailsFileList.getResultList());		
			}
			long countPass = 0;
			long countFail = 0;
			
					for (int i = 0; i < uploadList.size(); i++) {
						OBFacilitydetailsFile oBFacilitydetailsFile = (OBFacilitydetailsFile) uploadList.get(i);
	
						if ("FAIL".equals(oBFacilitydetailsFile.getStatus()) && "N".equals(oBFacilitydetailsFile.getUploadStatus())) {
							countFail++;
						} else if ("PASS".equals(oBFacilitydetailsFile.getStatus()) && "Y".equals(oBFacilitydetailsFile.getUploadStatus())) {
							countPass++;
						}
					}
			...
			resultMap.put("facilitydetailsList", uploadList);
