...
* Command for Create CaseBranch
 */
public class MakerUploadCaseBranchCmd extends AbstractCommand implements ICommonEventConstant {
	
	public static final String HOLIDAY_UPLOAD = "CaseBranchUpload";
	...
	public MakerUploadCaseBranchCmd() {
	}
	 ...
	 */
	 ...
	 public String[][] getResultDescriptor() {
	    ...
	    public HashMap doExecute(HashMap map) throws CommandProcessingException, CommandValidationException {
			...
			/*    if(!caseBranch.getFileUpload().getFileName().toLowerCase().endsWith(".csv")){
			    	fileType ="NOT_CSV";
			    	strError ="errorEveList";
			    }else{

				  //check if any pending files not approved
				if(getCaseBranchProxy().isPrevFileUploadPending()){
					fileUploadPending="PENDING";
					strError ="errorEveList";
				}else{
					//Read Uploaded File 
					ProcessDataFile dataFile = new ProcessDataFile();
					ArrayList resultList = dataFile.processFile(caseBranch.getFileUpload(),HOLIDAY_UPLOAD);
			 ...
			 // If there no error
				if(dataFile.isValid() &&  fileCheckSum.equals("") && fileUploadPending.equals("") && fileType.equals("")){	 
				    	...
				    	trxValueOut = getCaseBranchProxy().makerInsertMapperCaseBranch(ctx, obFileMapperID);
					    ...
					    //insert records in CaseBranch staging table and store it's ID and transactionId in a master table 
						try {
							int noOfRowsInserted = getCaseBranchProxy().insertCaseBranch(obFileMapperMaster, userName, resultList);
							DefaultLogger.debug(this,"No of rows inserted == " + noOfRowsInserted);
							resultMap.put("request.ITrxValue", trxValueOut);
						} catch (Exception e) {
							transDenied="true";     
							strError = "errorEveList";   
							getCaseBranchProxy().deleteTransaction(obFileMapperMaster);
						}
				}
