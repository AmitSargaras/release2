 ...
 */
public class SubmitGenerateReqDeferCommand extends AbstractCommand implements ICommonEventConstant {
	/**
	 * Default Constructor
	 */
	public SubmitGenerateReqDeferCommand() {
	}
	...
	public HashMap doExecute(HashMap map) throws CommandProcessingException, CommandValidationException {
		...
		try {
			...
			IDeferralRequest deferReq = (IDeferralRequest) map.get("deferReq");
			DefaultLogger.debug(this, "deferReq" + deferReq);
			...
			if (ICMSConstant.STATE_REJECTED.equals(iTrxValue.getStatus())) {
				...
				iTrxValue = proxy.makerEditRejectedGenerateRequest(ctx, iTrxValue, deferReq);
			}
			else {
				DefaultLogger.debug(this, ">>>>>>>>>>>>>>>>>>>>>maker generate");
				iTrxValue = proxy.makerGenerateRequest(ctx, iTrxValue, deferReq);
			}
