 ...
 * $Header: /home/cms2/cvsroot/cms2/src/com/integrosys/cms/ui/genreqdefer/GenerateReqDeferAction.java,v 1.2 2003/09/17 10:35:13 elango Exp $
 */
 ...
 * @since $Date: 2003/09/17 10:35:13 $ Tag: $Name: $
 */
public class GenerateReqDeferAction extends CommonAction {
	...
	public ICommand[] getCommandChain(String event) {
		...
		if ("generate_req_defer".equals(event)) {
			objArray = new ICommand[2];
			objArray[0] = new GenerateReqDeferCommand();
			objArray[1] = new FrameFlagSetterCommand();
		}
		else if ("submit".equals(event)) {
			objArray = new ICommand[2];
			objArray[0] = new SubmitGenerateReqDeferCommand();
			objArray[1] = new FrameFlagSetterCommand();
		}
		else if ("approve".equals(event)) {
			objArray = new ICommand[2];
			objArray[0] = new ApproveGenerateReqDeferCommand();
			objArray[1] = new FrameFlagSetterCommand();
		}
		else if ("reject".equals(event)) {
			objArray = new ICommand[2];
			objArray[0] = new RejectGenerateReqDeferCommand();
			objArray[1] = new FrameFlagSetterCommand();
		}
		else if ("process".equals(event) || "edit_staging".equals(event) || "close_item".equals(event)
				|| "to_track".equals(event)) {
			objArray = new ICommand[2];
			objArray[0] = new ReadStagingReqDeferCommand();
			objArray[1] = new FrameFlagSetterCommand();
		}
		else if ("close".equals(event)) {
			objArray = new ICommand[2];
			objArray[0] = new CloseGenerateReqDeferCommand();
			objArray[1] = new FrameFlagSetterCommand();
		}
	...
	public ActionErrors validateInput(ActionForm aForm, Locale locale) {
		...
		// )aForm,locale);
		return GenerateReqDeferFormValidator.validateInput((GenerateReqDeferForm) aForm, locale);
	}
	...
	public IPage getNextPage(String event, HashMap resultMap, HashMap exceptionMap) {
		...
		if (resultMap.get("error") != null) {
			...
			if (ICMSErrorCodes.DEFERRAL_NOT_REQUIRED.equals(errorCode)) {
				aPage.setPageReference("info");
				return aPage;
			}
			if (ICMSErrorCodes.DEFERRAL_PENDING_TRX_EXIST.equals(errorCode)) {
	...
	private String getReference(String event) {
		...
		if ("generate_req_defer".equals(event) || "prepare_form".equals(event)) {
			forwardName = "after_generate_req_defer";
		}
		if ("submit".equals(event)) {
