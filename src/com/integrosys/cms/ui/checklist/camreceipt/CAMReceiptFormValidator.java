 ...
 */
public class CAMReceiptFormValidator {
	public static ActionErrors validateInput(CAMReceiptForm aForm, Locale locale) {
		...
		if("save_receive".equals(event)||"save_waiver".equals(event)||"save_defer".equals(event)||"save_update".equals(event) || 
				CAMReceiptAction.EVENT_PREPARE_TAG_UNTAG_IMAGE.equals(event)){	
		...
		*/
		...
		if("save_receive".equals(event) || CAMReceiptAction.EVENT_PREPARE_TAG_UNTAG_IMAGE.equals(event)){
			// Obervation from HDFC Bank 19-Sep-2013 By Abhijit R
			/*if ((aForm.getOriginalTargetDate() == null) || (aForm.getOriginalTargetDate().length() <= 0)) {
				errors.add("originalTargetDate", new ActionMessage("error.date.mandatory"));
			}*/
		...
		if (!("save_perm_uplift".equals(event) || "save_temp_uplift".equals(event) || "save_relodge".equals(event)
				|| "save_delete".equals(event) || CAMReceiptAction.EVENT_SUBMIT.equals(event) || "save"
				.equals(event))) {
		 ...
		 if(CAMReceiptAction.EVENT_SUBMIT.equals(event)) {
