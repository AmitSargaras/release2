...
public class FrameFlagSetterCommand extends AbstractCommand implements ICommonEventConstant, ITagUntagImageConstant {
	...
	public HashMap doExecute(HashMap map) throws CommandProcessingException, CommandValidationException {
		...
		try {
			...
			/*
				This code will set status as inactive to the respective document.
			*/
			...
			if("approve_checklist_item".equals(event)){
				...
				for (int j = 0; j < temp.length; j++) {					
						String status=temp[j].getDocumentStatus();
						if("PENDING_INACTIVE".equals(status)){
			...
			if (currEvent.equals(CAMReceiptAction.EVENT_REFRESH_SHARE_CHECKLIST)
					|| currEvent.equals(CAMReceiptAction.EVENT_REFRESH_SHARE_CHECKLIST + "_prepare")) {
            ...
            if("view_ok".equals(event)){
            	...
            	resultMap.put("isCustomerCorporate", null);
            }
        }
		catch (Exception e) {
