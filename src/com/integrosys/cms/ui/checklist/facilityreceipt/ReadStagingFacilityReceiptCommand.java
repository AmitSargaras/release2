 ...
 */
public class ReadStagingFacilityReceiptCommand extends AbstractCommand implements ICommonEventConstant {
	/**
	 * Default Constructor
	 */
	public ReadStagingFacilityReceiptCommand() {
	}
	...
	public String[][] getResultDescriptor() {
		return (new String[][] {
				...
				{ "allowToProcess", "java.lang.Boolean", SERVICE_SCOPE },
	...
	public HashMap doExecute(HashMap map) throws CommandProcessingException, CommandValidationException {
		...
		try {
			...
			if(iladDao!=null){
			//	long limitProfileID1 = checkList.getCheckListOwner().getLimitProfileID();
			...
			if("process".equals(event)){
				...
				long limitProfileIDLong = Long.parseLong(limitProfileIDFromTable);	
					if(limitProfileIDLong == limitProfileID) {
						allowToProcess = false;
					}
					System.out.println("========================================================Inside ReadStaging Faciity Receipt Command ");
					System.out.println("value of limitProfileIDFromTable is : "+limitProfileIDFromTable);
					System.out.println("value of limitProfileID is : "+limitProfileID);
					System.out.println("value of allowToProcess is : "+allowToProcess);
	                                System.out.println("inside  Read Staging Facility Receipt Cmd checkFacilityReceiptPending ");
				int count = limitDAO.checkFacilityReceiptPending( String.valueOf(limitProfileID));
				System.out.println("count of LimitProfileID in temp table : "+count);
				if(count==0) {
				limitDAO.insertLimitProfileID( String.valueOf(limitProfileID));
				}
			}
			}}
	              ...
	              DefaultLogger.debug(this, "183 limitId before backend call====>" + limitId);
	              try {
	            	  CheckListSearchResult checkList1=proxy.getCheckListByCollateralID(limitId);
	            	  if(checkList1!=null)	            		  
					checkListIDMap.put(new Long(checkList1.getCheckListID()), obLimit);
	            	  DefaultLogger.debug(this, "186 checkList1 before backend call====>" + checkList1);
			...
			for(int i =0;i<list2.size();i++){
				...
				ICheckListItem OB1 = (ICheckListItem) compareResult.getObj();
				
				if(compareResult.isAdded()||compareResult.isDeleted()||compareResult.isModified()){
					
					linkedList.addFirst(OB1);
				
				}else{
					linkedList.addLast(OB1);
				}
			...
			checkList.setCheckListItemList((ICheckListItem[]) linkedList.toArray(new ICheckListItem[linkedList.size()]));
			 ...
			 DefaultLogger.debug(this, "284 checkList.getCheckListID() before backend call====>" + checkList.getCheckListID());
			...
			resultMap.put("allowToProcess",allowToProcess);
			resultMap.put("flag", "Edit");
			if ("close_checklist_item".equals(event)||"search_facility_doc_checker_close".equals(event)) {
...
//				if(null != list && !"".equalsIgnoreCase(list) && (itemList[i].getDocumentStatus().equalsIgnoreCase("PENDING_INACTIVE") || itemList[i].getDocumentStatus().equalsIgnoreCase("INACTIVE")))
			...
			if(!"ACTIVE".equalsIgnoreCase(list)&& !"INACTIVE".equalsIgnoreCase(list) && !"PENDING_INACTIVE".equalsIgnoreCase(list)){
				if(itemList[i].getItemStatus().equals("PENDING_RECEIVED")||itemList[i].getItemStatus().equals("UPDATE_RECEIVED")){
					if(itemList[i].getItemStatus().equals("PENDING_RECEIVED")&& list.equals("PENDING_RECEIVED")){
						validData=true;
					}
					else if(itemList[i].getItemStatus().equals("UPDATE_RECEIVED")&& list.equals("PENDING_RECEIVED")){
						validData=true;
					}
					else{
						validData=false;
