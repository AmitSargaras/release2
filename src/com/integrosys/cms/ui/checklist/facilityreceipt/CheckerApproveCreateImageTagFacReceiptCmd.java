...
public class CheckerApproveCreateImageTagFacReceiptCmd extends AbstractCommand implements
	 ...
	 * Default Constructor
	 */
	public CheckerApproveCreateImageTagFacReceiptCmd() {
	}
...
//			for (int j = 0; j < actCheckListItems.length; j++) {
			for (int j = 0; j < actCheckListItems1.length; j++) {
				...
				System.out.println("statusOfItem=>"+statusOfItem);
				if("UPDATE_RECEIVED".equals(statusOfItem) || "PENDING_RECEIVED".equals(statusOfItem) ) {
					...
					flag = true;
					System.out.println("actCheckListItems1[j].getFacImageTagUntagImgName()"+imStatus1);
				}
...
//			ICheckListTrxValue checkListTrxValforImageTag = proxyManager.getCheckListByTrxID(trxds1);
//			String statusOfTrxImage = checkListTrxValforImageTag.getStatus();
//			System.out.println("statusOfTrxImage=>"+statusOfTrxImage);
...
//			String imageId = map.get("imageId").toString();
			...
			if(trxds1.equals(trxhistId)) {
				...
				System.out.println("statusOfTrxImage=>"+statusOfTrxImage+" instance=>"+instance+" trxIdofimagetag=>"+trxhistId);
			}else {
			...
			if(flag == true && imStatus1 != null && !"".equals(imStatus1)) {
				System.out.println("flag=>"+flag+" imStatus1=>"+imStatus1);
//			if(imStatus != null) {
			if(resultList != null) {
			...
			if (trxValueOut.getReferenceID() != null) {
				...
				System.out.println("Long.parseLong(trxValueOut.getReferenceID())=>"+Long.parseLong(trxValueOut.getReferenceID()));
			} else {
				imageTagMapvalues.setTagId(Long.parseLong(trxValueOut.getStagingReferenceID()));
				System.out.println("trxValueOut.getStagingReferenceID()=>"+trxValueOut.getStagingReferenceID());
			}
			if(IImageTagConstants.TAG.equals(fromPage)){
				//Retriving existing image list
				System.out.println("IImageTagConstants.TAG.equals(fromPage)");
				...
				for (int i = 0; i < imageIdArray.length; i++) {
					if (!(imageIdArray[i].equals(""))) {
						...
						if(!existingImages.containsKey(imageIdArray[i])){
							System.out.println("Going for checkerCreateImageTagMap in checkerapproveimagefaccmd");
							...
							//
							System.out.println("Going for checkerApproveUpdateImageTagMap in checkerapproveimagefaccmd");
							imageTagMapvalues.setImageId(Long.parseLong(imageIdArray[i]));
