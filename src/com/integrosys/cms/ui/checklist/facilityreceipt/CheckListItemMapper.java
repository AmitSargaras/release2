...
public class CheckListItemMapper extends AbstractCommonMapper {
	...
	public Object mapFormToOB(CommonForm cForm, HashMap map) throws MapperException {
		...
		FacilityReceiptForm aForm = (FacilityReceiptForm) cForm;
...
//			temp.setItemStatus(aForm.getDocumentStatus());
		...
		if("prepare_tag_untag_image".equals(event) || "prepare_tag_untag_image_view".equals(event)){
		...
		if (FacilityReceiptAction.EVENT_REFRESH_SHARE_CHECKLIST.equals(event)) {
...
//			}
			...
			//temp.setFacImageTagUntagId(aForm.getFacImageTagUntagId());
			temp.setFacImageTagUntagImgName(aForm.getFacImageTagUntagImgName());
			//temp.setFacImageTagUntagStatus(aForm.getFacImageTagUntagStatus());
			//End:Added for image tag untag with facility receipt
//			temp.setFacImageTagUntagId(aForm.getDocDesc());
//			temp.setFacImageTagUntagStatus(aForm.getDocumentStatus());
...
//			aForm.setHdfcAmt(UIUtil.formatWithCommaAndDecimal(tempOb.getHdfcAmt()));
			...
			//Added for image tag untag with facility receipt
//			aForm.setFacImageTagUntagId(tempOb.getFacImageTagUntagId());
//			aForm.setFacImageTagUntagImgName(tempOb.getFacImageTagUntagImgName());
//			aForm.setFacImageTagUntagStatus(tempOb.getFacImageTagUntagStatus());
			//End:Added for image tag untag with facility receipt
...
//			aForm.setOriginalTargetDate(DateUtil.formatDate(locale, tempOb.getOriginalTargetDate()));
			...
			if(!"imagetaguntaglist".equals(aForm.getEvent()) && !"facilityreceipt_image_tag_untag_submit".equals(aForm.getEvent())) {
			aForm.setCreditApprover(tempOb.getCreditApprover());
			aForm.setDocDate(DateUtil.formatDate(locale, tempOb.getDocDate()));
			aForm.setDocExpDate(DateUtil.formatDate(locale, tempOb.getExpiryDate()));
			aForm.setHdfcAmt(UIUtil.formatWithCommaAndDecimal(tempOb.getHdfcAmt()));
			...
			if("facilityreceipt_image_tag_untag_submit".equals(aForm.getEvent()) || "RECEIVECANCEL".equals(aForm.getEvent())) {
				String combinedStrng = aForm.getFacImageTagUntagId();
...
//				aForm.setFacImageTagUntagImgName("");
//			}
			// CMSSP
			...
			if("RECEIVE".equals(aForm.getEvent()) && ("UPDATE_RECEIVED".equals(tempOb.getItemStatus()) || ("PENDING_RECEIVED".equals(tempOb.getItemStatus())))) {
				...
				aForm.setFacImageTagUntagId("");
				aForm.setFacImageTagUntagImgName("");
				aForm.setFacImageTagUntagStatus("");
			}
																								// 522
	...
	private IShareDoc[] getIShareDocFormToOB(FacilityReceiptForm aForm) {
