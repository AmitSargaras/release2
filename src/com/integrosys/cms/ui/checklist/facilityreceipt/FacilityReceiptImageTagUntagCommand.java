...
public class FacilityReceiptImageTagUntagCommand extends AbstractCommand implements ICommonEventConstant{
	...
	public String[][] getParameterDescriptor() {
		return (new String[][] { { "frame", "java.lang.String", SERVICE_SCOPE },
				...
				{"taguntagimage", "java.util.List", SERVICE_SCOPE},
				{"taguntagimage", "java.util.List", REQUEST_SCOPE},
	...
	public HashMap doExecute(HashMap map) throws CommandProcessingException, CommandValidationException {
		...
		try {
			...
			String checklistRefId=String.valueOf(checkListItem.getCheckListItemRef());
			String checklistIdForResubmit = getCheckListListForResubmit(checklistRefId);
			System.out.println("Checklist from checklist Reference Id=>checklistId="+checklistId+" checklistIdForResubmit="+checklistIdForResubmit+" checklistRefId=>"+checklistRefId);
			checklistId = checklistIdForResubmit;
			...
			if(event.equals("facilityreceipt_image_tag_untag_submit")) {
				...
				ArrayList taguntagimage = (ArrayList) map.get("taguntagimage");
				resultMap.put("taguntagimage", taguntagimage);
				System.out.println("tagUntag List == "+taguntagimage);
...
private List getImageTagUntagList(String checklistId, String custIdNum,List imageFileNameListFromTemp) {
		...
		String sql="SELECT IMG_FILENAME,IMG_ID,STATUS " + 
				...
				"      ) " + 
				"    ) " + 
				"  ) AND CUST_ID ='"+custIdNum+"' AND category = 'IMG_CATEGORY_FACILITY' ";
//		String sql = "SELECT * FROM CMS_UPLOADED_IMAGES  WHERE CUST_ID = '"+custIdNum+"'  AND IMG_DEPRICATED='N' AND CATEGORY = 'IMG_CATEGORY_FACILITY' ";
		System.out.println("FacilityReceiptImageTagUntagCommand.java => getImageTagUntagList() Query sql=>"+sql);
		DBUtil dbUtil=null;
		DBUtil dbUtil1=null;
		DBUtil dbUtil2=null;
		DBUtil dbUtil3=null;
...
//			 dbUtil5=new DBUtil();
			...
			if(null!=rs){
				while(rs.next()){
					...
					System.out.println("FacilityReceiptImageTagUntagCommand.java => getImageTagUntagList() Query sql inside while=>");
					System.out.println("IMG_FILENAME=>"+data+" IMG_ID=>"+data2+" STATUS=>"+data4);
					...
					sql1 = "SELECT DISTINCT UNTAGGED_STATUS  " + 
							...
							"				    )  " + 
							"				   )";
					System.out.println("FacilityReceiptImageTagUntagCommand image_id = "+data2+" doc_item_id as checklist id = "+checklistId);
					System.out.println("FacilityReceiptImageTagUntagCommand image_id sql1 query=>"+sql1);
					dbUtil1.setSQL(sql1);
					rs1 = dbUtil1.executeQuery();
					while(rs1.next()){
					data3=rs1.getString("UNTAGGED_STATUS");
					System.out.println("FacilityReceiptImageTagUntagCommand inside while UNTAGGED_STATUS data3=>"+data3);
					}
					imageDataList.add(data);
					imageDataList.add(data2);
					imageDataList.add(data3);
					imageDataList.add(data4);
					System.out.println("FacilityReceiptImageTagUntagCommand data="+data+" data2="+data2+" data3="+data3+" data4="+data4);
					if(!imageDataList.isEmpty()) {
						filenameOFimageList.add(imageDataList);
					}
					System.out.println("data3 UNTAGGED_STATUS after added to imagedatalist==>"+data3);
					data3="";
				}
			}
...
//			sql2 = "SELECT DISTINCT IMG_FILENAME,IMG_ID FROM CMS_TEMP_IMAGE_UPLOAD WHERE TYPE_OF_DOC = 'Facility'";
//			sql2 = "SELECT DISTINCT IMG_FILENAME,IMG_ID FROM CMS_TEMP_IMAGE_UPLOAD WHERE CATEGORY = 'IMG_CATEGORY_FACILITY'";
			sql2 = "SELECT image_id,UNTAGGED_STATUS " + 
					...
					"    )";
			System.out.println("SQL2 query faility=>"+sql2);
			dbUtil2.setSQL(sql2);
			rs2 = dbUtil2.executeQuery();
			if(null!=rs2){
				while(rs2.next()){
					System.out.println("SQL2 query faility=>Inside facility sql2 while");
					List imageDataList = new ArrayList();
					...
					System.out.println("SQL2 = image_id="+data2+" UNTAGGED_STATUS="+data3+" data4(status) is kept empty");
					sql5 = "SELECT IMG_ID FROM CMS_UPLOADED_IMAGES WHERE IMG_ID='"+data2+"'";
					
					System.out.println("sql5 query=>"+sql5);
					dbUtil5=new DBUtil();
					dbUtil5.setSQL(sql5);
					rs5 = dbUtil5.executeQuery();
					while(rs5.next()){
						data5=rs5.getString("IMG_ID");
						System.out.println("IMG_ID data5="+data5);
					}
					
					System.out.println("facility Outside while IMG_ID data5="+data5);
					...
					System.out.println("FacilityReceiptImageTagUntagCommand THIRD QUERY FOR IMAGE FILE NAME sql3=>"+sql3);
					...
					System.out.println("FacilityReceiptImageTagUntagCommand THIRD QUERY FOR IMAGE FILE NAME IMG_FILENAME=>"+data);
					}
					...
					System.out.println("FacilityReceiptImageTagUntagCommand THIRD QUERY data="+data+" data2="+data2+" data3="+data3+" data4="+data4);
			...
			sql4 = "SELECT DISTINCT IMG_FILENAME,IMG_ID,STATUS FROM CMS_TEMP_IMAGE_UPLOAD WHERE CATEGORY = 'IMG_CATEGORY_FACILITY' and CUST_ID ='"+custIdNum+"' " + 
					"    AND IMG_ID = '"+imageFileNameListFromTemp.get(i)+"' ";
			System.out.println("Temp folder Query In Facility sql4=>"+sql4);
			...
			imageDataList.add(data3);
			imageDataList.add(data4);
			System.out.println("FacilityReceiptImageTagUntagCommand sql4 FOURTH QUERY data="+data+" data2="+data2+" data3="+data3+" data4="+data4);
...
//			}
			...
			String sqls = "SELECT IMG_FILENAME,IMG_ID,STATUS " + 
					"FROM CMS_UPLOADED_IMAGES " + 
					"WHERE IMG_DEPRICATED = 'N' AND CUST_ID = '"+custIdNum+"' AND CATEGORY = 'IMG_CATEGORY_FACILITY' AND STATUS = '3' AND IMG_ID NOT IN " + 
			...
			data4 = "";
			data3 = "";
			System.out.println("FacilityReceiptImageTagUntagCommand => Quesry sqls=>"+sqls);
			...
			dbUtils=new DBUtil();
				...
				if(null!=rss){
					while(rss.next()){
						...
						data = rs.getString("IMG_FILENAME");
						data2=rs.getString("IMG_ID");
						data3=rs.getString("STATUS");
						data4="Y";
						System.out.println("FacilityReceiptImageTagUntagCommand.java => getImageTagUntagList() Query sqls from cms_upload_image only, inside while=>");
						...
						if(data != null && !"".equals(data)) {					
							...
							System.out.println("FacilityReceiptImageTagUntagCommand only from table cms_uploaded_images  data="+data+" data2="+data2+" data3="+data3+" data4="+data4);
...
private List getImageTagUntagListFromTemp(String custIdNum,String checklistId) {
	...
	System.out.println("getImageTagUntagListFromTemp => sql2=>"+sql2);
	ResultSet rs2=null;
	dbUtil2=new DBUtil();
	dbUtil2.setSQL(sql2);
	rs2 = dbUtil2.executeQuery();
	if(null!=rs2){
		while(rs2.next()){
			data2 = rs2.getString("image_id");
			imageDataList.add(data2);
			System.out.println("getImageTagUntagListFromTemp => sql2=>Image id=>"+data2);
		}
	}
	if(rs2 != null) {
		rs2.close();
	...
	String sql="SELECT DISTINCT IMG_ID FROM CMS_TEMP_IMAGE_UPLOAD WHERE CATEGORY = 'IMG_CATEGORY_FACILITY' and CUST_ID ='"+custIdNum+"' ";
	System.out.println("FacilityReceiptImageTagUntagCommand.java => getImageTagUntagListFromTemp() Query sql=>"+sql);
		...
		if(null!=rs){
			while(rs.next()){
				...
				System.out.println("getImageTagUntagListFromTemp => sql =>IMG_ID =>"+data);
...
private String getCheckListListForResubmit(String checklistRefId) {
//	List filenameOFimageList = new ArrayList();
	String data="";
	DBUtil dbUtil=null;
	try {
		String sql = "SELECT DOC_ITEM_ID FROM cms_checklist_item WHERE DOC_ITEM_REF = '"+checklistRefId+"' ";
	
	System.out.println("FacilityReceiptImageTagUntagCommand.java => getCheckListListForResubmit() Query sql=>"+sql);
		...
		if(null!=rs){
			while(rs.next()){
				data=rs.getString("DOC_ITEM_ID");
				System.out.println("FacilityReceiptImageTagUntagCommand.java => getCheckListListForResubmit() DOC_ITEM_ID=>"+data);
			}
		}
		rs.close();
	}catch (DBConnectionException e) {
