...
public class RetriveImageCommand extends AbstractCommand {
	...
	public HashMap doExecute(HashMap map) throws CommandProcessingException,
		...
		System.out.println("Inside facilityreceipt/RetriveImageCommand.java=> For View image Action Click.");
		System.out.println("pID --->" + pID + "    imageName=>" + imageName + "      status=>" + status);
		String imagePath="";
		String category=(String) map.get("category");
	
		String custLimitProfileID=(String) map.get("custLimitProfileID");
		String event=(String) map.get("event");
		System.out.println("First New Event=>"+event);
		...
		System.out.println("After Set => pID->" + pID + "    imageName=>" + imageName + "      status=>" + status);
		...
		/*if(count!=null && !count.equals("")&& Integer.parseInt(count)%2==0){
			...
			try {
				...
				if(null == img.getHCPStatus() || img.getHCPStatus().equals("N")) {
					System.out.println("Inside facilityreceipt/RetriveImageCommand.java=>  null == img.getHCPStatus() || img.getHCPStatus().equals(N)");
					imagePath=(String)getContentManagerFactory().getContentManagerService().retrieveDocument(params);
		...
		if(imagePath!=null){
			try {
				...
				System.out.println("RetriveImageCommand.java=> basePath =>"+basePath+"  pdfFilePath=>"+pdfFilePath);
				...
				System.out.println("RetriveImageCommand.java=> basePath+imagePath=>"+basePath+imagePath);
			//	com.lowagie.text.Image image = 
			...
			try{
				...
				System.out.println("RetriveImageCommand of Facility=>inside else imagename.");
				String basePath=PropertyManager.getValue("contextPath");
				...
				System.out.println("RetriveImageCommand of Facility=>basePath=>"+basePath+" pdfFilePath=>"+pdfFilePath);
				DefaultLogger.debug(this,"inside else pdfFilePath:"+pdfFilePath);
				pdfFilePath=pdfFilePath.replace(' ', '_');
				DefaultLogger.debug(this,"inside else pdfFilePath:"+pdfFilePath);
				System.out.println("RetriveImageCommand of Facility=>basePath=>"+basePath+" pdfFilePath After replace _ =>"+pdfFilePath);
				fileData = IOUtils.toByteArray(new FileInputStream(new File(pdfFilePath)));
				output.write(fileData);
				DefaultLogger.debug(this,"done");
				System.out.println("done");
