 ...
 */
public class CCReceiptFormValidator {
	public static ActionErrors validateInput(CCReceiptForm aForm, Locale locale) {
		ActionErrors errors = new ActionErrors();
		String errorCode = "";
		String event = aForm.getEvent();
		DefaultLogger.debug("$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$", "Inside Validation, event=" + event);

		Date d = DateUtil.getCalendar().getTime();
		d = DateUtil.clearTime(d);

		if (CCReceiptAction.EVENT_SUBMIT.equals(event) || "save".equals(event)) {
			Date deficiencyDate = null;
			Date nextActionDate = null;
			if (!(errorCode = Validator.checkDate(aForm.getDeficiencyDate(), false, locale))
					.equals(Validator.ERROR_NONE)) {
				errors.add("deficiencyDate", new ActionMessage(ErrorKeyMapper.map(ErrorKeyMapper.DATE, errorCode)));
			}
			else {
				deficiencyDate = DateUtil.convertDate(locale, aForm.getDeficiencyDate());
			}
			if (!(errorCode = Validator.checkDate(aForm.getNextActionDate(), false, locale))
					.equals(Validator.ERROR_NONE)) {
				errors.add("nextActionDate", new ActionMessage(ErrorKeyMapper.map(ErrorKeyMapper.DATE, errorCode)));
			}
			else {
				nextActionDate = DateUtil.convertDate(locale, aForm.getNextActionDate());
			}
			DefaultLogger.debug("cc form validator", "deficientyDate: " + deficiencyDate + "\tnextActionDate:"
					+ nextActionDate);
			if ((deficiencyDate != null) && (nextActionDate != null)) {
				if (deficiencyDate.after(nextActionDate)) {
					errors.add("nextActionDate", new ActionMessage("error.date.compareDate", "Next Action Date",
							"Date of Deficiency Identified"));
				}
			}
			if (deficiencyDate != null) {
				if (UIValidator.isFutureDate(deficiencyDate)) {
					errors.add("deficiencyDate", new ActionMessage("error.date.compareDate", "System Date",
							"Date of Deficiency Identified"));
				}
			}
			if (nextActionDate != null) {
				if (UIValidator.isExpiryDate(nextActionDate)) {
					errors.add("nextActionDate", new ActionMessage("error.date.compareDate", "Next Action Date",
							"System Date"));
				}
			}
		}
		else if ("save_receive".equals(event) || aForm.getAwaiting().equals("true")) {

			/*
			 * if (!(errorCode = Validator.checkString(aForm.getDocRef(), true,
			 * 1, 100)).equals(Validator.ERROR_NONE)) { errors.add("docRef", new
			...
			/*
			 * DefaultLogger.debug(CCReceiptFormValidator.class.getName(),"entered
			 * date after Locale"
			 * +DateUtil.formatDate(locale,DateUtil.convertDate(locale,aForm.
			 * getDocDate())));
			 * DefaultLogger.debug(CCReceiptFormValidator.class.
			 * getName(),"System Date"+d); if(aForm.getDocDate().length() > 0) {
			 * int a =
			 * d.compareTo(DateUtil.clearTime(DateUtil.convertDate(locale
			 * ,aForm.getDocDate()))); DefaultLogger.debug("vaidation
			 * ***********************************" ,"Eroororr date "+ a); if(a<0){
			 * errors.add("docDate", new ActionMessage
		...
		if (!(errorCode = Validator.checkDate(aForm.getIdentifyDate(), false, locale)).equals(Validator.ERROR_NONE)) {
			errors.add("identifyDate", new ActionMessage("error.date.format"));
		}
		...
		if ("save_defer_req".equals(event) || "save_defer".equals(event)) {
			...
			if (!aForm.getOrigDeferDate().equals(aForm.getDeferDate())) {
				if (!(errorCode = Validator.checkDate(aForm.getDeferDate(), true, locale)).equals(Validator.ERROR_NONE)) {
					...
					if (aForm.getDeferDate().length() > 0) {
						DefaultLogger.debug(CCReceiptFormValidator.class.getName(), "Current Date " + d);
						DefaultLogger.debug(CCReceiptFormValidator.class.getName(), "date from Form"
								+ aForm.getDeferDate());

						if (d.after(DateUtil.clearTime(DateUtil.convertDate(locale, aForm.getDeferDate())))) {
			...
			if (!aForm.getOrigDeferExtendedDate().equals(aForm.getDeferExtendedDate())) {
				if (!(errorCode = Validator.checkDate(aForm.getDeferExtendedDate(), true, locale))
					...
					} // else { //defer extended date is not a mandatory
					// field.
		...
		if ("save_update".equals(event)) {
			...
			if (aForm.getDocDate().length() > 0) {
				if (DateUtil.clearTime(DateUtil.convertDate(locale, aForm.getDocDate())).after(
					...
					errors.add("docDate", new ActionMessage("error.date.compareDate.more", "Doc Date", "Current Date"));
				}
			}
		...
		if ("save_remind".equals(event) || "save_receive".equals(event) || "save_waiver_req".equals(event)
			...
			if (!(errorCode = Validator.checkDate(aForm.getWaivedDate(), false, locale)).equals(Validator.ERROR_NONE)) {
				errors.add("waivedDate", new ActionMessage(ErrorKeyMapper.map(ErrorKeyMapper.DATE, errorCode)));
			}
			if (!(errorCode = Validator.checkDate(aForm.getCompletedDate(), false, locale))
					.equals(Validator.ERROR_NONE)) {
				errors.add("completedDate", new ActionMessage(ErrorKeyMapper.map(ErrorKeyMapper.DATE, errorCode)));
			}
			if (!(errorCode = Validator.checkDate(aForm.getLodgedDate(), false, locale)).equals(Validator.ERROR_NONE)) {
				errors.add("lodgedDate", new ActionMessage(ErrorKeyMapper.map(ErrorKeyMapper.DATE, errorCode)));
			}
			if ((aForm.getDocDate().length() > 0) && (aForm.getDocExpDate().length() > 0)) {
		...
		// Start for cr-17
		// if ("save_complete".equals(event) || "save_update".equals(event) ||
		// "save_update_renewal".equals(event)) {
		DefaultLogger.debug("CCReceiptFormValidator", ">>>>>>>>>>>> EVENT (Validator): " + event);
		...
		if (!("save_perm_uplift".equals(event) || "save_temp_uplift".equals(event) || "save_relodge".equals(event)
				|| "save_delete".equals(event) || CCReceiptAction.EVENT_SUBMIT.equals(event) || "save".equals(event))) {
			DefaultLogger.debug("CCReceiptFormValidator", ">>>>>>>>>> Validating for share checklist!!! ");
			String[] checkListId = aForm.getCheckListId();
			DefaultLogger.debug("CCReceiptFormValidator", ">>>>>>>>>> (CC Validator) checklistId: " + checkListId);
			...
			if ((deleteCheckListId != null) && (deleteCheckListId.length > 0)) {
				deleteCheckListIdList = Arrays.asList((String[]) deleteCheckListId);
				DefaultLogger.debug("CCReceiptFormValidator", ">>>>>>>>>>>>>> deleted checklist id list: "
						+ deleteCheckListIdList);
			}
			if ((aForm.getShared() != null) && ((aForm.getShared().length() == 0) || aForm.getShared().equals("true"))) {
				...
				if ((checkListId == null) || (checkListId.length == 0)
					...
					errors.add("noChecklist", new ActionMessage("error.ccChecklistId.noChecklist"));
				}
						...
						if (!((deleteCheckListIdList != null) && deleteCheckListIdList.contains(i + ""))) {
							if (StringUtils.isEmpty(checkListId[i]) || (checkListId[i].length() == 0)
								...
								if (!(StringUtils.isNumeric(checkListId[i]))) {
									...
									errors.add("errCheckListId" + i,
											new ActionMessage("error.checklistId.ownChecklist"));
								}
								else {
				...
				// ActionMessage("error.ccChecklistId.noChecklist"));
				// }
				...
				DefaultLogger.debug("CCReceiptFormValidator", ">>>>>>>>>> (CC Validator) checklistId: " + checkListId);
				DefaultLogger.debug("CCReceiptFormValidator", ">>>>>>>>>> (CC Validator) deleteCheckListIdList: "
						+ deleteCheckListIdList);
				if ((checkListId != null) && (checkListId.length > 0)) {
					// String[] isDeletedInd = aForm.getIsDeletedInd();
					outer: for (int i = 0; i < checkListId.length; i++) {
						// if (!("true".equals(isDeletedInd[i]))) {
						if ((deleteCheckListIdList != null) && deleteCheckListIdList.contains(i + "")) {
						}
						else {
							errors.add("noChecklist", new ActionMessage("error.ccChecklistId.deleteChecklist"));
							break outer;
						}
						// }
					}
		...
		/*
		 * if ("add_SHARE_CHECKLIST".equals(event) ||
		 * "update_SHARE_CHECKLIST".equals(event) ||
		 * "refresh_share_checklist".equals(event)) { String[] checkListId =
		 * aForm.getCheckListId(); String[] deleteCheckListId =
		 * aForm.getDeleteCheckListId(); String itemCheckListId =
		 * aForm.getCheckListID(); List deleteCheckListIdList = new ArrayList();
		 * if (deleteCheckListId != null && deleteCheckListId.length > 0) {
		 * deleteCheckListIdList = Arrays.asList((String[]) deleteCheckListId); }
		 * 
		 * if (checkListId != null && checkListId.length > 0) { outer: for (int
		 * i = 0; i < checkListId.length; i++) { if (deleteCheckListIdList !=
		 * null && deleteCheckListIdList.contains(i + "")) { } else { if
		 * (StringUtils.isEmpty(checkListId[i]) || checkListId[i].length() == 0 ||
		 * checkListId[i].trim().length() == 0) { errors.add("errCheckListId" +
		 * i, new ActionMessage("error.string.mandatory")); } else { if
		 * (!(StringUtils.isNumeric(checkListId[i]))) {
		 * errors.add("errCheckListId" + i, new
		 * ActionMessage("error.amount.format")); }else if
		 * (checkListId[i].equals(itemCheckListId)) {
		 * errors.add("errCheckListId" + i, new
		 * ActionMessage("error.checklistId.ownChecklist")); } else { for (int j =
		 * 0; j < checkListId.length; j++) { if (i != j) { if
		 * (checkListId[i].equals(checkListId[j])) { errors.add("errCheckListId" +
		 * i, new ActionMessage("error.checklistId.duplicate")); break outer; } } } //
		 * end for loop } } } } } }
		 */
		// End for cr-17
