 ...
 */
public class SubmitAnnexureCommand extends AbstractCommand {
	...
	public HashMap doExecute(HashMap map) throws CommandProcessingException, CommandValidationException {
		...
		if (checkListTrxVal.getStatus().equals(ICMSConstant.STATE_REJECTED)) {
			try {
				checkListTrxVal = getRecurrentProxyManager().makerEditRejectedAnnexureReceiptTrx(ctx, checkListTrxVal,
						recChkLst);
			}
			...
			try {
				checkListTrxVal = getRecurrentProxyManager().makerUpdateAnnexureReceipt(ctx, checkListTrxVal,
						recChkLst);
			}
			catch (RecurrentException ex) {
