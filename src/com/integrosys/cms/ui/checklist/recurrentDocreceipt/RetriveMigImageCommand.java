...
public class RetriveMigImageCommand extends AbstractCommand {
	...
	/*			{ "checkListID", "java.lang.String", REQUEST_SCOPE },
				...
				{ "migCheckListImages", "java.util.List", SERVICE_SCOPE },
	...
	public HashMap doExecute(HashMap map) throws CommandProcessingException,
		...
		List migCheckListImages =(List) map.get("migCheckListImages");
		
        String[] img = (String[]) migCheckListImages.get(Integer.parseInt(count));
		
		pID= img[3];
		status= String.valueOf(img[5]);
		imageName=(String) img[4];
		String hcpFileName=(String) img[6];
		String hcpStatus=(String) img[7];
		...
		imagePath="C:\\Documents and Settings\\All Users\\Documents\\My Pictures\\Sample Pictures"+"\\"+imageName;
			...
			try {
				...
				System.out.println("ContentManager to HCP........");
				String url=PropertyManager.getValue("hcp.rest.url");
				System.out.println("url = "+url+" , hcpFileName = "+hcpFileName+" , hcpStatus = "+hcpStatus);
				
				if(null == hcpStatus || "N".equals(hcpStatus)) {
					imagePath=(String)getContentManagerFactory().getContentManagerService().retrieveDocument(params);
				}else {
					System.out.println("inSide HTC..........");
					System.out.println("inSide HTC img.getHCPFileName().........."+hcpFileName);
					imagePath=(String)getContentManagerFactory().getContentManagerService().get(hcpFileName,String.valueOf(status));
		...
		byte[] fileData;
		System.out.println("before if imagename: ");
		if(!((imageName.toUpperCase()).endsWith(".PDF"))){
			System.out.println("inside if imagename: ");
		if(imagePath!=null){
			try {
				...
				if(null == hcpStatus || "N".equals(hcpStatus)) {
					...
					System.out.println("img.getHCPStatus()img.getHCPStatus().........."+hcpStatus);
					System.out.println("pdfFilePath.........."+pdfFilePath);
					pdfFilePath=basePath+"/dmsImages/"+hcpFileName.split("[.]")[0]+".pdf";
					System.out.println("img.getHCPFileName().split(\"[.]\")[0]............."+hcpFileName.split("[.]")[0]);
					System.out.println("pdfFilePath.........."+pdfFilePath);
				}
			...
			try{
				...
				if(null == hcpStatus || "N".equals(hcpStatus)) {
					...
					pdfFilePath=basePath+"/dmsImages/downloads/"+hcpFileName.split("[.]")[0]+".pdf";	
		...
		result.put("documentVersion", documentVersion);
		result.put("index", index);
		result.put("event", event);
		result.put("output", output);
		result.put("migCheckListImages", migCheckListImages);
		result.put("checkList", checkList);
		result.put("checkListForm", checkList);
		//Added by Anil ===============End
