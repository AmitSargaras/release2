...
import com.integrosys.cms.ui.checklist.secreceipt.SecurityReceiptAction;
 ...
 */
public class RecurrentDocReceiptFormValidator {
	public static ActionErrors validateInput(RecurrentDocReceiptForm aForm, Locale locale) {
		...
		if ("save_receive".equals(event) || aForm.getAwaiting().equals("true") ||
				"save_receive".equals(event) || RecurrentDocReceiptAction.EVENT_PREPARE_TAG_UNTAG_IMAGE_RECEIVE.equals(event)) {
		/*	if ((aForm.getDocExpDate() == null) || (aForm.getDocExpDate().length() <= 0)) {
				errors.add("docExpDate", new ActionMessage("error.date.mandatory"));
			}*/
		...
		if ("save_defer_req".equals(event) || "save_defer".equals(event)) {
			...
			if (!aForm.getOrigDeferDate().equals(aForm.getDeferDate())) {
				if (!(errorCode = Validator.checkDate(aForm.getDeferDate(), true, locale)).equals(Validator.ERROR_NONE)) {
					...
					if (aForm.getDeferDate().length() > 0) {
						// int a = d.compareTo();
						DefaultLogger.debug(SecurityReceiptFormValidator.class.getName(), "Current Date " + d);
						DefaultLogger.debug(SecurityReceiptFormValidator.class.getName(), "date from Form"
								+ aForm.getDeferDate());
			...
			if (!(errorCode = Validator.checkString(aForm.getDocRemarks(), false, 1, 200))
					.equals(Validator.ERROR_NONE)) {
				errors.add("docRemarks", new ActionMessage(ErrorKeyMapper.map(ErrorKeyMapper.STRING, errorCode), "1",
						"200"));
			}
//			if (!(errorCode = Validator.checkString(aForm.getDocJustification(), false, 1, 200))
//					.equals(Validator.ERROR_NONE)) {
//				errors.add("docJustification", new ActionMessage(ErrorKeyMapper.map(ErrorKeyMapper.STRING, errorCode), "1",
//						"200"));
//			}
  ...
  //          if(aForm.getActionParty() == null || aForm.getActionParty().length()<=0) {
   //             errors.add("actionParty", new ActionMessage("error.mandatory"));
    //        }
		...
		if ("save_update".equals(event) || RecurrentDocReceiptAction.EVENT_PREPARE_TAG_UNTAG_IMAGE_UPDATE.equals(event)) {
...
*/
		...
		if ("save_remind".equals(event) || "save_receive".equals(event) || "save_waiver_req".equals(event)
				...
				|| "save_delete".equals(event) || "save_relodge".equals(event) || RecurrentDocReceiptAction.EVENT_PREPARE_TAG_UNTAG_IMAGE_RECEIVE.equals(event) ||
				RecurrentDocReceiptAction.EVENT_PREPARE_TAG_UNTAG_IMAGE_UPDATE.equals(event)) {
			...
			if("save_receive".equals(event) || RecurrentDocReceiptAction.EVENT_PREPARE_TAG_UNTAG_IMAGE_RECEIVE.equals(event)){
			...
			if("save_waiver".equals(event) ){
				if (aForm.getWaivedDate().trim().length() > 0) {
					if (DateUtil.clearTime(DateUtil.convertDate(locale, aForm.getWaivedDate())).after(
							DateUtil.clearTime(DateUtil.getDate()))) {
						DefaultLogger.debug("aForm.getWaivedDate()", aForm.getWaivedDate());
						DefaultLogger.debug("DateUtil.getDate()", DateUtil.getDate());
						...
						errors.add("waivedDate", new ActionMessage("error.date.compareDate", "Waived Date",
							"Received Date"));
					}else if(StringUtils.isNotBlank(aForm.getDocDate())&&
							(DateUtil.clearTime(DateUtil.convertDate(locale, aForm.getWaivedDate())).before(
									DateUtil.clearTime(DateUtil.convertDate(locale, aForm.getDocDate()))))){
						DefaultLogger.debug("aForm.getWaivedDate()", aForm.getWaivedDate());
						DefaultLogger.debug("aForm.getDocDate()", aForm.getDocDate());
								errors.add("waivedDate", new ActionMessage("error.date.compareDate", "Waived Date",
									"Document Date"));
							}
				}else{
   ...
   //             if(aForm.getActionParty() == null || aForm.getActionParty().length()<=0) {
    ...
    //        }
		...
		if (!("save_perm_uplift".equals(event) || "save_temp_uplift".equals(event) || "save_relodge".equals(event)
				|| "save_delete".equals(event) || SecurityReceiptAction.EVENT_SUBMIT.equals(event) || "save"
				.equals(event))) {
        ...
        if(SecurityReceiptAction.EVENT_SUBMIT.equals(event)) {
