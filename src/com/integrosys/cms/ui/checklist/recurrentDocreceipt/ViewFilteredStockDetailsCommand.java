			...
			if(actualGeneralChargeDetails!=null){
				for (int i = 0; i < actualGeneralChargeDetails.length; i++) {
					actualChargeDetails = actualGeneralChargeDetails[i];
						 ...
						 appFlag = true;
					 if(actualChargeDetails!=null && actualChargeDetails.getDocCode().equals(docCode)){
...
//								}
								...
								if(actualChargeDetails.getDpShare()!=null || "".equals(actualChargeDetails.getDpShare())){
									fundedShareVal = actualChargeDetails.getDpShare();
...
//	    	Collections.sort(retrivedDueDateList,Collections.reverseOrder());
		...
		BigDecimal totalLonableLiability= new BigDecimal(0);
			...
			if(existingGeneralChargeDetails!=null){
				for (int i = 0; i < existingGeneralChargeDetails.length; i++) {
					...
					if(existingChargeDetails!=null && null != existingChargeDetails.getDocCode()  && existingChargeDetails.getDocCode().equals(docCode)&& existingChargeDetails.getDueDate().equals(dueDate)){
	...
	//						BigDecimal totalLonable= new BigDecimal(0);
							...
							helperFrom.setTotalLonable((totalLonableAset.subtract(totalLonableLiabilit)).toString());
