 ...
 */
public class ListNonBorrowerDocsCommand extends AbstractCommand implements ICommonEventConstant {
	/**
	 * Default Constructor
	 */
	public ListNonBorrowerDocsCommand() {
	}
	...
	public HashMap doExecute(HashMap map) throws CommandProcessingException, CommandValidationException {
		...
		try {
			...
			if (customer.getNonBorrowerInd()) // if non-borrower
			{
				...
				DefaultLogger.debug(this, "customerID=" + customerID);

				DocumentHeldSearchCriteria criteria = new DocumentHeldSearchCriteria();
				criteria.setSubProfileID(customerID);
				criteria.setSubCategory(ICMSConstant.CHECKLIST_NON_BORROWER);

				ILimitProfile limit = (ILimitProfile) map.get(IGlobalConstant.GLOBAL_LIMITPROFILE_OBJ);
				if (limit != null) {
					criteria.setLimitProfileID(limit.getLimitProfileID());
					DefaultLogger.debug(this, "<<< limit profile ID: " + limit.getLimitProfileID());
				}

				// HashMap docsHeldMap =
				// proxy.getCCDocumentsHeldForNonBorrower(customerID);
