...
public class EditImageTagSecReceiptCommand extends AbstractCommand {
...
//		}
		...
		System.out.println("In EditImageTagSecreceiptReceiptCommand = statusOfSubmit=>"+statusOfSubmit+" statusOfSubmitTostate=>"+statusOfSubmitTostate+" statusOfSubmitFromState=>"+statusOfSubmitFromState);
...
//			if(item.getSecImageTagUntagStatus() != null && !"".equals(item.getSecImageTagUntagStatus())) {
			if(item.getItemStatus() != null && !"".equals(item.getItemStatus()) && ("UPDATE_RECEIVED".equals(item.getItemStatus()) || "PENDING_RECEIVED".equals(item.getItemStatus())) 
					&& item.getSecImageTagUntagStatus() != null && !"".equals(item.getSecImageTagUntagStatus())) {
				secImageTagNames = item.getSecImageTagUntagImgName(); 
				checklistItemRefNumber = String.valueOf(item.getCheckListItemRef());
//				docDesc = item.getSecImageTagUntagId();
				docDesc = (String)item.getItem().getItemDesc();
				System.out.println("EditImageTagSecreceiptCommand => item.getItem().getItemDesc() =>docDesc=>"+docDesc);
		...
		if(trxds1.equals(trxhistId)) {
			...
			System.out.println("Editimagetagsecreceiptcmd = statusOfTrxImage=>"+statusOfTrxImage+" instance=>"+instance+" trxIdofimagetag=>"+trxhistId);
		}else {
		...
		if(!"".equals(checklistItemRefNumber)) {
			...
			System.out.println("EditImageTagSecReceiptCommand = checkList.getCheckListID()=>"+checklistId+"   getCheckListIds(docDesc,checklistItemRefNumber)=>"+checklist_id);
...
//		IImageTagTrxValue trxValueIn = (OBImageTagTrxValue) map.get("IImageTagTrxValue");
		...
		if(flag == true && secImageTagNames != null && !"".equals(secImageTagNames)) {
