...
import com.integrosys.cms.ui.checklist.otherreceipt.OtherReceiptForm;
 ...
 */
public class SecurityReceiptFormValidator {
	public static ActionErrors validateInput(SecurityReceiptForm aForm, Locale locale) {
		...
		if("save_receive".equals(event)){
			...
			if((aForm.getReceivedDate() != null) && (aForm.getReceivedDate().length() > 0)){
				...
				if(aForm.getIsCorporateParty() != null) {
					if("true".equals(aForm.getIsCorporateParty())) {
						if("".equals(aForm.getRocCharge())) {
							errors.add("rocCharge", new ActionMessage("error.string.mandatory"));
						}
					}
					
				}
				
//				if(aForm.getSecImageTagUntagId() == null || "".equals(aForm.getSecImageTagUntagId()) || aForm.getSecImageTagUntagImgName() == null || "".equals(aForm.getSecImageTagUntagImgName())
//						|| aForm.getSecImageTagUntagStatus() == null || "".equals(aForm.getSecImageTagUntagStatus())) {
//					errors.add("imagetaguntag", new ActionMessage("error.string.mandatory"));
//				}
				
//				if(aForm.getSecImageTagUntagImgName() == null || "".equals(aForm.getSecImageTagUntagImgName())) {
//					errors.add("imagetaguntag", new ActionMessage("error.string.mandatory"));
//				}
		...
		if("save_waiver".equals(event)){
			...
			if(aForm.getItemStatus().equals("DEFERRED")){
				if ((aForm.getWaivedDate() == null) || (aForm.getWaivedDate().length() <= 0)) {
					errors.add("waivedDate", new ActionMessage("error.date.mandatory"));
				}else if(d.before(DateUtil.clearTime(DateUtil.convertDate(locale, aForm.getWaivedDate())))) {
						errors.add("waivedDate", new ActionMessage("error.date.future"));
