 ...
 */
public class ApproveSecurityReceiptCommand extends AbstractCommand implements ICommonEventConstant {
	/**
	 * Default Constructor
	 */
	public ApproveSecurityReceiptCommand() {
	}
	...
	public HashMap doExecute(HashMap map) throws CommandProcessingException, CommandValidationException {
		...
		try {
			...
			long id=0 ;
			System.out.println("===============================limitProfileID inside Approve Security Receipt Cmd is : "+limitProfileID);
			...
			for (int j = 0; j < actCheckListItems.length; j++) {					
				String imStatus=actCheckListItems[j].getSecImageTagUntagImgName();
				if(imStatus != null){
			...
			List<Long> updatedStagingChecklistItems = new ArrayList<Long>();
			for(int i =0;i<list.size();i++){
				...
				if(compareResult.isAdded()||compareResult.isDeleted()||compareResult.isModified()){
					...
					updatedStagingChecklistItems.add(OB.getCheckListItemID());
				}
...
*/
			...
			String cifNumber = limitJdbc.getCifNumberByCmsLimitProfileId(checkListTrxVal.getLimitProfileID());
				...
				for (int j = 0; j < temp.length; j++) {					
					...
					if(ICMSConstant.NO.equals(temp[j].getRocCharge()) && updatedStagingChecklistItems.contains(temp[j].getCheckListItemID()) ) {
						CheckListHelper.processDiscrepencyTrx(temp[j], customer, ctx);
					}
			...
			if(isReceivedDoc){
				...
				String secInfo=(String)iladDao.getSecurityInfo(checkListTrxVal.getCheckList().getCheckListID());
				if(iladDao!=null){
...
//					String tCollateralID = "200701010000130";
		        	...
		        	iladItem=iladDao.createLADItem(iladItem);
		        		...
		        		for(int a=0;a<finalReceiveDoc.size();a++){
		        			...
		        			if(secInfo!=null && !"".equals(secInfo)){
		        			iladSubItem.setType(secInfo);
		        			}
					      ...
					      //  checkListItem.setDocDate(dateApplication);
							...
							ICheckListTrxValue checkListTrxVal2=null;
						    	...
						    	try {
									...
									throw new CommandProcessingException("Fail to create Stage LAD in Security", ex);
								}
							     ...
							     try{
							    	 checkListTrxValActualLad=proxy.checkerApproveCheckListReceipt(ctx, checkListTrxValStageLad);
										...
										throw new CommandProcessingException("Fail to create Actual LAD in Security", ex);
									}
		        		...
		        		ICaseCreationDao caseCreationDao1=(ICaseCreationDao)BeanHouse.get("caseCreationDao");
				        	for(int a=0;a<finalReceiveDoc.size();a++){
			        			...
			        			if(!chkMap.containsKey(String.valueOf(obj.getCheckListItemID()))){
				        			...
				        			/*String secListImages = obj.getSecImageTagUntagImgName();
				        			if(secListImages != null) {
				        			if(!"".equals(secListImages)) {
									 String[] secListImages1 = secListImages.split(",");

									 ArrayList secImageListAdd = new ArrayList();
									 ArrayList secImageStatusListAdd = new ArrayList();
									 
									 for(int i=0;i<secListImages1.length;i=i+3){
										secImageStatusListAdd.add(secListImages1[i+1].trim());
									 	secImageListAdd.add(secListImages1[i+2].trim());
									 }
									 if(!secImageListAdd.isEmpty()) {
									 for(int i=0;i<secImageListAdd.size();i++) {
									 if(obj.getSecImageTagUntagImgName() != null) {
					        				String imgId = (String) secImageListAdd.get(i);
					        				String imgStatus = (String) secImageStatusListAdd.get(i);
					        				String imageStatus = "";
				        			...
				        			/*if(obj.getSecImageTagUntagId() != null) {
				        				String imgId = obj.getSecImageTagUntagId();
				        				String imgStatus = obj.getSecImageTagUntagStatus();
				        				String imageStatus = "";
				        				if(imgStatus.equals("UnTagged")) {
				        					imageStatus = "Y";
				        				}else if(imgStatus.equals("Tagged")){
