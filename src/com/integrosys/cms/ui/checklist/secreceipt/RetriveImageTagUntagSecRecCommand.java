...
public class RetriveImageTagUntagSecRecCommand extends AbstractCommand {
	...
	public HashMap doExecute(HashMap map) throws CommandProcessingException,
		...
		System.out.println("RetriveImageTagUntagSecRecCommand.java =>doExecute ---pID=>" + pID + "   imageName=>" + imageName + "      status=>" + status);
		String imagePath="";
		...
		System.out.println("RetriveImageTagUntagSecRecCommand.java => flag=>"+flag);
		...
		for(int i=0;i<itList.length;i++)
			{
			if(!"".equals(itList[i].getSecImageTagUntagImgName()) && itList[i].getSecImageTagUntagImgName() != null){
				checklistId = String.valueOf(itList[i].getCheckListItemID());
				System.out.println("RetriveImageTagUntagSecRecCommand.java =>checklistId=>"+checklistId);
			}
			}
		if(category == null || "".equals(category) ){
			category =securityImg;
		}
		List imageInfoList = new ArrayList(); 
		System.out.println("RetriveImageTagUntagSecRecCommand.java =>imageId=>"+imageId+" imageName=>"+imageName);
		...
		if(!imageInfoList.isEmpty() && imageInfoList != null) {
			...
			System.out.println("RetriveImageTagUntagSecRecCommand ==> imageInfoList is not empty and null =>custId=>"+custId+" pID=>"+pID+" status=>"+status);
		}
		...
		IImageTagDetails existingTagDetails=tagDao.getCustImageListForView("actualOBImageTagDetails", custId, IImageTagConstants.SECURITY_DOC,checklistId);
		if(existingTagDetails!=null){
			if(tagDetailList == null || tagDetailList.isEmpty()) {
				System.out.println("RetriveImageTagUntagSecRecCommand => tagDetailList is null or empty");
...
//		OBImageUploadAdd img = (OBImageUploadAdd) tagDetailList.get(Integer.parseInt(count));
		...
		System.out.println("RetriveImageTagUntagSecRecCommand.java url............"+url);
		...
		String unCheckId =(String) map.get("unCheckId");
					...
					System.out.println("RetriveImageTagUntagSecRecCommand =>ImageTagResultCommand====================193========================>"+imageId);
				if(imageId!=null && !imageId.equals("")){
					...
					DefaultLogger.debug(this, "RetriveImageTagUntagSecRecCommand => ImageTagResultCommand====================214========================>"+unCheckId);
		...
		DefaultLogger.debug(this,"doExecute ---" + pID + "    " + imageName + "      " + status);
		System.out.println("RetriveImageTagUntagSecRecCommand => doExecute ---pID=>" + pID + "    imageName=>" + imageName + "      status=>" + status);
		...
		if(imagePath!=null){
			...
			//	com.lowagie.text.Image.getInstance(imagePath);
				...
				System.out.println("RetriveImageTagUntagSecRecCommand.java => Done");
			} catch (Exception e) {
				e.printStackTrace();
			}
		}
			...
			try{
				...
				System.out.println("RetriveImageTagUntagSecRecCommand => inside else imagename");
				String basePath=PropertyManager.getValue("contextPath");
				...
				System.out.println("RetriveImageTagUntagSecRecCommand => inside else pdfFilePath:"+pdfFilePath);
				...
				output.write(fileData);
				DefaultLogger.debug(this,"done");
				System.out.println("RetriveImageTagUntagSecRecCommand => done");
			} catch (Exception e) {
				e.printStackTrace();
			}
	...
	private List getImageInfoList(String imgId, String imgName) {
		...
		System.out.println("RetriveImageTagUntagSecRecCommand.java =>getImageInfoList() sql =>"+sql);
		DBUtil dbUtil=null;
		try {
		dbUtil=new DBUtil();
		dbUtil.setSQL(sql);
		...
		if(null!=rs){
			while(rs.next()){
				...
				System.out.println("RetriveImageTagUntagSecRecCommand.java =>getImageInfoList()  data=>"+data+" data1=>"+data1+" data2=>"+data2);
			}
		}
		rs.close();
		dbUtil.close();
