...
public class ViewImageCommand extends AbstractCommand implements ICommonEventConstant {
	...
	public HashMap doExecute(HashMap map) throws CommandProcessingException, CommandValidationException {
		...
		try {
			...
			String event1 =(String) map.get("event");
			ICheckListItem temp[] = checkList.getCheckListItemList();
			//SecurityReceiptForm securityReceiptForm = (SecurityReceiptForm) map.get("SecurityReceiptForm");
			
			String id = String.valueOf(temp[index].getCheckListItemID());
			System.out.println("ViewImageCommand Event is =>"+event1);
			if("view_image_page_to_do".equals(event1)) {
			String checkId = getCheckListIdsforImageView(String.valueOf(temp[index].getCheckListItemRef()));
	...
	private String getCheckListIdsforImageView(String checklistIdRefNumber) {
		String docdesc = "";
		String data="";
		//TreeMap map= new TreeMap();
		//List imageDataList = new ArrayList();
		String sql="SELECT distinct DOC_ITEM_ID FROM cms_checklist_item where  DOC_ITEM_REF = '"+checklistIdRefNumber+"' ";
		System.out.println("viewimagecommand of security getCheckListIdsforImageView(String checklistIdRefNumber) query =>"+sql);
