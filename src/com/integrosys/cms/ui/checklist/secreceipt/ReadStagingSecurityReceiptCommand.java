 ...
 */
public class ReadStagingSecurityReceiptCommand extends AbstractCommand implements ICommonEventConstant {
	/**
	 * Default Constructor
	 */
	public ReadStagingSecurityReceiptCommand() {
	}
	...
	public HashMap doExecute(HashMap map) throws CommandProcessingException, CommandValidationException {
		...
		try {
			...
			if(iladDao!=null){
				long limitProfileID = checkList.getCheckListOwner().getLimitProfileID();
		...
		long limitProfileIDLong = Long.parseLong(limitProfileIDFromTable);	
			if(limitProfileIDLong == limitProfileID) {
				allowToProcess = false;
                        ...
                        System.out.println("========================================================Inside ReadStaging Security Receipt Command ");
			...
			System.out.println("value of allowToProcess is : "+allowToProcess);
                        System.out.println("inside  Read Staging Security Receipt Cmd checkFacilityReceiptPending ");
			...
			for(int ab=0;ab<checkList.getCheckListItemList().length;ab++){
				...
				if(null != checkList.getCheckListItemList()[ab]){
			...
			System.out.println("BeanComparator first============");
			sortFields.add(new BeanComparator("documentStatus"));
			System.out.println("BeanComparator second===========");
			sortFields.add(new BeanComparator("itemStatus"));
			System.out.println("BeanComparator third ===========");
			sortFields.add(new BeanComparator("item.itemDesc"));
			ComparatorChain multiSort = new ComparatorChain(sortFields);
			
			System.out.println("Starting for loop");
			for (int i = 0; i < theListToBeSorted.size(); i++) {
				System.out.println("List values with field no ==="+i+" and value ====="+theListToBeSorted.get(i));
			}
			...
			if ("close_checklist_item".equals(event)||"search_security_doc_checker_close".equals(event)) {
