 ...
 * @since $Date: 2005/09/07 10:58:06 $ Tag: $Name: $
 */
public class ApproveRecurrentReceiptCommand extends AbstractCommand {
	 ...
	 * Default Constructor
	 */
	public ApproveRecurrentReceiptCommand() {
	}
	...
	public HashMap doExecute(HashMap map) throws CommandProcessingException, CommandValidationException {
		...
		try {
			checkListTrxVal = getRecurrentProxyManager().checkerApproveCheckListReceipt(ctx, checkListTrxVal);
			// For DP
			IRecurrentCheckList checkList = checkListTrxVal.getCheckList();
			if(checkList!=null){
				IRecurrentCheckListItem[] checkListItemList = checkList.getCheckListItemList();
				if(checkListItemList!=null){
					for (int i = 0; i < checkListItemList.length; i++) {
						...
						if("DP".equals(iRecurrentCheckListItem.getDocType())&& 
								iRecurrentCheckListItem.getRecurrentCheckListSubItemList().length==1){
							getRecurrentProxyManager().createDueDateEntries(iRecurrentCheckListItem);
						}
			...
			//getRecurrentProxyManager().createDefaultDueDateEntries(checkListTrxVal.getCheckList());
		}
		catch (RecurrentException ex) {
			throw new CommandProcessingException("failed to approve checklist receipt workflow", ex);
		}
