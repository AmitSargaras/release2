 ...
 * @since $Date: 2005/09/07 11:06:41 $ Tag: $Name: $
 */
public class RejectRecurrentReceiptCommand extends AbstractCommand {
	 ...
	 * Default Constructor
	 */
	public RejectRecurrentReceiptCommand() {
	}
	...
	public HashMap doExecute(HashMap map) throws CommandProcessingException, CommandValidationException {
		...
		String remarks = (String) map.get("remarks");
		 if(remarks == null||remarks.trim().equals("")){
				exceptionMap.put("recurrentRemarksError", new ActionMessage("error.reject.remark"));
				ICMSCustomerTrxValue trxValueInValidate = null;
				
				resultMap.put("trxId", checkListTrxVal.getTransactionID());
				returnMap.put(ICommonEventConstant.COMMAND_EXCEPTION_MAP, exceptionMap);
				returnMap.put(ICommonEventConstant.COMMAND_RESULT_MAP, resultMap);
				return returnMap;
	}else{
		try {
			checkListTrxVal = getRecurrentProxyManager().checkerRejectCheckListReceipt(ctx, checkListTrxVal);
		}
		catch (RecurrentException ex) {
			throw new CommandProcessingException("failed to rejected checklist receipt workflow", ex);
		}
	}
