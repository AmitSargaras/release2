...
/**
 * Command class to edit the rejected custodian doc by maker..
 * @author $Author: jychong $<br>
 * @version $Revision: 1.8.12.1 $
 * @since $Date: 2006/12/14 12:22:26 $ Tag: $Name: DEV_20060126_B286V1 $
 */
public class EditRejectCustodianCmd extends AbstractCommand implements ICommonEventConstant {
	/**
	 * Default Constructor
	 */
	public EditRejectCustodianCmd() {
	}
	...
	public HashMap doExecute(HashMap map) throws CommandProcessingException, CommandValidationException {
		...
		try {
			...
			// lodgeCustodianTrxVal.getStagingCustodianDoc().getCheckListID()));
			theOBTrxContext.setTrxCountryOrigin(CheckListUtil.getCustodianTrxCountry(theOBTrxContext.getTeam(),
					theOBTrxContext.getLimitProfile(), theOBTrxContext.getCustomer(), lodgeCustodianTrxVal
							.getStagingCustodianDoc().getCheckListID()));
			ICustodianProxyManager proxy = CustodianProxyManagerFactory.getCustodianProxyManager();
			ICMSTrxValue trxValue = proxy.editCustodian(theOBTrxContext, lodgeCustodianTrxVal, custDoc);
