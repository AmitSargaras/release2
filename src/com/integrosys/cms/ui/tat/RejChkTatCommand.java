 ...
 * @since $Date: 2004/09/03 07:23:16 $ Tag: $Name: $
 */
public class RejChkTatCommand extends AbstractCommand implements ICommonEventConstant {
	/**
	 * Default Constructor
	 */
	public RejChkTatCommand() {
	}
	...
	public HashMap doExecute(HashMap map) throws CommandProcessingException, CommandValidationException {
		...
		try {
			...
			if (iLimitprofileTrxValue.getStatus().equals(ICMSConstant.STATE_PENDING_CREATE)) {
				ICMSTrxResult resultOb = limitproxy.checkerRejectCreateTAT(theOBTrxContext, iLimitprofileTrxValue);
				resultMap.put("request.ITrxResult", resultOb);
			}
			else {
				iLimitprofileTrxValue.setTransactionSubType("TAT");
				ICMSTrxResult resultOb = limitproxy.checkerRejectUpdateLimitProfile(theOBTrxContext,
