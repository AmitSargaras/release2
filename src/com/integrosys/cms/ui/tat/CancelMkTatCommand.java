 ...
 * @since $Date: 2004/09/03 07:20:05 $ Tag: $Name: $
 */
public class CancelMkTatCommand extends AbstractCommand implements ICommonEventConstant {
	/**
	 * Default Constructor
	 */
	public CancelMkTatCommand() {
	}
	...
	public HashMap doExecute(HashMap map) throws CommandProcessingException, CommandValidationException {
		...
		try {
			...
			if (iLimitProfileTrxValue.getFromState().equals(ICMSConstant.STATE_PENDING_CREATE)) {
				ICMSTrxResult resultOb = limitproxy.makerCancelCreateTAT(theOBTrxContext, iLimitProfileTrxValue);
				resultMap.put("request.ITrxResult", resultOb);
			}
			else {
				iLimitProfileTrxValue.setTransactionSubType("TAT");
				ICMSTrxResult resultOb = limitproxy.makerCancelUpdateLimitProfile(theOBTrxContext,
