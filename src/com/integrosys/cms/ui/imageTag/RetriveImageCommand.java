	...
	CommandValidationException {
		...
		if(event!=null && event.equals("retrieveImageGallaryMakerTodo"))
		{
			if (searchResult != null) {
				tageedImageList = (List)searchResult.getResultList();
			}
		}
		else{
		 tageedImageList =(List) map.get("tageedImageList");
		}
		List imageList =(List) searchResult.getResultList();
			...
			if(imageTagDetails!=null){
				...
				for(int k=0;k<selected.length;k++){
				 ...
				 uploadImage = (OBImageUploadAdd)obImageTagAddList.get(Integer.parseInt(selected[k])-1);
					...
					if(!selectedArrayMap.containsKey(String.valueOf(uploadImage.getImgId()))){
		...
		if(imagePath!=null){
			...
			//	document.addHeader("ImageName : ",imageName);
			...
			//    com.lowagie.text.Image.getInstance(imagePath);
				...
				{
					 /* float actualDpi = image.getDpiX();
					     if (actualDpi > 0)
					     {
					     image.scalePercent(24f / actualDpi * 100);
					     }*/
