 ...
 * $Header: /home/cms2/cvsroot/cms2/src/com/integrosys/cms/ui/collateral/assetbased/AddChargeCommand.java,v 1.19 2006/09/15 08:32:03 hshii Exp $
 */
 ...
 * @since $Date: 2006/09/15 08:32:03 $ Tag: $Name: $
 */
...
public class AddChargeCommand extends AbstractCommand {
	...
	public HashMap doExecute(HashMap map) throws CommandProcessingException, CommandValidationException {
		...
		ILimitCharge iAssetLimit = (ILimitCharge) map.get("form.chargeObject");

		SecuritySubTypeUtil.validateChargeType(iAssetLimit.getChargeType(), iAssetLimit.getLimitMaps(), "chargeType",
				exceptionMap);

		ICollateralTrxValue itrxValue = (ICollateralTrxValue) map.get("serviceColObj");

		IAssetBasedCollateral iAsset = (IAssetBasedCollateral) itrxValue.getStagingCollateral();
		if (iAssetLimit != null) {

			if (iAssetLimit.getSecurityRank() == 0) {
				exceptionMap.put("rank", new ActionMessage("error.collateral.charge.rank"));
			}
			else if (iAssetLimit.getSecurityRank() == 1) {
				if ((iAssetLimit.getPriorChargeAmount() != null)
						&& (iAssetLimit.getPriorChargeAmount().getAmount() >= 0)) {
					exceptionMap.put("priorChargeAmount", new ActionMessage(
							"error.collateral.1st.charge.priorchargeamt"));
				}

				if ((iAssetLimit.getPriorChargeChargee() != null) && (iAssetLimit.getPriorChargeChargee().length() > 0)) {
				...
				if (StringUtils.isEmpty(iAssetLimit.getPriorChargeChargee())) {
					exceptionMap.put("chargeePriorCharge", new ActionMessage("error.string.mandatory"));
				}

				if (StringUtils.isEmpty(iAssetLimit.getPriorChargeType())) {
					exceptionMap.put("priorChargeType", new ActionMessage("error.string.mandatory"));
				}
			}

			if ((exceptionMap.get("rank") == null) && (iAssetLimit.getSecurityRank() != 0)) {
				ILimitCharge[] charge = iAsset.getLimitCharges();
				boolean isSameRank = false;
				for (int i = 0; (charge != null) && (i < charge.length) && !isSameRank; i++) {
					if (charge[i] != null) {
						if (charge[i].getSecurityRank() == iAssetLimit.getSecurityRank()) {
							isSameRank = true;
						}
					}
				}
			...
			if (exceptionMap.get("limitID") == null) {
				ICollateralLimitMap[] limitSecMap = iAssetLimit.getLimitMaps();
		...
		if (exceptionMap.size() == 0) {
			addCharge(iAsset, iAssetLimit);
			itrxValue.setStagingCollateral(iAsset);
			result.put("serviceColObj", itrxValue);
			result.put("subtype", map.get("subtype"));

			DefaultLogger.debug(this, "iAssetLimit.getLimitMaps().length: " + iAssetLimit.getLimitMaps().length);
		}
	...
	public static void addCharge(IAssetBasedCollateral iAsset, ILimitCharge iAssetLimit) {
		ILimitCharge[] existingArray = iAsset.getLimitCharges();
		int arrayLength = 0;
		...
		newArray[arrayLength] = iAssetLimit;
		...
		iAsset.setLimitCharges(newArray);
	}
}
