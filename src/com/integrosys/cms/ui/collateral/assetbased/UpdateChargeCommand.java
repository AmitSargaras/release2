 ...
 * $Header: /home/cms2/cvsroot/cms2/src/com/integrosys/cms/ui/collateral/assetbased/UpdateChargeCommand.java,v 1.22 2006/09/15 08:32:03 hshii Exp $
 */
...
public class UpdateChargeCommand extends AbstractCommand {
	...
	public HashMap doExecute(HashMap map) throws CommandProcessingException, CommandValidationException {
		...
		IAssetBasedCollateral iAssetCollateral = (IAssetBasedCollateral) itrxValue.getStagingCollateral();
		...
		if (iLimit.getSecurityRank() == 0) {
			...
			if ((iLimit.getPriorChargeAmount() != null) && (iLimit.getPriorChargeAmount().getAmount() >= 0)) {
				exceptionMap.put("priorChargeAmount", new ActionMessage("error.collateral.1st.charge.priorchargeamt"));
			}
		...
		if ((exceptionMap.get("rank") == null) && (iLimit.getSecurityRank() != 0)) {
			ILimitCharge[] charge = iAssetCollateral.getLimitCharges();
		...
		if (exceptionMap.size() == 0) {
			ILimitCharge limitArray[] = iAssetCollateral.getLimitCharges();
			...
			iAssetCollateral.setLimitCharges(limitArray);
			itrxValue.setStagingCollateral(iAssetCollateral);
