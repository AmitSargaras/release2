...
public class ViewchequeCmd extends AbstractCommand {
	...
	public HashMap doExecute(HashMap map) throws CommandProcessingException, CommandValidationException {
		...
		IPostDatedCheque chequeArrayobj[] =null;
	...
	//	IPostDatedCheque cheque = (IPostDatedCheque) map.get("form.chequeObject");
		...
		if("return_from_view".equals(event))
		{
			iAsset.setPostDatedCheques(chequeArray);
		
		}else if("delete_cheque_detail".equals(event)){
		/*//	ArrayList obj=(ArrayList)Arrays.asList(chequeArray);
			ArrayList objVal=new ArrayList();
			for(int x=0;x<chequeArray.length;x++){
				objVal.add(x,chequeArray[x]);
				
			}
			objVal.remove(index);
			//String[] y = x.toArray(new String[0]);
			for(int i=1;i<=objVal.size();i++){
				chequeArrayobj[i-1] = (IPostDatedCheque)objVal.get(i-1);
			}
		//	 chequeArrayobj=objVal.toArray();
*/			 chequeArray[index]=null;
			 iAsset.setPostDatedCheques(chequeArray);
		}
