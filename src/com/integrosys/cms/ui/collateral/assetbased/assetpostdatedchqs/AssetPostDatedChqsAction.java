...
public class AssetPostDatedChqsAction extends AssetBasedAction {// CollateralAction
	...
	public ICommand[] getCommandChain(String event) {
		...
		if (EVENT_PREPARE_CREATE.equals(event)) {
			...
			objArray[1] = new PrepareAssetPostDatedChqsCommand();
		}
		else if (EVENT_PREPARE_UPDATE.equals(event) || EVENT_PROCESS_UPDATE.equals(event)) {
			objArray = new ICommand[2];
			
		
			objArray[0] = (ICommand) getNameCommandMap().get("ReadCollateralCommand");
			objArray[1] = (ICommand) getNameCommandMap().get("PrepareAssetPostDatedChqsCommand");
		}
		else if (EVENT_PREPARE_FORM.equals(event)) {
			objArray = new ICommand[1];
			objArray[0] = (ICommand) getNameCommandMap().get("PrepareAssetPostDatedChqsCommand");
		}
		else if (EVENT_UPDATE_RETURN.equals(event)) {
			objArray = new ICommand[2];
			objArray[0] = new PrepareAssetPostDatedChqsCommand();
			...
			objArray = new ICommand[2];
			objArray[0] = new DeleteChequeCommand();
			objArray[1] = new PrepareAssetPostDatedChqsCommand();
		}
	...
	public ActionErrors validateInput(ActionForm aForm, Locale locale) {
		return AssetPostDatedChqsValidator.validateInput((AssetPostDatedChqsForm) aForm, locale);
	}
}
