...
import static  com.integrosys.cms.ui.collateral.assetbased.assetgencharge.AssetGenChargeAction.EVENT_PREPARE_ADD_LEAD_BANK_STOCK;
import static  com.integrosys.cms.ui.collateral.assetbased.assetgencharge.AssetGenChargeAction.EVENT_PREPARE_EDIT_LEAD_BANK_STOCK;
...
public class SaveLeadBankStockCommand extends AbstractCommand implements CollateralConstant {
	...
	public HashMap<String, Map<String, ?>> doExecute(HashMap inputMap)
		...
		if(EVENT_PREPARE_ADD_LEAD_BANK_STOCK.equals(event))
	...
	private static void editStock(IGeneralChargeDetails sessionChargeDetails, ILeadBankStock updatedStock, String index) {
		if (updatedStock != null && StringUtils.isNotBlank(index)) {
			List<ILeadBankStock> sessionStockList = sessionChargeDetails.getLeadBankStock();
			if (sessionStockList != null) {
				sessionStockList.set(Integer.parseInt(index), updatedStock);
				sessionChargeDetails.setLeadBankStock(sessionStockList);
			}	
		}
	}
