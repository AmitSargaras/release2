 ...
 * $Header: /home/cms2/cvsroot/cms2/src/com/integrosys/cms/ui/collateral/assetbased/assetgencharge/drawingpower/DrawingPowerAction.java,v 1.4 2006/01/26 11:22:41 jychong Exp $
 */
 ...
 * @since $Date: 2006/01/26 11:22:41 $ Tag: $Name: $
 */

public class DrawingPowerAction extends AssetGenChargeAction {
	public ICommand[] getCommandChain(String event) {
		ICommand objArray[] = null;
		if (EVENT_PREPARE.equals(event)) {
			objArray = new ICommand[1];
			objArray[0] = new PrepareDrawingPowerCommand();
		}
		else if (EVENT_PREPARE_UPDATE.equals(event) || EVENT_PROCESS_UPDATE.equals(event)) {
			objArray = new ICommand[2];
			objArray[0] = new ReadDrawingPowerCommand();
			objArray[1] = new PrepareDrawingPowerCommand();
		}
			...
			objArray = new ICommand[2];
			objArray[0] = new RefreshDrawingPowerCommand();
			objArray[1] = new PrepareDrawingPowerCommand();
		}
		else if (EVENT_EDIT.equals(event)) {
			objArray = new ICommand[2];
			objArray[0] = new NavigationGenChargeCommand();
			objArray[1] = new EditDrawingPowerCommand();
		}
		else {
	...
	public ActionErrors validateInput(ActionForm aForm, Locale locale) {
		...
		return DrawingPowerValidator.validateInput((DrawingPowerForm) aForm, locale);
	}
	...
	protected String getErrorEvent(String event) {
		...
		DefaultLogger.debug(this, "<<<<<<<<<<< getErrorEvent: " + errorEvent);
		return errorEvent;
	}
	...
	public IPage getNextPage(String event, HashMap resultMap, HashMap exceptionMap) {
		...
		if (event.equals(EVENT_EDIT) && (resultMap.get("hasError") != null)) {
			event = EVENT_PREPARE;
		}
