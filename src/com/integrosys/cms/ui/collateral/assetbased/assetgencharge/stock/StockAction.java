 ...
 * $Header: /home/cms2/cvsroot/cms2/src/com/integrosys/cms/ui/collateral/assetbased/assetgencharge/stock/StockAction.java,v 1.2 2005/08/26 10:12:06 hshii Exp $
 */
 ...
 * @since $Date: 2005/08/26 10:12:06 $ Tag: $Name: $
 */

public class StockAction extends AssetGenChargeAction {
	public ICommand[] getCommandChain(String event) {
		ICommand objArray[] = null;
		if (EVENT_PREPARE_FORM.equals(event)) {
			objArray = new ICommand[1];
			objArray[0] = new PrepareStockCommand();
		}
		else if (EVENT_UPDATE_RETURN.equals(event)) {
			objArray = new ICommand[3];
			objArray[0] = new ReadStockCommand();
			objArray[1] = new PrepareStockCommand();
			...
			objArray[0] = new RefreshStockCommand();
			objArray[1] = new PrepareStockCommand();
		}
		else if (event.endsWith(EVENT_DELETE_ITEM)) {
			objArray = new ICommand[2];
			objArray[0] = new DeleteStockCommand();
			objArray[1] = new PrepareStockCommand();
		}
		else {
	...
	public ActionErrors validateInput(ActionForm aForm, Locale locale) {
		...
		return StockValidator.validateInput((StockForm) aForm, locale);
	}
	...
	private String getAddItemReference(String event) {
		if (event.startsWith(CollateralConstant.STOCK)) {
			return "add_" + CollateralConstant.STOCK;
		}
	...
	private String getUpdateItemReference(String event) {
		if (event.startsWith(CollateralConstant.STOCK)) {
			return "update_" + CollateralConstant.STOCK;
		}
		if (event.startsWith(CollateralConstant.INSURANCE_POLICY)) {
	...
	private String getViewItemReference(String event) {
		if (event.startsWith(CollateralConstant.STOCK)) {
			return "view_" + CollateralConstant.STOCK;
		}
		if (event.startsWith(CollateralConstant.INSURANCE_POLICY)) {
