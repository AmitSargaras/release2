 ...
 * $Header: /home/cms2/cvsroot/cms2/src/com/integrosys/cms/ui/collateral/assetbased/assetgencharge/stock/stockitem/StockItemAction.java,v 1.3 2005/08/26 10:12:17 hshii Exp $
 */
...
import com.integrosys.cms.ui.collateral.assetbased.assetgencharge.stock.StockAction;
 ...
 * @since $Date: 2005/08/26 10:12:17 $ Tag: $Name: $
 */
...
public class StockItemAction extends StockAction {
	...
	public ICommand[] getCommandChain(String event) {
		...
		if (EVENT_CREATE.equals(event)) {
			...
			objArray[0] = new CreateStockItemCommand();
		}
		else if (EVENT_PREPARE_UPDATE_SUB.equals(event) || EVENT_PREPARE.equals(event)) {
			objArray = new ICommand[2];
			objArray[0] = new ReadStockItemCommand();
			objArray[1] = new PrepareStockItemCommand();
		}
			...
			objArray = new ICommand[1];
			objArray[0] = new UpdateStockItemCommand();
		}
		else if (EVENT_READ_RETURN.equals(event)) {
			objArray = new ICommand[1];
			objArray[0] = new ReturnStockItemCommand();
		}
		else if (EVENT_PREPARE_FORM.equals(event)) {
			objArray = new ICommand[1];
			objArray[0] = new PrepareStockItemCommand();
		}
		else if (EVENT_REFRESH.equals(event)) {
			objArray = new ICommand[2];
			objArray[0] = new RefreshStockItemCommand();
			objArray[1] = new PrepareStockItemCommand();
		}
		else {
	...
	public ActionErrors validateInput(ActionForm aForm, Locale locale) {
		...
		return StockItemValidator.validateInput((StockItemForm) aForm, locale);
	}
	...
	public IPage getNextPage(String event, HashMap resultMap, HashMap exceptionMap) {
		...
		if (EVENT_READ_RETURN.equals(event)) {
			DefaultLogger.debug(this, "<<<<<<<<< from_page: " + resultMap.get("from_page"));
	...
	private String getReadReference(String from_event) {
		if (from_event.equals(EVENT_PREPARE_CLOSE) || from_event.equals(EVENT_READ) || from_event.equals(EVENT_TRACK)
			...
			return "view_StockItem";
		}
		if (from_event.equals(EVENT_PROCESS)) {
			return "process_StockItem";
		}
		return from_event;
	}
}
