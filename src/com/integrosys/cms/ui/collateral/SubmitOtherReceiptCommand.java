 ...
 */
public class SubmitOtherReceiptCommand extends AbstractCommand implements ICommonEventConstant {
	/**
	 * Default Constructor
	 */
	public SubmitOtherReceiptCommand() {
	}
					...
					ICheckListItem iChecklistItem=checkListItemList[j];
						if(null!=insuranceList){
							for(int i=0;i<insuranceList.size();i++){
								...
								if(actualObj.getInsuranceCode().equals(iChecklistItem.getInsuranceId())){
								if(ICMSConstant.STATE_ITEM_RECEIVED.equals(actualObj.getInsuranceStatus())){
									...
									iChecklistItem.setCurrency(actualObj.getInsuranceCurrency());
									iChecklistItem.setHdfcAmt(actualObj.getInsuredAmount());
									iChecklistItem.setDocAmt(actualObj.getInsurancePolicyAmt());
