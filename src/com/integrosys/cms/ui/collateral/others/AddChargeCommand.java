 ...
 * $Header: /home/cms2/cvsroot/cms2/src/com/integrosys/cms/ui/collateral/others/AddChargeCommand.java,v 1.6 2006/09/15 08:32:34 hshii Exp $
 */
 ...
 * @since $Date: 2006/09/15 08:32:34 $ Tag: $Name: $
 */
...
public class AddChargeCommand extends AbstractCommand {
	...
	public HashMap doExecute(HashMap map) throws CommandProcessingException, CommandValidationException {
		...
		ILimitCharge iOthLimit = (ILimitCharge) map.get("form.chargeObject");

		SecuritySubTypeUtil.validateChargeType(iOthLimit.getChargeType(), iOthLimit.getLimitMaps(), "chargeType",
				exceptionMap);

		ICollateralTrxValue itrxValue = (ICollateralTrxValue) map.get("serviceColObj");

		IOthersCollateral iOth = (IOthersCollateral) itrxValue.getStagingCollateral();
		if (iOthLimit != null) {
			if (iOthLimit.getSecurityRank() == 0) {
				exceptionMap.put("rank", new ActionMessage("error.collateral.charge.rank"));
			}
			else if (iOthLimit.getSecurityRank() == 1) {
				if ((iOthLimit.getPriorChargeAmount() != null) && (iOthLimit.getPriorChargeAmount().getAmount() >= 0)) {
					exceptionMap.put("priorChargeAmount", new ActionMessage(
							"error.collateral.1st.charge.priorchargeamt"));
				}

				if ((iOthLimit.getPriorChargeChargee() != null) && (iOthLimit.getPriorChargeChargee().length() > 0)) {
				...
				if (StringUtils.isEmpty(iOthLimit.getPriorChargeChargee())) {
					exceptionMap.put("chargeePriorCharge", new ActionMessage("error.string.mandatory"));
				}

				if (StringUtils.isEmpty(iOthLimit.getPriorChargeType())) {
					exceptionMap.put("priorChargeType", new ActionMessage("error.string.mandatory"));
				}
			}

			if ((exceptionMap.get("rank") == null) && (iOthLimit.getSecurityRank() != 0)) {
				ILimitCharge[] charge = iOth.getLimitCharges();
				boolean isSameRank = false;
				for (int i = 0; (charge != null) && (i < charge.length) && !isSameRank; i++) {
					if (charge[i] != null) {
						if (charge[i].getSecurityRank() == iOthLimit.getSecurityRank()) {
							isSameRank = true;
						}
					}
				}
			...
			if (exceptionMap.get("limitID") == null) {
				ICollateralLimitMap[] limitSecMap = iOthLimit.getLimitMaps();
		...
		if (exceptionMap.size() == 0) {
			addCharge(iOth, iOthLimit);

			itrxValue.setStagingCollateral(iOth);
			result.put("serviceColObj", itrxValue);

			result.put("subtype", map.get("subtype"));

			DefaultLogger.debug(this, "iOthLimit.getLimitMaps().length: " + iOthLimit.getLimitMaps().length);
		}
	...
	public static void addCharge(IOthersCollateral iOth, ILimitCharge iOthLimit) {
		ILimitCharge[] existingArray = iOth.getLimitCharges();
		int arrayLength = 0;
		...
		newArray[arrayLength] = iOthLimit;
