 ...
 * @since $Date: 2006/06/21 06:41:29 $
 * Tag: $Name:  $
 */
...
public class ChargeMapper extends AbstractCommonMapper {
	public Object mapFormToOB(CommonForm cForm, HashMap inputs) throws MapperException {
		...
		IOthersCollateral iOthers = (IOthersCollateral) (((ICollateralTrxValue) inputs.get("serviceColObj"))
				.getStagingCollateral());
		ILimitCharge[] obLimitCharge = iOthers.getLimitCharges();
		...
		try {
			...
			obToChange.setChargeCcyCode(iOthers.getCurrencyCode());
			obToChange.setPriorChargeCcyCode(iOthers.getCurrencyCode());

			ICollateralLimitMap[] colLimitMap = iOthers.getCollateralLimits();
			...
			/*
			 * if (aForm.getChargeAmount().equals("") &&
			 * obToChange.getChargeAmount() != null &&
			 * obToChange.getChargeAmount().getAmount() > 0) {
			 * obToChange.setChargeAmount
			 * (CurrencyManager.convertToAmount(locale,
			 * obToChange.getChargeCcyCode(), "0")); } else if
			 * (!AbstractCommonMapper.isEmptyOrNull(aForm.getChargeAmount())) {
			 * obToChange
			 * .setChargeAmount(CurrencyManager.convertToAmount(locale,
			...
			if (aForm.getRedemption().equals("Yes")) {
				 ...
				 * obToChange
				 * .setIsLEByJurisdiction(aForm.getIsLEJurisdiction());
				 ...
				 * obToChange.setLEDateByJurisdiction(null); } if
				 * (!AbstractCommonMapper
				 * .isEmptyOrNull(aForm.getDateLEGovernLaw())) { stageDate =
				 * CollateralMapper.compareDate(locale,
				 * obToChange.getLEDateByGovernLaws(),
				 * aForm.getDateLEGovernLaw());
	...
	public CommonForm mapOBToForm(CommonForm cForm, Object obj, HashMap inputs) throws MapperException {
		...
		try {
			IOthersCollateral iOthers = null;
			if ((inputs.get("from_event") != null) && inputs.get("from_event").equals("read")) {
				iOthers = (IOthersCollateral) trxValue.getCollateral();
			}
			else {
				iOthers = (IOthersCollateral) trxValue.getStagingCollateral();
			}
			...
			ICollateralLimitMap[] limit = iOthers.getCollateralLimits();
			...
			if (ICMSConstant.TRUE_VALUE.equals(iLimitCharge.getRedemption())) {
				 ...
				 * 
				 * aForm.setDateLEChageRank(DateUtil.formatDate(locale,
				 * iLimitCharge.getLEDateByChargeRanking()));
				 * aForm.setDateLEJurisdiction(DateUtil.formatDate(locale,
				 * iLimitCharge.getLEDateByJurisdiction()));
				 * aForm.setDateLEGovernLaw(DateUtil.formatDate(locale,
				 * iLimitCharge.getLEDateByGovernLaws()));
				 * 
				 * // added by zhanjia for R1.5 CR 235
				 * aForm.setIsLE(iLimitCharge.getIsLE());
				 * aForm.setDateLE(DateUtil.formatDate(locale,
				 * iLimitCharge.getLEDate()));
				 */
			}
