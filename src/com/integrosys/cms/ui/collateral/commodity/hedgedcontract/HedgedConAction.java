 ...
 * $Header: /home/cms2/cvsroot/cms2/src/com/integrosys/cms/ui/collateral/commodity/hedgedcontract/HedgedConAction.java,v 1.5 2004/08/04 05:48:28 hshii Exp $
 */
...
public class HedgedConAction extends CommodityMainAction {
	public ICommand[] getCommandChain(String event) {
		ICommand objArray[] = null;
		if (EVENT_CREATE.equals(event)) {
			objArray = new ICommand[1];
			objArray[0] = new CreateHedgedConCommand();
		}
		else if (EVENT_PREPARE.equals(event)) {
			objArray = new ICommand[1];
			objArray[0] = new PrepareHedgedConCommand();
		}
		else if (EVENT_PREPARE_UPDATE_SUB.equals(event)) {
			objArray = new ICommand[2];
			objArray[0] = new ReadHedgedConCommand();
			objArray[1] = new PrepareHedgedConCommand();
		}
			...
			objArray = new ICommand[1];
			objArray[0] = new UpdateHedgedConCommand();
		}
		else if (EVENT_READ_RETURN.equals(event)) {
			objArray = new ICommand[1];
			objArray[0] = new ReturnHedgedConCommand();
		}
	...
	public ActionErrors validateInput(ActionForm aForm, Locale locale) {
		...
		return HedgedConValidator.validateInput((HedgedConForm) aForm, locale);
	}
	...
	private String getReturnReference(String from_event) {
		...
		if (from_event.equals(EVENT_PROCESS_UPDATE) || from_event.equals(EVENT_PREPARE_UPDATE)) {
			return EVENT_UPDATE_RETURN;
		}
		return from_event;
	}
	...
	private String getReadReference(String from_event) {
		if (from_event.equals(EVENT_PREPARE_CLOSE) || from_event.equals(EVENT_TRACK) || from_event.equals(EVENT_READ)
			...
			return "view_hedgedContract";
		}
		if (from_event.equals(EVENT_PROCESS)) {
			return "process_hedgedContract";
		}
		return from_event;
	}
}
