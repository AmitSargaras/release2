 ...
 */
public class ReadInsurancePolicyCommand extends AbstractCommand {
	...
	public String[][] getResultDescriptor() {
		return (new String[][] { { "insurancePolicyObj", "java.lang.HashMap", FORM_SCOPE },
				{ "actualInsurance", "com.integrosys.cms.app.collateral.bus.IInsurancePolicy", REQUEST_SCOPE },
				{ "stageInsurance", "com.integrosys.cms.app.collateral.bus.IInsurancePolicy", REQUEST_SCOPE },
	...
	public HashMap doExecute(HashMap map) throws CommandProcessingException, CommandValidationException {
		...
		if ((from_event != null) && from_event.equals("read")) {
			...
			if ((from_event != null) && from_event.equals("process")) {
				IInsurancePolicy actualInsurance = null;
				if (itrxValue.getCollateral() != null)
					actualInsurance = getItem(((ICollateral) itrxValue.getCollateral()).getInsurancePolicies(),
							indexStr);
				IInsurancePolicy stageInsurance = getItem(iCol.getInsurancePolicies(), indexStr);

				result.put("actualInsurance", actualInsurance);
				result.put("stageInsurance", stageInsurance);
			}
		}
		IInsurancePolicy insurance;
		if (from_event != null) {
			insurance = getItem(iCol.getInsurancePolicies(), indexStr);
			if ((insurance == null) && from_event.equals("process")) {
				insurance = getItem(((ICollateral) itrxValue.getCollateral()).getInsurancePolicies(), indexStr);
				DefaultLogger.debug(this, insurance.getInsuranceClaimDate());
			}
		}
		else {
			int index = Integer.parseInt(indexStr);
			if ((int) index >= 0) {
				insurance = iCol.getInsurancePolicies()[index];
				DefaultLogger.debug("***" + this, insurance.getInsuranceClaimDate());
				DefaultLogger.debug("***REMARIK*" + this, insurance.getRemark1());
			}
			else {
				insurance = null;
			}
		...
		// DefaultLogger.debug(this,"InsuranceDoc:" +insurance.getDocumentNo());
		...
		*/
		...
		objMap.put("obj", insurance);
		objMap.put("col", iCol);

		result.put("insurancePolicyObj", objMap);
		...
		if(CollateralAction.MAKER_VIEW_INSURANCE_WAIVED.equals(event) ||
				CollateralAction.CHECKER_VIEW_INSURANCE_WAIVED.equals(event) || CollateralAction.MAKER_CREATE_INSURANCE_WAIVED.equals(event)
		||  CollateralAction.MAKER_SUBMIT_INS_WAIVED_ERROR.equals(event) || CollateralAction.MAKER_UPDATE_INSURANCE_WAIVED.equals(event)
		||CollateralAction.MAKER_UPDATE_INSWAIVED_LIST_ERROR.equals(event)){
		 ...
		 if(CollateralAction.MAKER_VIEW_INSURANCE_DEFERRED.equals(event) ||
				 CollateralAction.CHECKER_VIEW_INSURANCE_DEFERRED.equals(event)  || CollateralAction.MAKER_CREATE_INSURANCE_DEFERRED.equals(event)
				 || CollateralAction.MAKER_SUBMIT_INS_DEFERRED_ERROR.equals(event)  || CollateralAction.MAKER_UPDATE_INSURANCE_DEFERRED.equals(event)
	...
	private IInsurancePolicy getItem(IInsurancePolicy temp[], String itemRef) {
		IInsurancePolicy item = null;
		if (temp == null) {
			return item;
		}
