...
public class InsurancePolicyValidator {
	public static ActionErrors validateInput(InsurancePolicyForm aForm, Locale locale) {
		String errorCode = null;
		ActionErrors errors = new ActionErrors();
		String maximumAmt = new BigDecimal(IGlobalConstant.MAXIMUM_ALLOWED_AMOUNT_20_STR).toString();
		System.out.println("aForm.getEvent()======="+aForm.getEvent());
		boolean isMandatory = (aForm.getEvent().equals(CollateralAction.EVENT_CREATE) || aForm.getEvent().equals(
				CollateralAction.EVENT_UPDATE) || aForm.getEvent().equals("REST_UPDATE_INSURSANCE") || aForm.getEvent().equals("REST_ADD_INSURSANCE"));
...
//					}	
		...
		}else if("maker_submit_insurance_received".equals(aForm.getEvent()) || "maker_edit_insreceived_list".equals(aForm.getEvent()) || "maker_update_insreceived_list".equals(aForm.getEvent())){
...
//			
			...
			try {
				...
				DefaultLogger.error("InsurancePolicyValidator", "Exception throw at mapperUtil convert String to double");
			}
...
//				}
			...
			if ( null == aForm.getInsurancePremium() || "".equals(aForm.getInsurancePremium().trim())) {
				...
				errors.add("insurancePremium", new ActionMessage("error.string.mandatory"));
			...
			if (StringUtils.isNotBlank(aForm.getInsurancePremium())) {
				if (!(errorCode = Validator.checkAmount(aForm.getInsurancePremium(), false, 1,IGlobalConstant.MAXIMIUM_ALLOWED_AMOUNT, IGlobalConstant.DEFAULT_CURRENCY, locale))
						.equals(Validator.ERROR_NONE)) {
					errors.add("insurancePremium", new ActionMessage(ErrorKeyMapper.map(ErrorKeyMapper.AMOUNT, errorCode),
							"1", maximumAmt));
				}
			}
		...
		if (insClaimDate != null && insClaimDate.after(new Date())) {
			errors.add("insuranceClaimDate", new ActionMessage("error.date.compareDate.greater",
		...
		// errors.add("insuranceExchangeRate", new
	...
	public static ActionErrors validateAccSearch(InsurancePolicyForm aForm, Locale locale) {
		ActionErrors errors = new ActionErrors();
		if (StringUtils.isNotBlank(aForm.getDebitingACNo())) {
			if (aForm.getDebitingACNo().length() > 19) {
				errors.add("debitingACNo", new ActionMessage("error.integer.greaterthan", "0", StringUtils.repeat("9",
