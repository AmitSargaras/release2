 ...
 */
public class CashSameAction extends CashAction {
	...
	public ICommand[] getCommandChain(String event) {
		...
		if (EVENT_PREPARE_UPDATE.equals(event) || EVENT_PROCESS_UPDATE.equals(event)) {
			...
			objArray[1] = new PrepareCashSameCommand();
		}
		else if (EVENT_PREPARE_FORM.equals(event)) {
			objArray = new ICommand[1];
			objArray[0] = new PrepareCashSameCommand();
		}
	...
	public ActionErrors validateInput(ActionForm aForm, Locale locale) {
		return CashSameValidator.validateInput((CashSameForm) aForm, locale);
