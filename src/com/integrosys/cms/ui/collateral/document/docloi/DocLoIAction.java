...
public class DocLoIAction extends DocumentAction {
	...
	public ICommand[] getCommandChain(String event) {
		...
		if (EVENT_PREPARE_CREATE.equals(event)) {
			...
			objArray[1] = new PrepareDocLoICommand();
		}
			...
			objArray[2] = new PrepareDocLoICommand();
		}
		else if (EVENT_PREPARE_FORM.equals(event)) {
			objArray = new ICommand[1];
			objArray[0] = new PrepareDocLoICommand();
		}
	...
	public ActionErrors validateInput(ActionForm aForm, Locale locale) {
		return DocLoIValidator.validateInput((DocLoIForm) aForm, locale);
	}
