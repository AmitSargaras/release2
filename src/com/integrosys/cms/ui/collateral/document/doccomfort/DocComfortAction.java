 ...
 */
public class DocComfortAction extends DocumentAction {
	...
	public ICommand[] getCommandChain(String event) {
		...
		if (EVENT_PREPARE_CREATE.equals(event)) {
			...
			objArray[1] = new PrepareDocComfortCommand();
		}
			...
			objArray[2] = new PrepareDocComfortCommand();
		}
		else if (EVENT_PREPARE_FORM.equals(event)) {
			DefaultLogger.debug("DocComfortAction", "in prepare_form...");
			objArray = new ICommand[1];
			objArray[0] = new PrepareDocComfortCommand();
		}
	...
	public ActionErrors validateInput(ActionForm aForm, Locale locale) {
		return DocComfortValidator.validateInput((DocComfortForm) aForm, locale);
	}
}
