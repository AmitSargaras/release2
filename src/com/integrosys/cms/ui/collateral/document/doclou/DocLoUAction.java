 ...
 */
public class DocLoUAction extends DocumentAction {
	...
	public ICommand[] getCommandChain(String event) {
		...
		if (EVENT_PREPARE_CREATE.equals(event)) {
			...
			objArray[1] = new PrepareDocLoUCommand();
		}
			...
			objArray[2] = new PrepareDocLoUCommand();
		}
		else if (EVENT_PREPARE_FORM.equals(event)) {
			objArray = new ICommand[1];
			objArray[0] = new PrepareDocLoUCommand();
		}
	...
	public ActionErrors validateInput(ActionForm aForm, Locale locale) {
		return DocLoUValidator.validateInput((DocLoUForm) aForm, locale);
	}
}
