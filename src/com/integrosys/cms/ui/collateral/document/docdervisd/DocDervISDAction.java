 ...
 */
public class DocDervISDAction extends DocumentAction {
	...
	public ICommand[] getCommandChain(String event) {
		...
		if (EVENT_PREPARE_CREATE.equals(event)) {
			...
			objArray[1] = new PrepareDocDervISDCommand();
		}
			...
			objArray[2] = new PrepareDocDervISDCommand();
		}
		else if (EVENT_PREPARE_FORM.equals(event)) {
			objArray = new ICommand[1];
			objArray[0] = new PrepareDocDervISDCommand();
		}
	...
	public ActionErrors validateInput(ActionForm aForm, Locale locale) {
		return DocDervISDValidator.validateInput((DocDervISDForm) aForm, locale);
	}
}
