 ...
 */
public class DocDervNetAction extends DocumentAction {
	...
	public ICommand[] getCommandChain(String event) {
		...
		if (EVENT_PREPARE_CREATE.equals(event)) {
			...
			objArray[1] = new PrepareDocDervNetCommand();
		}
			...
			objArray[2] = new PrepareDocDervNetCommand();
		}
		else if (EVENT_PREPARE_FORM.equals(event)) {
			objArray = new ICommand[1];
			objArray[0] = new PrepareDocDervNetCommand();
		}
	...
	public ActionErrors validateInput(ActionForm aForm, Locale locale) {
		return DocDervNetValidator.validateInput((DocDervNetForm) aForm, locale);
	}
}
