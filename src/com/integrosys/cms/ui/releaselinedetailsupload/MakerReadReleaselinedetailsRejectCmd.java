...
public class MakerReadReleaselinedetailsRejectCmd extends AbstractCommand implements ICommonEventConstant{
	public MakerReadReleaselinedetailsRejectCmd(){
	...
	public HashMap doExecute(HashMap map) throws CommandProcessingException, CommandValidationException,FileUploadException {
		...
		try {
			...
			SearchResult releaselinedetailsFileList = new SearchResult();
			...
			trxValue = (OBFileUploadTrxValue) getReleaselinedetailsuploadProxy().getFielUploadByTrxID(fileId);
			IFileUploadJdbc jdbc=(IFileUploadJdbc)BeanHouse.get("fileUploadJdbc");
			releaselinedetailsFileList=jdbc.getAllReleaselinedetailsFile(trxValue.getStagingReferenceID());
			
			List uploadList=new ArrayList();
			if(releaselinedetailsFileList!=null){
				uploadList = new ArrayList(releaselinedetailsFileList.getResultList());		
			}
			long countPass = 0;
			long countFail = 0;
			
					for (int i = 0; i < uploadList.size(); i++) {
						OBReleaselinedetailsFile oBReleaselinedetailsFile = (OBReleaselinedetailsFile) uploadList.get(i);
	
						if ("FAIL".equals(oBReleaselinedetailsFile.getStatus()) && "N".equals(oBReleaselinedetailsFile.getUploadStatus())) {
							countFail++;
						} else if ("PASS".equals(oBReleaselinedetailsFile.getStatus()) && "Y".equals(oBReleaselinedetailsFile.getUploadStatus())) {
							countPass++;
						}
					}
			...
			resultMap.put("releaselinedetailsList", uploadList);
