...
public class GroupSubLimitValidator {
    ...
    public static ActionErrors validateInput(GroupSubLimitForm aForm, Locale locale) {
        ...
        try {
            if (!("approve".equals(event)) || "reject".equals(event)) {
                if ("create".equals(event) || "update".equals(event)) {
                    if (AbstractCommonMapper.isEmptyOrNull(aForm.getSubLimitTypeCD())) {
                        errors.add("subLimitTypeCD", new ActionMessage("error.string.mandatory"));
                        DefaultLogger.debug(" ERROR occured , subLimitTypeCD is  mandatory", "");
                    }
