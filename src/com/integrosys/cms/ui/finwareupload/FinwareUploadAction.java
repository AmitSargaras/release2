...
/**
 * @author $Author: Abhijeet Jadhav Action For UBS Upload
 */
public class FinwareUploadAction extends CommonAction{
	...
	public static final String FINWAREUPLOAD ="finwareupload";
	...
	public ICommand[] getCommandChain(String event) {
		...
		if (CSVUPLOAD.equals(event)) {
			objArray = new ICommand[1];
			objArray[0] = (ICommand) getNameCommandMap().get("FinwareUploadCmd");

		} else if (FINWAREUPLOAD.equals(event)) {
			objArray = new ICommand[1];
			objArray[0] = (ICommand) getNameCommandMap().get("FinwareFileUploadCmd");
	...
	private String getReference(String event) {
		...
		} else if ((event != null) && (FINWAREUPLOAD.equals(event)|| (EVENT_PAGINATION).equals(event))) {
			forwardName = "finwareupload";
		}
		return forwardName;
	}
