...
public class FetchCoBorrowerName_GCCmd extends AbstractCommand {
...
/*	public HashMap doExecute(HashMap map)
		...
		try {
			...
			String coBorroer_FCUBS_Webservice=PropertyManager.getValue("fcubs.facility.webservice.CoBorroewr_GC.url");
			...
			DefaultLogger.debug(this, "creating FCUBSFetchLiabilityGcApplicationServiceSpiServiceLocator");
			FCUBSFetchLiabilityGcApplicationServiceSpiServiceLocator coBorrowerFCUBSClient = new FCUBSFetchLiabilityGcApplicationServiceSpiServiceLocator();
			DefaultLogger.debug(this, "coBorrowerFCUBSClient:"+coBorrowerFCUBSClient);
			
			DefaultLogger.debug(this, "setting liability endpoint");
			coBorrowerFCUBSClient.setFCUBSFetchLiabilityGcApplicationServiceSpiPortEndpointAddress(coBorroer_FCUBS_Webservice);
			
			DefaultLogger.debug(this, "getting CoBorrower_FCUBS port");
			FCUBSFetchLiabilityGcApplicationServiceSpi coBorrower_FCUBS_ApplicationServiceSpiPort = coBorrowerFCUBSClient.getFCUBSFetchLiabilityGcApplicationServiceSpiPort();
		...
		//	sessionContext.setServiceCode(serviceCode);
			...
			FcubsFetchLiabilityGcRequestDTO coBorrower_FCUBS_IOFSRequest = new FcubsFetchLiabilityGcRequestDTO();
			FcubsheaderTypeGc fcubsheader = new FcubsheaderTypeGc();
			FcubsbodyGc fcubsbody = new FcubsbodyGc();

			FetchLiabQueryIOTypeGc liabIO = new FetchLiabQueryIOTypeGc();

			DefaultLogger.debug(this, "fcubsheader:" + fcubsheader + " fcubsbody:" + fcubsbody);
			UbscompTypeGc ubscomp2 = fcubsheader.getUbscomp();
			...
			FcubsFetchLiabilityGcResponseDTO response = coBorrower_FCUBS_ApplicationServiceSpiPort.fcubsFetchLiabilityGc(sessionContext,
			...
			DefaultLogger.debug(this, "DoQueryLiabilityIOResponseReturn:" + response);
			 ...
			 FCUBSFetchLiabilityGcApplicationServiceSpiPortBindingStub stub=(FCUBSFetchLiabilityGcApplicationServiceSpiPortBindingStub)coBorrower_FCUBS_ApplicationServiceSpiPort;
			...
			if (null != status) {
				...
				if (0 == status.getReplyCode()) {
					...
					if (null != response.getFcubsbody() && null != response.getFcubsbody().getLiabilityFull()) {
						FcubsbodyResGc fcubsbody2 = response.getFcubsbody();
						FetchLiabFullTypeGc liabilityFull = fcubsbody2.getLiabilityFull();
