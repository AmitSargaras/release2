...
public class AddGoodsRestrictionCommand extends AbstractCommand implements ILmtCovenantConstants {
	...
	if(!isParentCodeExists(list,goodsParentCode)) {
		OBLimitCovenant value = getLimitCovenant(goodsParentCode,null,null);
		list.add(value);
	}
	...
	Collections.sort(list, OBLimitCovenant.Comparators.GOODS_RESTRICTION_COMPARATOR);
	...
	private boolean isChildSubChildGoodsCodeExistsInList(List<OBLimitCovenant> list, String goodsCode, boolean isChild) {
	...
	private boolean isParentCodeExists(List<OBLimitCovenant> list, String goodsParentCode) {
	...
	private boolean isChildCodeExistsByParent(List<OBLimitCovenant> list, String goodsParentCode) {
		...
		if(StringUtils.isNotBlank(goodsParentCode)) {
			...
			if(isExists) {
				List<OBLimitCovenant> childGoodsCodes = getChildGoodsCodeByParent(list,parentComboCode);
	...
	private List<OBLimitCovenant> getChildGoodsCodeByParent(List<OBLimitCovenant> list, String goodsCodeParentCombo) {
		String targetParent = goodsCodeParentCombo!=null && goodsCodeParentCombo.split("\\|").length ==3 ?goodsCodeParentCombo.split("\\|")[0]:null;
		List<OBLimitCovenant> childList = new ArrayList<OBLimitCovenant>();
		for(OBLimitCovenant cov : list) {
	...
	private OBLimitCovenant getLimitCovenant(String goodsParentCode, String goodsChildCode, String goodsSubChildCode) {
