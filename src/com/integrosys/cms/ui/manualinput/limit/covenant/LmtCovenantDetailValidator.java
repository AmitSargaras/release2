...
public class LmtCovenantDetailValidator {

	public static ActionErrors validateInput(LmtCovenantDetailForm form, Locale locale) {
		...
		if(form.getDraweeRestriction()!=null && "Y".equalsIgnoreCase(form.getDraweeRestriction())) {
			...
			if(!form.getDraweeAmount().equals("") && !"".equals(form.getDraweeAmount())) {
		...
		if(form.getBeneRestriction()!=null && "Y".equalsIgnoreCase(form.getBeneRestriction())) {
			...
			if(!form.getBeneficiaryAmount().equals("") && !"".equals(form.getBeneficiaryAmount())) {
		...
		if(form.getCombinedTenorRestriction()!=null && "Y".equalsIgnoreCase(form.getCombinedTenorRestriction())) {
			...
			if(!form.getMaxCombinedTenor().equals("") && !"".equals(form.getMaxCombinedTenor())) {
		...
		if(form.getRunningAccountRestriction()!=null && "Y".equalsIgnoreCase(form.getRunningAccountRestriction())) {
			...
			if(!form.getCommitmentTenor().equals("") && !"".equals(form.getCommitmentTenor())) {
				if (!(errorCode = Validator.checkNumber(form.getCommitmentTenor(), true, 0, IGlobalConstant.MAXIMUM_ALLOWED_VALUE_10)).equals(Validator.ERROR_NONE)) {
					errors.add("commitmentTenor", new ActionMessage(ErrorKeyMapper.map(ErrorKeyMapper.NUMBER, errorCode), "0", IGlobalConstant.MAXIMUM_ALLOWED_VALUE_10_STR));
				}
			}
		...
		if(form.getCurrencyRestriction()!=null && "Y".equalsIgnoreCase(form.getCurrencyRestriction())) {
			...
			if(AbstractCommonMapper.isEmptyOrNull(form.getCurrencyRestrictionAmount())) {
				errors.add("currencyRestrictionAmount", new ActionMessage("error.string.mandatory"));
			}
			}
		}
		if(form.getBankRestriction()!=null && "Y".equalsIgnoreCase(form.getBankRestriction())) {
			...
			}if(AbstractCommonMapper.isEmptyOrNull(form.getBankRestrictionAmount())) {
				errors.add("bankRestrictionAmount", new ActionMessage("error.string.mandatory"));
			}
		}
		}
		...
		DefaultLogger.warn(LmtCovenantDetailValidator.class.getName() ,"  errors " + errors.size());
		return errors;
	}
}
