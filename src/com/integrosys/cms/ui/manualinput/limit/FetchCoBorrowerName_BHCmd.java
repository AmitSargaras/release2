...
public class FetchCoBorrowerName_BHCmd extends AbstractCommand {
	...
	/*public HashMap doExecute(HashMap map)
		...
		try {
			...
			String coBorroer_FCUBS_Webservice=PropertyManager.getValue("fcubs.facility.webservice.CoBorroewr_BH.url");
			...
			DefaultLogger.debug(this, "creating FCUBSFetchLiabilityBhApplicationServiceSpiServiceLocator");
			FCUBSFetchLiabilityBhApplicationServiceSpiServiceLocator coBorrowerFCUBSClient = new FCUBSFetchLiabilityBhApplicationServiceSpiServiceLocator();
			DefaultLogger.debug(this, "coBorrowerFCUBSClient:"+coBorrowerFCUBSClient);
			
			DefaultLogger.debug(this, "setting liability endpoint");
			coBorrowerFCUBSClient.setFCUBSFetchLiabilityBhApplicationServiceSpiPortEndpointAddress(coBorroer_FCUBS_Webservice);
			
			DefaultLogger.debug(this, "getting CoBorrower_FCUBS port");
			FCUBSFetchLiabilityBhApplicationServiceSpi coBorrower_FCUBS_ApplicationServiceSpiPort = coBorrowerFCUBSClient.getFCUBSFetchLiabilityBhApplicationServiceSpiPort();
		...
		//	sessionContext.setServiceCode(serviceCode);
			...
			FcubsFetchLiabilityBhRequestDTO coBorrower_FCUBS_IOFSRequest = new FcubsFetchLiabilityBhRequestDTO();
			FcubsheaderTypeBh fcubsheader = new FcubsheaderTypeBh();
			FcubsbodyBh fcubsbody = new FcubsbodyBh();

			FetchLiabQueryIOTypeBh liabIO = new FetchLiabQueryIOTypeBh();

			DefaultLogger.debug(this, "fcubsheader:" + fcubsheader + " fcubsbody:" + fcubsbody);
			UbscompTypeBh ubscomp2 = fcubsheader.getUbscomp();
			...
			FcubsFetchLiabilityBhResponseDTO response = coBorrower_FCUBS_ApplicationServiceSpiPort.fcubsFetchLiabilityBh(sessionContext,
			...
			DefaultLogger.debug(this, "DoQueryLiabilityIOResponseReturn:" + response);
			 ...
			 FCUBSFetchLiabilityBhApplicationServiceSpiPortBindingStub stub=(FCUBSFetchLiabilityBhApplicationServiceSpiPortBindingStub)coBorrower_FCUBS_ApplicationServiceSpiPort;
			...
			if (null != status) {
				...
				if (0 == status.getReplyCode()) {
					...
					if (null != response.getFcubsbody() && null != response.getFcubsbody().getLiabilityFull()) {
						FcubsbodyResBh fcubsbody2 = response.getFcubsbody();
						FetchLiabFullTypeBh liabilityFull = fcubsbody2.getLiabilityFull();
