...
public class FetchCoBorrowerName_FCUBSCmd extends AbstractCommand {
...
/*	public HashMap doExecute(HashMap map)
		...
		try {
			...
			String coBorroer_FCUBS_Webservice=PropertyManager.getValue("fcubs.facility.webservice.CoBorroewr_FCUBS.url");
			...
			DefaultLogger.debug(this, "creating FCUBSFetchLiabilityInApplicationServiceSpiServiceLocator");
			FCUBSFetchLiabilityInApplicationServiceSpiServiceLocator coBorrowerFCUBSClient = new FCUBSFetchLiabilityInApplicationServiceSpiServiceLocator();
			DefaultLogger.debug(this, "coBorrowerFCUBSClient:"+coBorrowerFCUBSClient);
			
			DefaultLogger.debug(this, "setting liability endpoint");
			coBorrowerFCUBSClient.setFCUBSFetchLiabilityInApplicationServiceSpiPortEndpointAddress(coBorroer_FCUBS_Webservice);
		...
		//	sessionContext.setServiceCode(serviceCode);
			...
			FcubsFetchLiabilityInRequestDTO coBorrower_FCUBS_IOFSRequest = new FcubsFetchLiabilityInRequestDTO();
			FcubsheaderTypeIn fcubsheader = new FcubsheaderTypeIn();
			FcubsbodyIn fcubsbody = new FcubsbodyIn();

			FetchLiabQueryIOTypeIn liabIO = new FetchLiabQueryIOTypeIn();

			DefaultLogger.debug(this, "fcubsheader:" + fcubsheader + " fcubsbody:" + fcubsbody);
			UbscompTypeIn ubscomp2 = fcubsheader.getUbscomp();
			...
			CoBorrower_WsLog.setRequestDateTime(Calendar.getInstance().getTime());
			 DefaultLogger.debug(this, "getting CoBorrower_FCUBS port");
				FCUBSFetchLiabilityInApplicationServiceSpi coBorrower_FCUBS_ApplicationServiceSpiPort = coBorrowerFCUBSClient.getFCUBSFetchLiabilityInApplicationServiceSpiPort();

			
			 DefaultLogger.debug(this,"casting port to stub");
			 FCUBSFetchLiabilityInApplicationServiceSpiPortBindingStub stub=(FCUBSFetchLiabilityInApplicationServiceSpiPortBindingStub)coBorrower_FCUBS_ApplicationServiceSpiPort;
			 ...
			 System.out.println("sss request ===================================:"+request);
				...
				FcubsFetchLiabilityInResponseDTO response = coBorrower_FCUBS_ApplicationServiceSpiPort.fcubsFetchLiabilityIn(sessionContext,
			...
			if (null != status) {
				...
				if (0 == status.getReplyCode()) {
					...
					if (null != response.getFcubsbody() && null != response.getFcubsbody().getLiabilityFull()) {
						FcubsbodyResIn fcubsbody2 = response.getFcubsbody();
						FetchLiabFullTypeIn liabilityFull = fcubsbody2.getLiabilityFull();
