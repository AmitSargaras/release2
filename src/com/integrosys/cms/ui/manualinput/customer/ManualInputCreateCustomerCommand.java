...
public class ManualInputCreateCustomerCommand extends AbstractCommand {
	...
	public HashMap doExecute(HashMap map) throws CommandProcessingException,
		...
		List lst = (List) map.get("facList");		
		ICriFac fac[] = new ICriFac[lst.size()];

		if (lst != null) {
			for (int i = 0; i < lst.size(); i++) {
				ICriFac facObj = new OBCriFac();
				facObj = (ICriFac) lst.get(i);
				fac[i] = facObj;
			...
			for (int i = 0; i < ifscBranchList.size(); i++) {
				...
				if (sysObj.getBankType()!= null && sysObj.getBankType().equals("O")) {
					...
					if (oflag) {
						...
						/*if (sysObj.getIfscCode().equals(bankId)
								&& (obCustomer.getBankingMethod().contains("CONSORTIUM-CONSORTIUM") || obCustomer.getBankingMethod().contains("OUTSIDECONSORTIUM-OUTSIDECONSORTIUM"))) {
							sysObj.setLead("Y");
						}*/
					}
					...
					if (oflagLead) {
						...
						/*if (sysObj.getIfscCode().equals(bankId)
								&& (obCustomer.getBankingMethod().contains("MULTIPLE-MULTIPLE") || obCustomer.getBankingMethod().contains("OUTSIDEMULTIPLE-OUTSIDEMULTIPLE"))) {
							sysObj.setNodal("Y");
						} */
		...
		} else	 if("SOLE".equals(obCustomer.getBankingMethod()) || "SOLE-SOLE".equals(obCustomer.getBankingMethod())){
				if (bankList != null) {
					for (int i = 0; i < bankList.size(); i++) {
					IBankingMethod sysObj = (IBankingMethod) bankList.get(i);
					sysObj.setLead("");
		...
		obCustomer.getCMSLegalEntity().setCriFacList(fac);
		 HashMap exceptionMap = new HashMap();
		/*if(obCustomer.getBankingMethod() != null && !"".equals(obCustomer.getBankingMethod()))
		{
			if(obCustomer.getBankingMethod().equals("MULTIPLE") || obCustomer.getBankingMethod().equals("CONSORTIUM"))
			{
				if(bankList != null && bankList.size()==0)
				{
					exceptionMap.put("bankingMethodEmptyError", new ActionMessage("error.string.bankingmethod.empty"));
					IPartyGroupTrxValue partyGroupTrxValue = null;
					resultMap.put("request.ITrxValue", partyGroupTrxValue);
					resultMap.put("event", "display_financial_details");
					returnMap.put(ICommonEventConstant.COMMAND_RESULT_MAP, resultMap);
					returnMap.put(ICommonEventConstant.COMMAND_EXCEPTION_MAP, exceptionMap);
					return returnMap;
				}
		...
		try {
			...
			{
					trx = CustomerProxyFactory.getProxy().createCustomerBrmaker(trxValue,loginUser);
			}else
			{
					trx = CustomerProxyFactory.getProxy().createCustomer(trxValue);
			}
