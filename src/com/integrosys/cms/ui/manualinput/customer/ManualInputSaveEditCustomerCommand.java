...
public class ManualInputSaveEditCustomerCommand extends AbstractCommand{
	...
	public HashMap doExecute(HashMap map) throws CommandProcessingException, CommandValidationException {		
		...
		if (name != null) {
			for (int i = 0; i < name.size(); i++) {
				IVendor Obj = new OBVendor();
				Obj = (IVendor) name.get(i);
				vendor[i] = Obj;
...
if ((bankList != null && !bankList.isEmpty()) && (newArray != null && newArray.length != 0)) {		
	...
	for (int i = 0; i < bankList.size(); i++) {
		...
		customer.getCMSLegalEntity().setCriFacList(criFacility);
		...
		try {
			trxValueOut = (ICMSTrxResult)CustomerProxyFactory.getProxy().makerEditDraftCustomer(ctx,trxValueIn,customer);
