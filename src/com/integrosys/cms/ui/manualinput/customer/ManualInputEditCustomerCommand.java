...
public class ManualInputEditCustomerCommand extends AbstractCommand{
...
/*	private ICustomerInfoProxyManager customerProxy;
	...
	public HashMap doExecute(HashMap map) throws CommandProcessingException, CommandValidationException {		
		...
		ICriFac criFacList[] = new ICriFac[facList.size()];
		if (facList != null) {
		...
		/*if (customer.getBankingMethod().equals("MULTIPLE")
				|| customer.getBankingMethod().equals("CONSORTIUM") || customer.getBankingMethod().equals("OUTSIDEMULTIPLE")
		...
		if (!"SOLE-SOLE".equals(customer.getBankingMethod())
			...
			IBankingMethod[] banking = customer.getCMSLegalEntity()
					.getBankList();
			...
			try {
				...
				/*if (bankList != null) {
					for (int i = 0; i < bankList.size(); i++) {
						IBankingMethod sysObj = (IBankingMethod) bankList
								.get(i);
						sysObj.setLead("");
						sysObj.setNodal("");
						if (sysObj.getBankType()!= null && sysObj.getBankType().equals("O")) {
		...
		else if(customer.getBankingMethod().equals("SOLE") || "SOLE-SOLE".equals(customer.getBankingMethod())){
			List bankList = (List) map.get("branchList");
			IBankingMethod bankingMethod[] = new IBankingMethod[bankList.size()];
			IBankingMethod[] banking = customer.getCMSLegalEntity()
					.getBankList();
		...
		customer.getCMSLegalEntity().setCriFacList(criFacList);
		
		String bankingMethodss = customer.getBankingMethod();
		
		customer.setBankingMethod("");
		customer.getCMSLegalEntity().setBankingMethod("");
		...
		String newCustomerName = (String) customer.getCustomerName();*/
			...
			if ( event.equals("edit_customer") ){
				oldCustomerName = trxValueIn.getCustomer().getCustomerName();
				validate = true;
			}
		...
		try {
			...
			BigDecimal memoExpo = new BigDecimal("0");
			if(trxValueIn != null){
				if(trxValueIn.getCustomer().getFundedShareLimit()!=null){
				funded = new BigDecimal(customer.getFundedShareLimit().trim()).subtract(new BigDecimal(trxValueIn.getCustomer().getFundedShareLimit()));
				}
				if(trxValueIn.getCustomer().getNonFundedShareLimit()!=null){
				nonFunded = new BigDecimal(customer.getNonFundedShareLimit().trim()).subtract(new BigDecimal(trxValueIn.getCustomer().getNonFundedShareLimit()));
				}
				if(trxValueIn.getCustomer().getMemoExposure()!=null){
				memoExpo = new BigDecimal(customer.getMemoExposure().trim()).subtract(new BigDecimal(trxValueIn.getCustomer().getMemoExposure()));
				}
				}
				
			customer.setFundedIncreDecre(funded.toString());
			customer.setNonFundedIncreDecre(nonFunded.toString());
			customer.setMemoExposIncreDecre(memoExpo.toString());
			...
			//Shiv 071212 ---------------------------------------
			trxValueOut = (ICMSTrxResult)CustomerProxyFactory.getProxy().makerUpdateCustomer(ctx,trxValueIn,customer);
