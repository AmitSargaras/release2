...
public class PrepareDeleteOtherCovenantCommand extends AbstractCommand implements ICommonEventConstant {
	...
	public HashMap doExecute(HashMap map) throws CommandProcessingException, CommandValidationException {
		...
		try {
			facilityNamesList = proxy.getFacilityNameByAAId(customerID);
		} catch (LimitException e) {
			e.printStackTrace();
		} catch (RemoteException e) {
			e.printStackTrace();
		}
		if(!facilityNamesList.isEmpty() || facilityNamesList!=null)
		{
		...
		resultMap.put("monitoringResponsibilityList1", getmonitoringResponsibilityList());
