...
import com.integrosys.cms.ui.otherdoc.OtherGlobalForm;

/**
 * Created by IntelliJ IDEA. User: elango Date: Jul 4, 2003 Time: 11:23:06 AM To
 * change this template use Options | File Templates.
 */
public class CAMGlobalFormValidator extends DocumentationGlobalFormValidator {
	public static ActionErrors validateInput(DocumentationGlobalForm aForm, Locale locale) {
		ActionErrors errors = new ActionErrors();
		String errorCode = "";
		String go="";
		CAMGlobalForm rForm=(CAMGlobalForm)aForm;
		if(aForm instanceof CAMGlobalForm ){
			go=rForm.getGo();
		}
		...
		DefaultLogger.debug(" Total Errors", "-------111111111111111111111111111111111-->" + errors.size());
			...
			if(go.equalsIgnoreCase("y")){
				if (isValidDocumentName(rForm.getCamDocDesc())) {
					errors.add("itemDescError", new ActionMessage("error.string.specialcharacter"));
				}
				if (isValidDocumentName(rForm.getCamDocCode())) {
					errors.add("itemCodeError", new ActionMessage("error.string.specialcharacter"));
				}
					if(!rForm.getCamDocTenureCount().equalsIgnoreCase("")){
						if (!(errorCode = Validator.checkNumber(rForm.getCamDocTenureCount(), false,0,
