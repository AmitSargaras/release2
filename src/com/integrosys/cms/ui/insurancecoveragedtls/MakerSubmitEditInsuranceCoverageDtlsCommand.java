 ...
 */
public class MakerSubmitEditInsuranceCoverageDtlsCommand extends AbstractCommand implements ICommonEventConstant {
	...
	public MakerSubmitEditInsuranceCoverageDtlsCommand() {
	}
	    ...
	    public HashMap doExecute(HashMap map) throws CommandProcessingException, CommandValidationException {
	        ...
	        try {
				...
				if(trxValueIn.getFromState().equals(ICMSConstant.STATE_PENDING_PERFECTION)){
					trxValueOut = getInsuranceCoverageDtlsProxyManager().makerUpdateCreateInsuranceCoverageDtls(ctx, trxValueIn, insuranceCoverageDtls);
				}else{
					if (event.equals("maker_submit_edit") || event.equals("maker_confirm_resubmit_update")) {
						trxValueOut = getInsuranceCoverageDtlsProxyManager().makerUpdateInsuranceCoverageDtls(ctx, trxValueIn, insuranceCoverageDtls);
					} else if (event.equals("maker_save_update_insurance_coverage_dtls")){
						trxValueOut = getInsuranceCoverageDtlsProxyManager().makerUpdateSaveInsuranceCoverageDtls(ctx, trxValueIn, insuranceCoverageDtls);
					}else {
