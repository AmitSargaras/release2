...
public class CheckerRejectFileUploadCommand extends AbstractCommand implements IBondDetailsUploadConstants{
	...
	public HashMap doExecute(HashMap map) throws CommandValidationException, CommandProcessingException, AccessDeniedException {
		...
		try {
			...
			List<OBBondDetailsFile> totalFileList =  (List)map.get(SESSION_TOTAL_UPLOADED_LIST);
			...
			IFileUploadTrxValue trxValueOut = getBondDetailsuploadProxy().checkerRejectFileUpload(ctx, trxValueIn);
			...
			for (int j = 0; j < totalFileList.size(); j += batchSize) {
					List<OBBondDetailsFile> batchList = totalFileList.subList(j,j + batchSize > totalFileList.size() ? totalFileList.size(): j + batchSize);
