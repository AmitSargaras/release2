...
public class DownloadReportCommand extends AbstractCommand implements IBondDetailsUploadConstants,IFileUploadConstants {
	...
	public HashMap doExecute(HashMap map) throws CommandValidationException, CommandProcessingException, AccessDeniedException {
		...
		try {
			...
			fileName = String.format("%s_%s%s", ICMSConstant.BOND_DETAILS_UPLOAD,sdf.format(new Date()),".xls");
			...
			List<OBBondDetailsFile> totalFailedList = (List<OBBondDetailsFile>) map.get(SESSION_TOTAL_FAILED_LIST);
			List<OBBondDetailsFile> totalList = new ArrayList<OBBondDetailsFile>();
			...
			if(totalList != null) {
				for(OBBondDetailsFile uploadedItem : totalList) {
					List<String> item = Arrays.asList(ifEmptyOrNullReturnDash(uploadedItem.getPartyId()), ifEmptyOrNullReturnDash(uploadedItem.getSourceSecurityId()),
							ifEmptyOrNullReturnDash(uploadedItem.getSecuritySubType()), ifEmptyOrNullReturnDash(uploadedItem.getBondCode()),
							ifEmptyOrNullReturnDash(uploadedItem.getNoOfUnits()), ifEmptyOrNullReturnDash(uploadedItem.getInterest()),
			...
			Map<String, Object> parameters = generateParamatersMap(ICMSConstant.BOND_DETAILS_UPLOAD);
	...
	private Map<String, Object> generateParamatersMap(String uploadFileType) {
		...
		columnLabels= new String[]{"Party ID", "Source Security ID", "Security Sub Type", "Bond Code", 
				"Number Of Units", "Interest","Status", "Reason"};
		columnWidths= new Integer[]{50, 50 ,50 ,50 ,50 ,50 ,50 ,50};
		columnType = new int[]{textFormat, textFormat, textFormat, textFormat,
				amountFormat, textFormat, textFormat, textFormat};
