...
/**
 * Describe this class. Purpose: for Maker to reedit the ISDA CSA Deal value
...
public class ISDACSADealMakerReadRejectedCmd extends AbstractCommand implements ICommonEventConstant {
	...
	public HashMap doExecute(HashMap map) throws CommandProcessingException, CommandValidationException {
		...
		try {
			...
			// Get Trx By TrxID
			IISDACSADealValTrxValue iSDADealValTrxVal = proxy.getISDACSADealValuationTrxValueByTrxID(trxContext, trxId);
			...
			if ((!iSDADealValTrxVal.getStatus().equals(ICMSConstant.STATE_ACTIVE))
					&& (!iSDADealValTrxVal.getStatus().equals(ICMSConstant.STATE_REJECTED))

			) {
				resultMap.put("wip", "wip");
				resultMap.put("InitialISDACSADeal", iSDADealValTrxVal.getStagingISDACSADealValuation());
			}
			else {
				resultMap.put("ISDACSADealTrxValue", iSDADealValTrxVal);
			}
			...
			resultMap.put("timefrequency.labels", CommonDataSingleton
					.getCodeCategoryLabels(ISDACSADealHelper.TIME_FREQUENCY_CODE));
			resultMap.put("timefrequency.values", CommonDataSingleton
					.getCodeCategoryValues(ISDACSADealHelper.TIME_FREQUENCY_CODE));
			resultMap.put("InitialISDACSADeal", obISDACSADealVal);

			// set FrequencyUnit List
			resultMap.put("timefrequencies.map", ISDACSADealHelper.buildTimeFrequencyMap());
