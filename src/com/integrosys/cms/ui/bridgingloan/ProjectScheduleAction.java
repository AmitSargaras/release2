 ...
 * @since $Date$ Tag: $Name$
 */
public class ProjectScheduleAction extends BridgingLoanAction {
	/**
	...
	public ICommand[] getCommandChain(String event) {
		...
		if (EVENT_PREPARE_FORM.equals(event) || EVENT_MAKER_PREPARE_CREATE.equals(event)) {
			...
			objArray[0] = new PrepareProjectScheduleCommand();
		}
			...
			objArray[0] = new PrepareProjectScheduleCommand();
			objArray[1] = new ReadProjectScheduleCommand();
		}
		else if (EVENT_VIEW.equals(event) || EVENT_MAKER_PREPARE_DELETE.equals(event)
				|| EVENT_CHECKER_VIEW.equals(event)) {
			objArray = new ICommand[1];
			objArray[0] = new ReadProjectScheduleCommand();
		}
		else if (EVENT_CREATE.equals(event)) {
			objArray = new ICommand[1];
			objArray[0] = new CreateProjectScheduleCommand();
		}
		else if (EVENT_UPDATE.equals(event)) {
			objArray = new ICommand[1];
			objArray[0] = new UpdateProjectScheduleCommand();
		}
		else if (EVENT_DELETE.equals(event)) {
			objArray = new ICommand[1];
			objArray[0] = new DeleteProjectScheduleCommand();
		}
	...
	public ActionErrors validateInput(ActionForm aForm, Locale locale) {
		...
		return ProjectScheduleValidator.validateInput((ProjectScheduleForm) aForm, locale);
	}
