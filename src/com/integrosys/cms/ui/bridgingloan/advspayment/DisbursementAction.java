 ...
 * @since $Date$ Tag: $Name$
 */
public class DisbursementAction extends BridgingLoanAction {

	public static final String EVENT_MAKER_PREPARE_SAVE_CREATE = "maker_prepare_save_create"; // Save
																								// and
																								// then
																								// create
	...
	public ICommand[] getCommandChain(String event) {
		...
		if (EVENT_PREPARE_FORM.equals(event) || EVENT_MAKER_PREPARE_CREATE.equals(event)) {
			...
			objArray[0] = new PrepareDisbursementCommand();
		}
			...
			objArray[0] = new PrepareDisbursementCommand();
			objArray[1] = new ReadDisbursementCommand();
		}
		else if (EVENT_VIEW.equals(event) || EVENT_MAKER_PREPARE_DELETE.equals(event)
				|| EVENT_CHECKER_VIEW.equals(event)) {
			objArray = new ICommand[1];
			objArray[0] = new ReadDisbursementCommand();
		}
		else if (EVENT_CREATE.equals(event) || EVENT_MAKER_PREPARE_SAVE_CREATE.equals(event)) {
			objArray = new ICommand[1];
			objArray[0] = new CreateDisbursementCommand();
		}
		else if (EVENT_UPDATE.equals(event)) {
			objArray = new ICommand[1];
			objArray[0] = new UpdateDisbursementCommand();
		}
		else if (EVENT_DELETE.equals(event)) {
			objArray = new ICommand[1];
			objArray[0] = new DeleteDisbursementCommand();
		}
	...
	public ActionErrors validateInput(ActionForm aForm, Locale locale) {
		...
		return DisbursementValidator.validateInput((DisbursementForm) aForm, locale);
	}

	protected boolean isValidationRequired(String event) {
		boolean result = false;
		if (EVENT_CREATE.equals(event) || EVENT_UPDATE.equals(event) || EVENT_MAKER_PREPARE_SAVE_CREATE.equals(event)) {
			result = true;
		}
		return result;
	}
