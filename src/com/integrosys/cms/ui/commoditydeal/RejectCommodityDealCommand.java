 ...
 * $Header: /home/cms2/cvsroot/cms2/src/com/integrosys/cms/ui/commoditydeal/RejectCommodityDealCommand.java,v 1.10 2004/08/23 07:58:00 hshii Exp $
 */
...
public class RejectCommodityDealCommand extends AbstractCommand {
	...
	public HashMap doExecute(HashMap map) throws CommandProcessingException, CommandValidationException {
		...
		try {
			if (trxValue.getStatus().equals(ICMSConstant.STATE_PENDING_CREATE)
				...
				trxValue = proxy.checkerRejectCommodityDeal(ctx, trxValue);
			}
			else if (((trxValue.getTeamTypeID() == ICMSConstant.TEAM_TYPE_CMT) && (trxValue.getStatus().equals(
					ICMSConstant.STATE_PENDING_CREATE_VERIFY)
					|| ICMSConstant.STATE_PENDING_CLOSE_VERIFY.equals(trxValue.getStatus()) || ICMSConstant.STATE_PENDING_UPDATE_VERIFY
				...
				trxValue = proxy.FAMRejectCommodityDeal(ctx, trxValue);
			}
			else {
				trxValue.setToUserId(ICMSConstant.LONG_INVALID_VALUE);
				trxValue.setToAuthGId(ICMSConstant.LONG_INVALID_VALUE);
				trxValue.setToAuthGroupTypeId(ICMSConstant.TEAM_TYPE_FAM_USER);
				trxValue = proxy.officerRejectCommodityDeal(ctx, trxValue);
			}
