 ...
 * $Header: /home/cms2/cvsroot/cms2/src/com/integrosys/cms/ui/commoditydeal/finance/purchasesale/PurchaseSaleAction.java,v 1.6 2004/07/22 14:33:02 hshii Exp $
 */
 ...
 * @since $Date: 2004/07/22 14:33:02 $ Tag: $Name: $
 */

public class PurchaseSaleAction extends FinanceAction {
	public ICommand[] getCommandChain(String event) {
		ICommand objArray[] = null;
		if (EVENT_PREPARE.equals(event)) {
			objArray = new ICommand[1];
			objArray[0] = new PreparePurchaseSaleCommand();
		}
		else if (EVENT_PREPARE_UPDATE_SUB.equals(event)) {
			objArray = new ICommand[2];
			objArray[0] = new ReadPurchaseSaleCommand();
			objArray[1] = new PreparePurchaseSaleCommand();
		}
		else if (EVENT_REFRESH.equals(event)) {
			objArray = new ICommand[2];
			objArray[0] = new RefreshPurchaseSaleCommand();
			objArray[1] = new PreparePurchaseSaleCommand();
		}
			...
			objArray = new ICommand[1];
			objArray[0] = new UpdatePurchaseSaleCommand();
		}
		else if (EVENT_READ_RETURN.equals(event)) {
			objArray = new ICommand[1];
			objArray[0] = new ReturnPurchaseSaleCommand();
		}
		else {
	...
	public ActionErrors validateInput(ActionForm aForm, Locale locale) {
		...
		return PurchaseSaleValidator.validateInput((PurchaseSaleForm) aForm, locale);
	}
	...
	private String getReadReference(String from_event, String previous_event) {
		...
		if (from_event.equals(EVENT_PREPARE_CLOSE) || from_event.equals(EVENT_READ) || from_event.equals(EVENT_TRACK)
				|| from_event.equals(EVENT_CLOSE_DEAL)) {
			return "view_purchase_sale";
		}
		if (from_event.equals(EVENT_PROCESS)) {
			return "process_purchase_sale";
		}
		return from_event;
	}
