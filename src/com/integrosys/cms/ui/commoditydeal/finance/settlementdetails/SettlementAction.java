 ...
 * $Header: /home/cms2/cvsroot/cms2/src/com/integrosys/cms/ui/commoditydeal/finance/settlementdetails/SettlementAction.java,v 1.7 2004/09/07 08:34:09 hshii Exp $
 */
...
public class SettlementAction extends FinanceAction {
	...
	public ICommand[] getCommandChain(String event) {
		...
		if (EVENT_CREATE.equals(event)) {
			...
			objArray[0] = new CreateSettlementCommand();
		}
		else if (EVENT_PREPARE_UPDATE_SUB.equals(event) || EVENT_PREPARE.equals(event)) {
			objArray = new ICommand[2];
			objArray[0] = new ReadSettlementCommand();
			objArray[1] = new PrepareSettlementCommand();
		}
			...
			objArray = new ICommand[1];
			objArray[0] = new UpdateSettlementCommand();
		}
		else if (EVENT_READ_RETURN.equals(event)) {
			objArray = new ICommand[1];
			objArray[0] = new ReturnSettlementCommand();
		}
		else if (EVENT_PREPARE_NOOP.equals(event)) {
			objArray = new ICommand[1];
			objArray[0] = new PrepareSettlementCommand();
		}
		else {
	...
	public ActionErrors validateInput(ActionForm aForm, Locale locale) {
		...
		return SettlementValidator.validateInput((SettlementForm) aForm, locale);
	}
	...
	private String getReference(String event) {
		...
		if (event.equals(EVENT_PREPARE_UPDATE_SUB) || event.equals(EVENT_PREPARE_NOOP)) {
