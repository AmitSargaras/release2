...
public class MakerReadAutoupdationLmtsRejectCmd extends AbstractCommand implements ICommonEventConstant{
	public MakerReadAutoupdationLmtsRejectCmd(){
	...
	public HashMap doExecute(HashMap map) throws CommandProcessingException, CommandValidationException,FileUploadException {
		...
		try {
			...
			SearchResult autoupdationlmtsFileList = new SearchResult();
			...
			autoupdationlmtsFileList=jdbc.getAllAutoupdationLmtsFile(trxValue.getStagingReferenceID());
			
			List uploadList=new ArrayList();
			if(autoupdationlmtsFileList!=null){
				uploadList = new ArrayList(autoupdationlmtsFileList.getResultList());		
			}
			long countPass = 0;
			long countFail = 0;
			
					for (int i = 0; i < uploadList.size(); i++) {
						OBAutoupdationLmtsFile oBAutoupdationLmtsFile = (OBAutoupdationLmtsFile) uploadList.get(i);
	
						if ("Reject".equals(oBAutoupdationLmtsFile.getStatus()) && "N".equals(oBAutoupdationLmtsFile.getUploadStatus())) {
							countFail++;
						} else if ("Success".equals(oBAutoupdationLmtsFile.getStatus()) && "Y".equals(oBAutoupdationLmtsFile.getUploadStatus())) {
							countPass++;
						}
					}
