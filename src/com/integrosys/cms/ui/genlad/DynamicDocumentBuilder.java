...
public class DynamicDocumentBuilder implements Runnable {
	...
	public static class Builder {
		...
		public DynamicDocumentBuilder build() {
			return new DynamicDocumentBuilder(this);
		}

	}

	public DynamicDocumentBuilder(Builder b) {
	...
	public void run() {
		...
		try {
			...
			DefaultLogger.debug(this,
					" Main thread started ---------DynamicDocumentBuilder");
			latch = new CountDownLatch(resultList.size());
			ExecutorService executor = ThreadPoolSynchronizer.getInstance().getExecutorService(); // of 10 threads
			...
			for (Iterator iterator = resultList.iterator(); iterator.hasNext();) {
				...
				Runnable lad = new DynamicGenerateLad.Builder()
			...
			DefaultLogger.debug(this,"File directory deletion begin---------------");
			FileUtils.forceDelete(directory);
			filesListInDir.clear();

			DefaultLogger.debug(this,"File directory deletion end---------------");

			if (partialCounter.intValue() == resultList.size()) {
				
				partyLadDao.updateToFail(this.relationshipMgrId, this.partyId,this.dueMonth, this.dueYear, this.segment,reportGenerationDate);
				
			}else if (partialCounter.intValue() > 0 && partialCounter.intValue()<resultList.size()) {
				
				
				partyLadDao.updateToPartialSuccess(this.relationshipMgrId,this.partyId, this.dueMonth, this.dueYear,this.segment,this.dirName + ".zip", reportGenerationDate);
				
				
			} else  if(partialCounter.intValue()==0){
				
				partyLadDao.updateToCompletedAndFileName(this.relationshipMgrId, this.partyId, this.dueMonth,this.dueYear, this.segment, this.dirName + ".zip",reportGenerationDate);
			}
		} catch (Exception e) {
			e.printStackTrace();
