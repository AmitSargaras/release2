 ...
 * @since $Date: 2005/08/29 06:05:46 $ Tag: $Name: $
 */
public class ApproveGenerateSCCCommand extends AbstractCommand implements ICommonEventConstant {
	/**
	 * Default Constructor
	 */
	public ApproveGenerateSCCCommand() {
	}
	...
	public HashMap doExecute(HashMap map) throws CommandProcessingException, CommandValidationException {
		...
		try {
			...
			DefaultLogger.debug(this, "<<<<<<<<<<<<<<<<<<<<<<<<<<<< remarks: " + ctx.getRemarks());
			ISCCertificate cert = certTrxVal.getStagingSCCertificate();
			cert.setRemarks(ctx.getRemarks());
			certTrxVal.setStagingSCCertificate(cert);
			certTrxVal = proxy.checkerApproveGenerateSCC(ctx, certTrxVal);
