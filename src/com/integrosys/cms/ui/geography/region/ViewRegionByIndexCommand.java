...
public class ViewRegionByIndexCommand extends AbstractCommand {
	...
	public ViewRegionByIndexCommand() {
	}
	...
	public HashMap doExecute(HashMap map) throws CommandProcessingException,
		...
		DefaultLogger.debug(this, "============ ViewRegionCommandByIndex ()"
				+ regionId);
		try {
			IRegionTrxValue regionTrx = getRegionProxy()
					.getRegionById(regionId);
			IRegion region = regionTrx.getActualRegion();
			resultMap.put("regionObj", region);
			resultMap.put("checkerEvent", event);
			resultMap.put("startIndex", startIdx);
			resultMap.put("countryList", getCountryList(region.getCountryId()
					.getIdCountry()));
		...
		boolean status = false;	
		 status = limitDao.getCAMMigreted("CMS_REGION",regionId,"ID");
