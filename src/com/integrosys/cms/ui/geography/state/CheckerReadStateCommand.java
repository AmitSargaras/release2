...
public class CheckerReadStateCommand extends AbstractCommand implements
	...
	public CheckerReadStateCommand() {
	}
	...
	public HashMap doExecute(HashMap map) throws CommandProcessingException,
		...
		try {
			...
			String stateCode = (String) (map.get("TrxId"));
			String event = (String) map.get("event");
			IStateTrxValue trxValue = (OBStateTrxValue) getStateProxy()
					.getStateByTrxID(stateCode);
			IState state = (OBState) trxValue.getStagingState();

			long countryId = 0;
			long regionId = state.getRegionId().getCountryId().getIdCountry();

			if (!(event.equals("maker_resubmit_create_state") || event
					.equals("todo_maker_save_create_state")))
				countryId = state.getRegionId().getCountryId().getIdCountry();
			;
			...
			resultMap.put("regionList", getRegionList(regionId));
