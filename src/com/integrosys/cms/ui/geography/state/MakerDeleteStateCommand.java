...
public class MakerDeleteStateCommand extends AbstractCommand {
	...
	public MakerDeleteStateCommand() {
	}
	...
	public HashMap doExecute(HashMap map) throws CommandProcessingException,
		...
		try {
			...
			IState state = (OBState) map.get("stateObj");
			...
			if (event.equals("maker_delete_state")) {
				trxValueOut = getStateProxy().makerDeleteState(ctx, trxValueIn,
						state);
			} else if (event.equals("maker_activate_state")) {
				trxValueOut = getStateProxy().makerActivateState(ctx,
						trxValueIn, state);
			} else {
				// event is maker_confirm_resubmit_edit
				String remarks = (String) map.get("remarks");
				ctx.setRemarks(remarks);
				trxValueOut = getStateProxy().makerEditRejectedState(ctx,
						trxValueIn, state);
			}
