...
public class FacilityIslamicSecDepositAction extends FacilityMainAction {
	protected ICommand[] getCommandChain(String event) {
		if (EVENT_VIEW_EDIT.equals(event) || EVENT_VIEW_EDIT_WO_FRAME.equals(event) 
				|| TAB_ISLAMIC_SECURITY_DEPOSIT_CHECKER.equals(event) || TAB_ISLAMIC_SECURITY_DEPOSIT_VIEW_WO_FRAME.equals(event) 
				|| TAB_ISLAMIC_SECURITY_DEPOSIT_VIEW.equals(event)|| TAB_ISLAMIC_SECURITY_DEPOSIT_CHECKER_PROCESS.equals(event)) {
			return new ICommand[] { (ICommand) getNameCommandMap().get("ReadFacilityIslamicSecDepositCommand")};
		}
		else if (EVENT_NAVIGATE.equals(event) || EVENT_NAVIGATE_WO_FRAME.equals(event)) {
			return new ICommand[] { (ICommand) getNameCommandMap().get("SaveFacilityIslamicSecDepositToSessionCommand"),
	...
	protected IPage getNextPage(String event, HashMap map, HashMap exceptionMap) {
		...
		if (TAB_ISLAMIC_SECURITY_DEPOSIT_CHECKER.equals(event) ||
				TAB_ISLAMIC_SECURITY_DEPOSIT_VIEW_WO_FRAME.equals(event) ||
				TAB_ISLAMIC_SECURITY_DEPOSIT_VIEW.equals(event) ||
				TAB_ISLAMIC_SECURITY_DEPOSIT_CHECKER_PROCESS.equals(event)) {
			forward = event;
	...
	public ActionErrors validateInput(ActionForm aForm, Locale locale) {
		...
		return FacilityIslamicSecDepositFormValidator.validateInput((FacilityIslamicSecDepositForm) aForm, locale);
	}
}
