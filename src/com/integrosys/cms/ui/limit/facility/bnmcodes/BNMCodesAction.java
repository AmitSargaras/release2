...
public class BNMCodesAction extends FacilityMainAction {

	protected ICommand[] getCommandChain(String event) {
		if (EVENT_VIEW_EDIT.equals(event) || EVENT_VIEW_EDIT_WO_FRAME.equals(event)
				|| TAB_BNM_CODES_CHECKER.equals(event) || TAB_BNM_CODES_VIEW_WO_FRAME.equals(event)
				|| TAB_BNM_CODES_VIEW.equals(event)||TAB_BNM_CODES_CHECKER_PROCESS.equals(event)) {
			return new ICommand[] { (ICommand) getNameCommandMap().get("ReadBNMCodesCommand")};
		}
		else if (EVENT_NAVIGATE.equals(event) || EVENT_NAVIGATE_WO_FRAME.equals(event)) {
			return new ICommand[] { (ICommand) getNameCommandMap().get("SaveBNMCodesToSessionCommand"),
	...
	protected IPage getNextPage(String event, HashMap map, HashMap exceptionMap) {
		...
		else if (TAB_BNM_CODES_CHECKER.equals(event)) {
			forward = TAB_BNM_CODES_CHECKER;
		}
		else if (TAB_BNM_CODES_CHECKER_PROCESS.equals(event)) {
			forward = TAB_BNM_CODES_CHECKER_PROCESS;
		}
		else if (TAB_BNM_CODES_VIEW_WO_FRAME.equals(event)) {
			forward = TAB_BNM_CODES_VIEW_WO_FRAME;
		}
		else if (TAB_BNM_CODES_VIEW.equals(event)) {
	...
	public ActionErrors validateInput(ActionForm aForm, Locale locale) {
		...
		return BNMCodesFormValidator.validateInput((BNMCodesForm) aForm, locale);
	}

	protected boolean isValidationRequired(String event) {
		return (EVENT_NAVIGATE.equals(event) || EVENT_NAVIGATE_WO_FRAME.equals(event)
				|| EVENT_SAVE_WHOLE_OBJ.equals(event) || EVENT_SUBMIT.equals(event) || EVENT_SUBMIT_WO_FRAME
				.equals(event));
	}
