...
public class MultiTierFinAction extends FacilityMainAction {

	protected ICommand[] getCommandChain(String event) {
		if (EVENT_LIST.equals(event) || EVENT_LIST_WO_FRAME.equals(event) || TAB_MULTI_TIER_FINANCING_CHECKER.equals(event)
				|| TAB_MULTI_TIER_FINANCING_VIEW_WO_FRAME.equals(event) || TAB_MULTI_TIER_FINANCING_VIEW.equals(event)
				||TAB_MULTI_TIER_FINANCING_CHECKER_PROCESS.equals(event)) {
			return new ICommand[] { (ICommand) getNameCommandMap().get("ReadMultiTierFinListCommand"),
					(ICommand) getNameCommandMap().get("CompareMultiTierFinListCommand") };
		}
			...
			return new ICommand[] { (ICommand) getNameCommandMap().get("SaveMultiTierFinDetailToSessionCommand") };
		}
			...
			return new ICommand[] { (ICommand) getNameCommandMap().get("ReadMultiTierFinCommand") };
		}
		// before saving the whole object, we need to save the current tab first
			...
			return new ICommand[] { (ICommand) getNameCommandMap().get("DeleteMultiTierFinCommand") };
		}
	...
	protected IPage getNextPage(String event, HashMap map, HashMap exceptionMap) {
		...
		else if (TAB_MULTI_TIER_FINANCING_CHECKER.equals(event)) {
			forward = TAB_MULTI_TIER_FINANCING_CHECKER;
		}
		else if (TAB_MULTI_TIER_FINANCING_CHECKER_PROCESS.equals(event)) {
		...
		else if (TAB_MULTI_TIER_FINANCING_VIEW_WO_FRAME.equals(event)) {
		...
		else if (TAB_MULTI_TIER_FINANCING_VIEW.equals(event)) {
	...
	public ActionErrors validateInput(ActionForm aForm, Locale locale) {
		...
		return MultiTierFinFormValidator.validateInput((MultiTierFinForm) aForm, locale);
	}
	...
	protected String getErrorEvent(String event) {
		...
		else if (EVENT_SEARCH_CUSTOMER_WO_FRAME.equals(event) || EVENT_ADD_WO_FRAME.equals(event)) {
