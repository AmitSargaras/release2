...
public class ReadListOfficerCommand extends FacilityMainCommand {
	...
	public String[][] getResultDescriptor() {
		return (new String[][] {
				{ "facilityOfficers", "[Lcom.integrosys.cms.app.limit.bus.IFacilityOfficer", REQUEST_SCOPE },
	...
	public HashMap doExecute(HashMap map) throws CommandProcessingException, CommandValidationException {
		...
		try {
            ...
            if (errorMap != null) {
                ActionErrors errorMessages = (ActionErrors) errorMap.get("officer");
			...
			if (facilityMaster != null) {
				...
				List listOfficers = new ArrayList();
				if (setOfficers != null) {
					listOfficers.addAll(setOfficers);
				}
				recordSize = listOfficers.size();
				Collections.sort(listOfficers, new FacilityOfficerComparator());
				if (listOfficers != null && listOfficers.size() != 0) {

					if ((startIdx + FacilityMainAction.RECORD_PER_PAGE) <= listOfficers.size()) {
						listOfficers = listOfficers.subList(startIdx, (startIdx + FacilityMainAction.RECORD_PER_PAGE));
					}
					else {
						listOfficers = listOfficers.subList(startIdx, startIdx
								+ ((listOfficers.size() - startIdx) % FacilityMainAction.RECORD_PER_PAGE));
					}
					IFacilityOfficer[] facilityOfficers = (IFacilityOfficer[]) listOfficers
							.toArray(new OBFacilityOfficer[listOfficers.size()]);

					result.put("facilityOfficers", facilityOfficers);
					result.put("startIdx", String.valueOf(startIdx));
				}
			}
			result.put("recordSize", String.valueOf(recordSize));
			result.put("facilityOfficerObj", null);
		}
		catch (Exception e) {
