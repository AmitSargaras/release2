...
public class FacilityIncrementalAction extends FacilityMainAction {
	protected ICommand[] getCommandChain(String event) {
		if (EVENT_LIST.equals(event) || EVENT_LIST_WO_FRAME.equals(event) ||
				TAB_FACILITY_INCREMENTAL_CHECKER.equals(event) ||
				TAB_FACILITY_INCREMENTAL_CHECKER_PROCESS.equals(event) ||
				TAB_FACILITY_INCREMENTAL_VIEW.equals(event) ||
				TAB_FACILITY_INCREMENTAL_VIEW_WO_FRAME.equals(event)) {
			return new ICommand[] { (ICommand) getNameCommandMap().get("ReadListIncrementalCommand")};
		}	
			...
			return new ICommand[] { (ICommand) getNameCommandMap().get("ReadIncrementalCommand") };
		}	
		else if (EVENT_UPDATE.equals(event) || EVENT_UPDATE_WO_FRAME.equals(event)) {
            return new ICommand[] { (ICommand) getNameCommandMap().get("SaveIncrementalDetailToSessionCommand")	};
        }		
	...
	protected IPage getNextPage(String event, HashMap map, HashMap exceptionMap) {
		...
		if (EVENT_ADD.equals(event) || EVENT_EDIT.equals(event) || EVENT_ERROR.equals(event)) {
			...
			forward = TAB_FACILITY_INCREMENTAL+"_no_frame";
		}
		else if (EVENT_UPDATE.equals(event)) {
			forward = TAB_FACILITY_INCREMENTAL;
		}		
	...
	public ActionErrors validateInput(ActionForm aForm, Locale locale) {
		...
		return FacilityIncrementalFormValidator.validateInput((FacilityIncrementalForm)aForm, locale);
	}
