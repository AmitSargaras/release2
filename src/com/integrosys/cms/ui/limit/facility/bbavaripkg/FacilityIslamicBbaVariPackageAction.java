...
public class FacilityIslamicBbaVariPackageAction extends FacilityMainAction {
	
	protected ICommand[] getCommandChain(String event) {
		if (EVENT_VIEW_EDIT.equals(event) || EVENT_VIEW_EDIT_WO_FRAME.equals(event) 
				|| TAB_ISLAMIC_BBA_VARI_PACKAGE_CHECKER.equals(event) || TAB_ISLAMIC_BBA_VARI_PACKAGE_VIEW_WO_FRAME.equals(event) 
				|| TAB_ISLAMIC_BBA_VARI_PACKAGE_VIEW.equals(event)||TAB_ISLAMIC_BBA_VARI_PACKAGE_CHECKER_PROCESS.equals(event)) {
			return new ICommand[] { (ICommand) getNameCommandMap().get("ReadFacilityIslamicBbaVariPackageCommand")};
		}
		else if (EVENT_NAVIGATE.equals(event) || EVENT_NAVIGATE_WO_FRAME.equals(event)) {
			return new ICommand[] { (ICommand) getNameCommandMap().get("SaveFacilityIslamicBbaVariPackageToSessionCommand"),
	...
	protected IPage getNextPage(String event, HashMap map, HashMap exceptionMap) {
		...
		else if (TAB_ISLAMIC_BBA_VARI_PACKAGE_CHECKER.equals(event)) {
			forward = TAB_ISLAMIC_BBA_VARI_PACKAGE_CHECKER;
		}
		else if (TAB_ISLAMIC_BBA_VARI_PACKAGE_CHECKER_PROCESS.equals(event)) {
			forward = TAB_ISLAMIC_BBA_VARI_PACKAGE_CHECKER_PROCESS;
		}		
		else if (TAB_ISLAMIC_BBA_VARI_PACKAGE_VIEW_WO_FRAME.equals(event)) {
			forward = TAB_ISLAMIC_BBA_VARI_PACKAGE_VIEW_WO_FRAME;
		}
		else if (TAB_ISLAMIC_BBA_VARI_PACKAGE_VIEW.equals(event)) {
	...
	public ActionErrors validateInput(ActionForm aForm, Locale locale) {
		...
		return FacilityIslamicBbaVariPackageFormValidator.validateInput((FacilityIslamicBbaVariPackageForm) aForm, locale);
	}
