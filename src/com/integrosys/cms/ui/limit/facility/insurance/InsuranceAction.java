...
public class InsuranceAction extends FacilityMainAction {
	protected ICommand[] getCommandChain(String event) {
		if (EVENT_VIEW_EDIT.equals(event) || EVENT_VIEW_EDIT_WO_FRAME.equals(event)
				|| TAB_INSURANCE_CHECKER.equals(event) || TAB_INSURANCE_VIEW_WO_FRAME.equals(event)
				|| TAB_INSURANCE_VIEW.equals(event)||TAB_INSURANCE_CHECKER_PROCESS.equals(event)) {
			return new ICommand[] { (ICommand) getNameCommandMap().get("ReadInsuranceCommand") };
		}
		else if (EVENT_NAVIGATE.equals(event) || EVENT_NAVIGATE_WO_FRAME.equals(event)) {
			return new ICommand[] { (ICommand) getNameCommandMap().get("SaveInsuranceToSessionCommand"),
	...
	protected IPage getNextPage(String event, HashMap map, HashMap exceptionMap) {
		...
		if (EVENT_VIEW_EDIT_WO_FRAME.equals(event) || EVENT_ERROR_WO_FRAME.equals(event)) {
			forward = EVENT_VIEW_EDIT_WO_FRAME;
		}
		else if (EVENT_CLOSE.equals(event)) {
			forward = EVENT_CLOSE;
		}
		else if (TAB_INSURANCE_CHECKER.equals(event)) {
			forward = TAB_INSURANCE_CHECKER;
		}
		else if (TAB_INSURANCE_CHECKER_PROCESS.equals(event)) {
			forward = TAB_INSURANCE_CHECKER_PROCESS;
		}
		else if (TAB_INSURANCE_VIEW_WO_FRAME.equals(event)) {
			forward = TAB_INSURANCE_VIEW_WO_FRAME;
		}
		else if (TAB_INSURANCE_VIEW.equals(event)) {
	...
	public ActionErrors validateInput(ActionForm aForm, Locale locale) {
		...
		return InsuranceFormValidator.validateInput((InsuranceForm) aForm, locale);
	}

	protected boolean isValidationRequired(String event) {
		return (EVENT_NAVIGATE.equals(event) || EVENT_NAVIGATE_WO_FRAME.equals(event)
				|| EVENT_SAVE_WHOLE_OBJ.equals(event) || EVENT_SUBMIT.equals(event));
	}
