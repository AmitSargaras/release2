...
public class FacilityMasterAction extends FacilityMainAction {
	protected ICommand[] getCommandChain(String event) {
		if (EVENT_VIEW_EDIT.equals(event) || EVENT_VIEW_EDIT_WO_FRAME.equals(event) || EVENT_PROCESS.equals(event)
				|| EVENT_PREPARE_CLOSE.equals(event) || TAB_MASTER_CHECKER.equals(event)||TAB_MASTER_CHECKER_PROCESS.equals(event)
				|| TAB_MASTER_VIEW_WO_FRAME.equals(event) || TAB_MASTER_VIEW.equals(event)||EVENT_VIEW.equals(event)) {
			...
			return new ICommand[] { (ICommand) getNameCommandMap().get("SaveFacilityMasterToSessionCommand"),
	...
	protected IPage getNextPage(String event, HashMap map, HashMap exceptionMap) {
		...
		else if (EVENT_PROCESS.equals(event) || EVENT_VIEW_EDIT_WO_FRAME.equals(event)
				|| EVENT_ERROR_WO_FRAME.equals(event)) {
			forward = EVENT_VIEW_EDIT_WO_FRAME;
		}
		else if (EVENT_CLOSE.equals(event)) {
			forward = EVENT_CLOSE;
		}
		else if (TAB_MASTER_CHECKER.equals(event)||EVENT_PREPARE_CLOSE.equals(event)||EVENT_VIEW.equals(event)) {
			forward = TAB_MASTER_CHECKER;
		}
		else if(TAB_MASTER_CHECKER_PROCESS.equals(event) ){
			forward = TAB_MASTER_CHECKER_PROCESS;
		}
		else if (EVENT_VIEW_WO_FRAME.equals(event)) {
			forward = EVENT_VIEW_WO_FRAME;
		}
		else if (TAB_MASTER_VIEW_WO_FRAME.equals(event)) {
			forward = TAB_MASTER_VIEW_WO_FRAME;
		}
		else if (TAB_MASTER_VIEW.equals(event)) {
