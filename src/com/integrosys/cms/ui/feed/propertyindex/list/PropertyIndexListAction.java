...
import com.integrosys.cms.ui.feed.propertyindex.PropertyIndexAction;

/**
 * This action implements ...
 */
public class PropertyIndexListAction extends PropertyIndexAction {
	...
	public ICommand[] getCommandChain(String event) {
		...
		if (EVENT_READ.equals(event)) {
			return new ICommand[] { new ReadPropertyIndexListCommand() };
			// return new ICommand[]{new
			// ReadActualToStagingPropertyIndexListCommand()};
		}
			...
			return new ICommand[] { new AddPropertyIndexListCommand() };
		}
		else if (EVENT_ADD_NOOP.equals(event)) {
			return null;
		}
		else if (EVENT_REMOVE.equals(event)) {
			return new ICommand[] { new DeletePropertyIndexListCommand() };
		}
		else if (EVENT_REMOVE_NOOP.equals(event)) {
			return null;
		}
		else if (EVENT_SAVE.equals(event)) {
			return new ICommand[] { new SavePropertyIndexListCommand() };
		}
		else if (EVENT_PAGINATE.equals(event)) {
			return new ICommand[] { new PaginatePropertyIndexListCommand() };
		}
		else if (EVENT_SAVE_NOOP.equals(event)) {
			return null;
		}
		else if (EVENT_SUBMIT.equals(event)) {
			return new ICommand[] { new SubmitPropertyIndexListCommand() };
		}
		else if (EVENT_SUBMIT_NOOP.equals(event)) {
			return null;
		}
		else if (EVENT_READ_CHECKER_APPROVE_REJECT.equals(event)) {
			return new ICommand[] { new ReadPropertyIndexListCommand(), new ComparePropertyIndexListCommand() };
		}
		else if (EVENT_READ_MAKER_CLOSE.equals(event)) {
			return new ICommand[] { new ReadPropertyIndexListCommand() };
		}
		else if (EVENT_APPROVE.equals(event)) {
			return new ICommand[] { new ApprovePropertyIndexListCommand() };
		}
		else if (EVENT_REJECT.equals(event)) {
			return new ICommand[] { new RejectPropertyIndexListCommand() };
		}
		else if (EVENT_LIST_CHECKER_APPROVE_REJECT.equals(event)) {
			return new ICommand[] { new ListComparePropertyIndexListCommand() };
		}
		else if (EVENT_LIST_MAKER_CLOSE.equals(event)) {
			return new ICommand[] { new ListViewPropertyIndexListCommand() };
		}
		else if (EVENT_CLOSE.equals(event)) {
			return new ICommand[] { new ClosePropertyIndexListCommand() };
		}
			...
			return new ICommand[] { new ListPropertyIndexListCommand() };
		}
	...
	public ActionErrors validateInput(ActionForm aForm, Locale locale) {
		DefaultLogger.debug(this, "Inside validate Input  class" + ((PropertyIndexListForm) aForm));

		return PropertyIndexListFormValidator.validateInput((PropertyIndexListForm) aForm, locale);
	}
	...
	public IPage getNextPage(String event, HashMap resultMap, HashMap exceptionMap) {
		...
		IPropertyIndexFeedGroupTrxValue value = (IPropertyIndexFeedGroupTrxValue) resultMap
