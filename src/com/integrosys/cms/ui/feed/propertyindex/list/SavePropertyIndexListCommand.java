...
$Header: /home/cms2/cvsroot/cms2/src/com/integrosys/cms/ui/feed/propertyindex/list/SavePropertyIndexListCommand.java,v 1.8 2005/08/30 09:51:18 hshii Exp $
 */
 ...
 */
public class SavePropertyIndexListCommand extends AbstractCommand {
	...
	public HashMap doExecute(HashMap map) throws CommandProcessingException, CommandValidationException {
		...
		try {
			...
			if (value.getStatus().equals(ICMSConstant.STATE_REJECTED)) {
				resultValue = proxy.makerUpdateRejectedPropertyIndexFeedGroup(trxContext, value);
			}
			else {
				resultValue = proxy.makerUpdatePropertyIndexFeedGroup(trxContext, value, value
						.getStagingPropertyIndexFeedGroup());
			}

			DefaultLogger.debug(this, "after: " + resultValue.getVersionTime());

			entriesArr = resultValue.getStagingPropertyIndexFeedGroup().getFeedEntries();
			...
			resultValue.getStagingPropertyIndexFeedGroup().setFeedEntries(entriesArr);
			...
			targetOffset = PropertyIndexListMapper.adjustOffset(targetOffset, length, entriesLength);

			resultMap.put("request.ITrxValue", resultValue);
			resultMap.put("propertyIndexFeedGroupTrxValue", resultValue);
			resultMap.put("offset", new Integer(targetOffset));
			resultMap.put(PropertyIndexListForm.MAPPER, resultValue);
