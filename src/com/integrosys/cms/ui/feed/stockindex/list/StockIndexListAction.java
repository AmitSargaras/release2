 ...
 */
public class StockIndexListAction extends StockIndexAction {
	...
	/**
	 * <p>
	 * Set the "Name - {@link com.integrosys.base.uiinfra.common.ICommand}" pair
	 * into the map. <code>&lt;String, ICommand&gt;</code>
	 * 
	 * <p>
	 * To be used by {@link #getCommandChain(String)}
	 * 
	 * @param nameCommandMap <code>&lt;String, ICommand&gt;</code> pair map to
	 *        be injected
	 */
	...
	/**
	 * @return <code>&lt;String, ICommand&gt;</code> pair map injected
	 */
	...
	public ICommand[] getCommandChain(String event) {
		...
		if (EVENT_READ.equals(event)) {
			return new ICommand[] { (ICommand) getNameCommandMap().get("ReadStockIndexListCommand") };
		}
			...
			return new ICommand[] { (ICommand) getNameCommandMap().get("AddStockIndexListCommand") };
		}
		else if (EVENT_ADD_NOOP.equals(event)) {
			return null;
		}
		else if (EVENT_REMOVE.equals(event)) {
			return new ICommand[] { (ICommand) getNameCommandMap().get("DeleteStockIndexListCommand") };
		}
		else if (EVENT_REMOVE_NOOP.equals(event)) {
			return null;
		}
		else if (EVENT_SAVE.equals(event)) {
			return new ICommand[] { (ICommand) getNameCommandMap().get("SaveStockIndexListCommand") };
		}
		else if (EVENT_PAGINATE.equals(event)) {
			return new ICommand[] { (ICommand) getNameCommandMap().get("PaginateStockIndexListCommand") };
		}
		else if (EVENT_SAVE_NOOP.equals(event)) {
			return null;
		}
		else if (EVENT_SUBMIT.equals(event)) {
			return new ICommand[] { (ICommand) getNameCommandMap().get("SubmitStockIndexListCommand") };
		}
			...
			commands[0] = (ICommand) getNameCommandMap().get("ReadStockIndexListCommand");
			commands[1] = (ICommand) getNameCommandMap().get("CompareStockIndexListCommand");
			...
			return new ICommand[] { (ICommand) getNameCommandMap().get("ApproveStockIndexListCommand"),
					(ICommand) getNameCommandMap().get("ReloadMarketableSecValuationProfileCommand") };
		}
		else if (EVENT_REJECT.equals(event)) {
			return new ICommand[] { (ICommand) getNameCommandMap().get("RejectStockIndexListCommand") };
		}
		else if (EVENT_LIST_CHECKER_APPROVE_REJECT.equals(event)) {
			return new ICommand[] { (ICommand) getNameCommandMap().get("ListCompareStockIndexListCommand") };
		}
		else if (EVENT_LIST_MAKER_CLOSE.equals(event)) {
			return new ICommand[] { (ICommand) getNameCommandMap().get("ListViewStockIndexListCommand") };
		}
		else if (EVENT_CLOSE.equals(event)) {
			return new ICommand[] { (ICommand) getNameCommandMap().get("CloseStockIndexListCommand") };
		}
			...
			return new ICommand[] { (ICommand) getNameCommandMap().get("ListStockIndexListCommand") };
		}
	...
	public ActionErrors validateInput(ActionForm aForm, Locale locale) {
		DefaultLogger.debug(this, "Inside validate Input  class" + ((StockIndexListForm) aForm));

		return StockIndexListFormValidator.validateInput((StockIndexListForm) aForm, locale);
	}
	...
	public IPage getNextPage(String event, HashMap resultMap, HashMap exceptionMap) {
		...
		IStockIndexFeedGroupTrxValue value = (IStockIndexFeedGroupTrxValue) resultMap
