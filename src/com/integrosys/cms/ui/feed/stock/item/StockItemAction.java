 ...
 */
public class StockItemAction extends StockAction {
	...
	public ICommand[] getCommandChain(String event) {
		...
		if (EVENT_PREPARE.equals(event)) {
			return new ICommand[] { (ICommand) getNameCommandMap().get("PrepareStockItemCommand") };
		}
		else if (EVENT_SAVE.equals(event)) {
			return new ICommand[] { (ICommand) getNameCommandMap().get("SaveStockItemCommand") };
		}
		 ...
		 /**********File Upload****************/
		else if (event.equals(MAKER_PREPARE_UPLOAD_STOCKITEM)) {
			return new ICommand[] { (ICommand) getNameCommandMap().get("MakerPrepareUploadStockItemCommand") };
		}
		else if (event.equals(MAKER_EVENT_UPLOAD_STOCKITEM)) {
			return new ICommand[] { (ICommand) getNameCommandMap().get("MakerUploadStockItemCommand") };
		}
			...
			return new ICommand[] { (ICommand) getNameCommandMap().get("CheckerApproveInsertStockItemCommand") };
		}else if ( event.equals(CHECKER_REJECT_INSERT)) {
			return new ICommand[] { (ICommand) getNameCommandMap().get("CheckerRejectInsertStockItemCommand") };
			...
			return new ICommand[] { (ICommand) getNameCommandMap().get("MakerInsertCloseStockItemCommand") };
		}
		 /************************/
	...
	public ActionErrors validateInput(ActionForm aForm, Locale locale) {
		DefaultLogger.debug(this, "Inside validate Input  class" + ((StockItemForm) aForm));

		return StockItemFormValidator.validateInput((StockItemForm) aForm, locale);
	}
	...
	public IPage getNextPage(String event, HashMap resultMap, HashMap exceptionMap) {
		...
		/*Add by govind for file upload*/
				...
				forward = "maker_fileupload_stockItem_page";
			}
		 else if (event.equals("maker_event_upload_stockItem")) {
			 forward = "common_submit_page";
	    }
		 else if ((event != null) && event.equals("maker_prepare_upload_stockItem")) {
			 forward = "prepare_upload_stockItem_page";
				...
				forward = "checker_stockItem_insert_page";
			}else if ((event != null)
	...
	public static final String MAKER_PREPARE_UPLOAD_STOCKITEM = "maker_prepare_upload_stockItem";
	public static final String MAKER_EVENT_UPLOAD_STOCKITEM = "maker_event_upload_stockItem";
