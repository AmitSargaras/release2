 ...
 */
public class MutualFundsListAction extends MutualFundsAction {
	...
	public ICommand[] getCommandChain(String event) {
		...
		if (EVENT_READ.equals(event)) {
			return new ICommand[] { (ICommand) getNameCommandMap().get("ReadMutualFundsListCommand") };
			// return new ICommand[]{new ReadActualToStagingMutualFundsListCommand()};
		}
			...
			return new ICommand[] { (ICommand) getNameCommandMap().get("AddMutualFundsListCommand") };
		}
		else if (EVENT_ADD_NOOP.equals(event)) {
			return new ICommand[] { (ICommand) getNameCommandMap().get("AddMutualFundsListCommand") };
		}
		else if (EVENT_REMOVE.equals(event)) {
			return new ICommand[] { (ICommand) getNameCommandMap().get("DeleteMutualFundsListCommand") };
		}
		else if (EVENT_REMOVE_NOOP.equals(event)) {
			return null;
		}
		else if (EVENT_SAVE.equals(event)) {
			return new ICommand[] { (ICommand) getNameCommandMap().get("SaveMutualFundsListCommand") };
		}
		else if (EVENT_PAGINATE.equals(event)) {
			return new ICommand[] { (ICommand) getNameCommandMap().get("PaginateMutualFundsListCommand") };
		}
		else if (EVENT_SAVE_NOOP.equals(event)) {
			return null;
		}
		else if (EVENT_SUBMIT.equals(event)) {
			return new ICommand[] { (ICommand) getNameCommandMap().get("SubmitMutualFundsListCommand") };
		}
			...
			return new ICommand[] { (ICommand) getNameCommandMap().get("ReadMutualFundsListCommand"),
					(ICommand) getNameCommandMap().get("CompareMutualFundsListCommand") };
		}
		else if (EVENT_READ_MAKER_CLOSE.equals(event)) {
			return new ICommand[] { (ICommand) getNameCommandMap().get("ReadMutualFundsListCommand") };
		}
		else if (EVENT_APPROVE.equals(event)) {
			return new ICommand[] { (ICommand) getNameCommandMap().get("ApproveMutualFundsListCommand"),
					(ICommand) getNameCommandMap().get("ReloadMarketableSecValuationProfileCommand") };
		}
		else if (EVENT_REJECT.equals(event)) {
			return new ICommand[] { (ICommand) getNameCommandMap().get("RejectMutualFundsListCommand") };
		}
		else if (EVENT_LIST_CHECKER_APPROVE_REJECT.equals(event)) {
			return new ICommand[] { (ICommand) getNameCommandMap().get("ListCompareMutualFundsListCommand") };
		}
		else if (EVENT_LIST_MAKER_CLOSE.equals(event)) {
			return new ICommand[] { (ICommand) getNameCommandMap().get("ListViewMutualFundsListCommand") };
		}
		else if (EVENT_CLOSE.equals(event)) {
			return new ICommand[] { (ICommand) getNameCommandMap().get("CloseMutualFundsListCommand") };
		}
		else if (EVENT_LIST_STAGING.equals(event)) {
			return new ICommand[] { (ICommand) getNameCommandMap().get("ListMutualFundsListCommand") };
		}
	...
	public ActionErrors validateInput(ActionForm aForm, Locale locale) {
		DefaultLogger.debug(this, "Inside validate Input  class" + ((MutualFundsListForm) aForm));

		return MutualFundsListFormValidator.validateInput((MutualFundsListForm) aForm, locale);
	}
	...
	public IPage getNextPage(String event, HashMap resultMap, HashMap exceptionMap) {
		...
		IMutualFundsFeedGroupTrxValue value = (IMutualFundsFeedGroupTrxValue) resultMap.get("mutualFundsFeedGroupTrxValue");
