...
public class GoldListAction extends GoldAction {
	...
	protected ICommand[] getCommandChain(String event) {
		...
		if (EVENT_READ.equals(event)) {
			return new ICommand[] { (ICommand) getNameCommandMap().get("ReadGoldListCommand") };
		}
			...
			return new ICommand[] { (ICommand) getNameCommandMap().get("AddGoldListCommand") };
		}
		else if (EVENT_ADD_NOOP.equals(event)) {
			return null;
		}
		else if (EVENT_REMOVE.equals(event)) {
			return new ICommand[] { (ICommand) getNameCommandMap().get("DeleteGoldListCommand") };
		}
		else if (EVENT_REMOVE_NOOP.equals(event)) {
			return null;
		}
		else if (EVENT_SAVE.equals(event)) {
			return new ICommand[] { (ICommand) getNameCommandMap().get("SaveGoldListCommand") };
		}
		else if (EVENT_PAGINATE.equals(event)) {
			return new ICommand[] { (ICommand) getNameCommandMap().get("PaginateGoldListCommand") };
		}
		else if (EVENT_SAVE_NOOP.equals(event)) {
			return null;
		}
		else if (EVENT_SUBMIT.equals(event)) {
			return new ICommand[] { (ICommand) getNameCommandMap().get("SubmitGoldListCommand") };
		}
			...
			return new ICommand[] { (ICommand) getNameCommandMap().get("ReadGoldListCommand"),
					(ICommand) getNameCommandMap().get("CompareGoldListCommand") };
		}
		else if (EVENT_READ_MAKER_CLOSE.equals(event)) {
			return new ICommand[] { (ICommand) getNameCommandMap().get("ReadGoldListCommand") };
		}
		else if (EVENT_APPROVE.equals(event)) {
			return new ICommand[] { (ICommand) getNameCommandMap().get("ApproveGoldListCommand"),
					(ICommand) getNameCommandMap().get("ReloadGoldValuationProfileCommand")};
		}
		else if (EVENT_REJECT.equals(event)) {
			return new ICommand[] { (ICommand) getNameCommandMap().get("RejectGoldListCommand") };
		}
		else if (EVENT_LIST_CHECKER_APPROVE_REJECT.equals(event)) {
			return new ICommand[] { (ICommand) getNameCommandMap().get("ListCompareGoldListCommand") };
		}
		else if (EVENT_LIST_MAKER_CLOSE.equals(event)) {
			return new ICommand[] { (ICommand) getNameCommandMap().get("ListViewGoldListCommand") };
		}
		else if (EVENT_CLOSE.equals(event)) {
			return new ICommand[] { (ICommand) getNameCommandMap().get("CloseGoldListCommand") };
		}
		else if (EVENT_LIST_STAGING.equals(event)) {
			return new ICommand[] { (ICommand) getNameCommandMap().get("ListGoldListCommand") };
		}
	...
	public ActionErrors validateInput(ActionForm aForm, Locale locale) {
		DefaultLogger.debug(this, "Inside validate Input  class " + ((GoldListForm) aForm));
		return GoldListFormValidator.validateInput((GoldListForm) aForm, locale);
	}
	...
	protected IPage getNextPage(String event, HashMap resultMap, HashMap exceptionMap) {
		...
		IGoldFeedGroupTrxValue value = (IGoldFeedGroupTrxValue) resultMap.get("goldFeedGroupTrxValue");
