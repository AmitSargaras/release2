...
import com.integrosys.cms.ui.feed.unittrust.UnitTrustAction;

/**
 * This action implements ...
 */
public class UnitTrustListAction extends UnitTrustAction {
	...
	public ICommand[] getCommandChain(String event) {
		...
		if (EVENT_READ.equals(event)) {
			return new ICommand[] { (ICommand) getNameCommandMap().get("ReadUnitTrustListCommand") };
			...
			return new ICommand[] { (ICommand) getNameCommandMap().get("AddUnitTrustListCommand") };
		}
		else if (EVENT_EDIT_FEED.equals(event)) {
			return new ICommand[] { (ICommand) getNameCommandMap().get("UpdateUnitTrustListCommand") };
		}
		else if (EVENT_ADD_NOOP.equals(event)) {
			return null;
		}
		else if (EVENT_REMOVE.equals(event)) {
			return new ICommand[] { (ICommand) getNameCommandMap().get("DeleteUnitTrustListCommand") };
		}
		else if (EVENT_REMOVE_NOOP.equals(event)) {
			return null;
		}
		else if (EVENT_SAVE.equals(event)) {
			return new ICommand[] { (ICommand) getNameCommandMap().get("SaveUnitTrustListCommand") };
		}
		else if (EVENT_PAGINATE.equals(event)) {
			return new ICommand[] { (ICommand) getNameCommandMap().get("PaginateUnitTrustListCommand") };
		}
		else if (EVENT_SAVE_NOOP.equals(event)) {
			return null;
		}
		else if (EVENT_SUBMIT.equals(event)) {
			return new ICommand[] { (ICommand) getNameCommandMap().get("SubmitUnitTrustListCommand") };
		}
			...
			return new ICommand[] { (ICommand) getNameCommandMap().get("ReadUnitTrustListCommand"),
					(ICommand) getNameCommandMap().get("CompareUnitTrustListCommand") };
		}
		else if (EVENT_READ_MAKER_CLOSE.equals(event)) {
			return new ICommand[] { (ICommand) getNameCommandMap().get("ReadUnitTrustListCommand") };
		}
		else if (EVENT_APPROVE.equals(event)) {
			return new ICommand[] { (ICommand) getNameCommandMap().get("ApproveUnitTrustListCommand"),
					(ICommand) getNameCommandMap().get("ReloadMarketableSecValuationProfileCommand") };
		}
		else if (EVENT_REJECT.equals(event)) {
			return new ICommand[] { (ICommand) getNameCommandMap().get("RejectUnitTrustListCommand") };
		}
		else if (EVENT_LIST_CHECKER_APPROVE_REJECT.equals(event)) {
			return new ICommand[] { (ICommand) getNameCommandMap().get("ListCompareUnitTrustListCommand") };
		}
		else if (EVENT_LIST_MAKER_CLOSE.equals(event)) {
			return new ICommand[] { (ICommand) getNameCommandMap().get("ListViewUnitTrustListCommand") };
		}
		else if (EVENT_CLOSE.equals(event)) {
			return new ICommand[] { (ICommand) getNameCommandMap().get("CloseUnitTrustListCommand") };
		}
			...
			return new ICommand[] { (ICommand) getNameCommandMap().get("ListUnitTrustListCommand") };
		}
	...
	public ActionErrors validateInput(ActionForm aForm, Locale locale) {
		DefaultLogger.debug(this, "Inside validate Input  class" + ((UnitTrustListForm) aForm));
		return UnitTrustListFormValidator.validateInput((UnitTrustListForm) aForm, locale);
	}
	...
	public IPage getNextPage(String event, HashMap resultMap, HashMap exceptionMap) {
		...
		IUnitTrustFeedGroupTrxValue value = (IUnitTrustFeedGroupTrxValue) resultMap.get("unitTrustFeedGroupTrxValue");
	...
	public static final String EVENT_EDIT_FEED = "editFeed";
}
