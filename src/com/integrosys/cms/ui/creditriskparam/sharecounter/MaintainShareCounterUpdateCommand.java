 ...
 * $Header$
 *
 * MaintainShareCounterUpdateCommand
 *
 * Created on 4:04:41 PM
 *
 * Purpose:
 * Description:
 *
...
/**
 * Created by IntelliJ IDEA. User: Eric Date: Feb 21, 2007 Time: 4:04:41 PM
 */
public class MaintainShareCounterUpdateCommand extends AbstractCommand implements ICommonEventConstant {
	...
	public HashMap doExecute(HashMap hashMap) throws CommandValidationException, CommandProcessingException,
		...
		try {
			stagingData = MaintainShareCounterUtil.mergeStagingWithOb(actual, obTrx);

			DefaultLogger.debug(this, "trxValue.getReferenceID () : " + trxValue.getReferenceID());
			DefaultLogger.debug(this, "moa: " + stagingData.getFeedEntries()[0].getMarginOfAdvance());
			// DefaultLogger.debug ( this ,
			// "AccessorUtil.printMethodValue ( stagingData ) : " +
			// AccessorUtil.printMethodValue ( stagingData.getFeedEntries () ) )
			// ;
			// DefaultLogger.debug ( this ,
			// "stagingData.getFeedEntries ().length : " +
			// stagingData.getFeedEntries ().length ) ;
			//
			// for ( int loop = 0 ; loop < stagingData.getFeedEntries ().length
			// ; loop ++ )
			// {
			// DefaultLogger.debug ( this , "AccessorUtil.printMethodValue ( " +
			// loop + " ) : " + AccessorUtil.printMethodValue (
			// stagingData.getFeedEntries ()[ loop ] ) ) ;
			//
			// }

			ICMSTrxValue temp = proxy.makerUpdateCreditRiskParam(ctx, trxValue, stagingData,
					CreditRiskParamType.SHARE_COUNTER);

			result.put("request.ITrxValue", temp);
