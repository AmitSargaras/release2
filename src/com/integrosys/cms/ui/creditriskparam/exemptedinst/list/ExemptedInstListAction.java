 ...
 * Tag:     $Name:  $
 */
public class ExemptedInstListAction extends ExemptedInstAction {
	...
	protected ICommand[] getCommandChain(String event) {
		...
		if (EVENT_READ.equals(event)) {
			...
			objArray[0] = new ReadExemptedInstListCommand();			
		}
			...
			objArray = new ICommand[1];
			objArray[0] = new DeleteExemptedInstListCommand();
			
		} else if (EVENT_SUBMIT.equals(event)) {
            return new ICommand[]{new SubmitExemptedInstListCommand()};
        } else if (EVENT_SUBMIT_NOOP.equals(event)) {
            return null;
            
        } else if (EVENT_VIEW.equals(event)) {
            return new ICommand[]{new ReadExemptedInstListCommand()};
        } else if (EVENT_READ_CHECKER_APPROVE_REJECT.equals(event)) {
            return new ICommand[]{new ReadExemptedInstListCommand(),
                    new CompareExemptedInstListCommand()};
        } else if (EVENT_READ_MAKER_CLOSE.equals(event)) {
	        return new ICommand[]{new ReadExemptedInstListCommand()};
        
        } else if (EVENT_APPROVE.equals(event)) {
            return new ICommand[]{new ApproveExemptedInstListCommand()};
        } else if (EVENT_REJECT.equals(event)) {
          
        	return new ICommand[]{new RejectExemptedInstListCommand()};
			...
			objArray = new ICommand[1];
			objArray[0] = new CloseExemptedInstListCommand();			
		} else if (EVENT_CANCEL.equals(event)) {
			objArray = new ICommand[1];
			objArray[0] = new CancelExemptedInstListCommand();
		}
		return objArray;
	}
    ...
    public ActionErrors validateInput(ActionForm aForm, Locale locale) {
        DefaultLogger.debug(this, "Inside validate Input  class " +  ((ExemptedInstListForm) aForm));
        return ExemptedInstListValidator.validateInput( (ExemptedInstListForm) aForm, locale);
    }
    ...
    public IPage getNextPage(String event, HashMap resultMap,
        ...
        IExemptedInstTrxValue value = (IExemptedInstTrxValue) resultMap.get(
