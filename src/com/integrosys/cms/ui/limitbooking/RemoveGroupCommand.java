...
public class RemoveGroupCommand extends AbstractCommand implements ICommonEventConstant {
     /**
     * Default Constructor
     */
    public RemoveGroupCommand() {
        DefaultLogger.debug(this,"Inside RemoveGroupCommand >>>>>>>>>>>>>>>>>>>>>>>>>>>");
    }
    ...
    public HashMap doExecute(HashMap map) throws CommandProcessingException, CommandValidationException {
        ...
        try {
            ...
            List bankGroupList = null;
            if (limitBooking  != null) {
            	bankGroupList = limitBooking.getBankGroupList();
            }
            
            String[] deletedBankGroupList = (String [])map.get(LimitBookingAction.LIMIT_BOOKING);
            
            OBBankGroupDetail dtl = null;

            for (int i=0; i<deletedBankGroupList.length; i++) { 
                
                for (Iterator iterator = bankGroupList.iterator(); iterator.hasNext();) {
                    dtl = (OBBankGroupDetail) iterator.next();
                    if (dtl.getBkgTypeCode().equals(deletedBankGroupList[i])) {
                    	 break;
                    }    
                }
                if (dtl != null) {
                	bankGroupList.remove(dtl);
                }
                    
            }
            limitBooking.setBankGroupList(bankGroupList);
