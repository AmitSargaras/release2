...
public class ReadProcessBulkUDFFile  extends AbstractCommand  implements ICommonEventConstant {
	...
	public HashMap doExecute(HashMap map) throws CommandProcessingException, CommandValidationException,FileUploadException {
		...
		try {
			...
			SearchResult bulkUDFFileList = new SearchResult();
			...
			bulkUDFFileList = jdbc.getAllBulkUDFFile(trxValue.getStagingReferenceID());
			
			List uploadList=new ArrayList();
			if(bulkUDFFileList!=null){
				uploadList = new ArrayList(bulkUDFFileList.getResultList());		
			}
			long countPass = 0;
			long countFail = 0;
			
					for (int i = 0; i < uploadList.size(); i++) {
						OBTempBulkUDFFileUpload oBTempBulkUDFFileUpload = (OBTempBulkUDFFileUpload) uploadList.get(i);
	
						if ("FAIL".equals(oBTempBulkUDFFileUpload.getStatus()) && "N".equals(oBTempBulkUDFFileUpload.getUploadStatus())) {
							countFail++;
						} else if ("PASS".equals(oBTempBulkUDFFileUpload.getStatus()) && "Y".equals(oBTempBulkUDFFileUpload.getUploadStatus())) {
							countPass++;
						}
					}
