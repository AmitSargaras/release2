 ...
 */
public class ApproveCCColTaskCommand extends AbstractCommand implements ICommonEventConstant {
	/**
	 * Default Constructor
	 */
	public ApproveCCColTaskCommand() {
	}
	...
	public HashMap doExecute(HashMap map) throws CommandProcessingException, CommandValidationException {
		...
		try {
			...
			ICCTask stagingTask = colTrxVal.getStagingCCTask();
			if (customer.getNonBorrowerInd()) {
				ctx.setTrxCountryOrigin(CCColTaskUtil.getCCColTaskTrxCountry(ctx, stagingTask, null, customer));
			}
			else {
				ILimitProfile limit = (ILimitProfile) map.get(IGlobalConstant.GLOBAL_LIMITPROFILE_OBJ);
				ctx.setTrxCountryOrigin(CCColTaskUtil.getCCColTaskTrxCountry(ctx, stagingTask, limit, customer));
			}
			String remarks = (String) map.get("colRemarks");
			stagingTask.setRemarks(remarks);
			colTrxVal.setStagingCCTask(stagingTask);
			ctx.setRemarks(remarks);

			proxy.checkerApproveCollaborationTask(ctx, colTrxVal);
